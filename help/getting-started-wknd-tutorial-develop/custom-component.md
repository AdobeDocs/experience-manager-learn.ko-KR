...
>[!NOTE]
>
> 자습서의 이전 부분을 따라 했다면 이 장에 대한 시작 프로젝트가 구현을 가속화하는 것을 볼 수 있습니다. 템플릿과 더 많은 콘텐츠가 포함되어 있습니다. 또한 사용자 지정 구성 요소 개발 외부의 새로운 컨텐츠 및 기타 구현 영역을 자유롭게 탐색할 수 있습니다.
...
1. **ui.apps** 모듈에서 `/apps/wknd/components/content`로 이동하여 `cq:Component` 유형의 **byline**&#x200B;이라는 이름의 새 노드를 만듭니다.
...
1. `byline` 노드 아래에 구성 요소의 HTML 프레젠테이션을 담당하는 새 파일 `byline.html`을 추가합니다. `cq:Component` 노드와 동일한 파일 이름을 지정하는 것은 Sling이 이 리소스 유형을 렌더링하는 데 사용하는 기본 스크립트가 되기 때문에 중요합니다.
...
1. `byline` 구성 요소 노드 아래에 `nt:unstructured` 유형의 `cq:dialog`이라는 새 노드를 만듭니다.
1. 다음 XML로 `cq:dialog`을 업데이트합니다. `.content.xml`을 열고 다음 XML을 복사하여 붙여 넣는 것이 가장 쉽습니다.
   ...
   이러한 노드 정의는 [Sling 리소스 합병](https://sling.apache.org/documentation/bundles/resource-merger.html)을 사용하여 `sling:resourceSuperType` 구성 요소에서 상속되는 대화 상자 탭을 제어합니다. 이 경우 **핵심 구성 요소의 이미지 구성 요소**&#x200B;입니다.
...
1. 다음 XML로 `cq:design_dialog`을 업데이트합니다. `.content.xml`을 열고 아래에 있는 XML을 복사하여 붙여 넣는 것이 가장 쉽습니다.
...
[Byline 구성 요소를 **WKND.Content** 구성 요소 그룹](#create-component-node)에 추가했으므로 `/apps/wknd/components/content/byline@componentGroup=WKND.Content` 속성을 통해 **레이아웃 컨테이너**&#x200B;에서 **Policy**&#x200B;이 **WKND.A를 허용하는 모든 &lt;a5/>레이아웃 컨테이너&lt;a3/>에 자동으로 사용할 수 있습니다. 컨텐츠** 구성 요소 그룹. 아티클 페이지의 레이아웃 컨테이너는 다음과 같습니다.

#### 구성 요소를 페이지 {#drag-and-drop-the-component-onto-the-page}에 드래그하여 놓습니다.

1. **** AEM >  **사이트 > WKND 사이트 > 언어 기본 > 영어 > 잡지 > LA Skatetpark에 대한 최종 가이드**&#x200B;에서 아티클 페이지를 편집합니다.
...
AEM 작성자는 대화 상자를 통해 구성 요소를 구성하고 작성합니다. 이 시점에서 [필자] 구성 요소 개발의 경우 대화 상자는 데이터를 수집하는 데 포함되지만 제작된 컨텐츠를 렌더링하는 논리는 아직 추가되지 않았습니다.
...
1. 대화 상자를 저장한 후 [CRXDE Lite](http://localhost:4502/crx/de/index.jsp#/content/wknd/language-masters/en/magazine/guide-la-skateparks/jcr:content/root/responsivegrid/responsivegrid/byline)으로 이동하여 구성 요소의 컨텐츠가 AEM 페이지의 byline 구성 요소 컨텐츠 노드에 어떻게 저장되는지 확인합니다.

   `jcr:content/root/responsivegrid/responsivegrid` 노드 아래의 Byline 구성 요소 컨텐트 노드(예: `/content/wknd/language-masters/en/magazine/guide-la-skateparks/jcr:content/root/responsivegrid/responsivegrid/byline`)를 찾습니다.
   ...
   */content/wknd/language-masters/en/magazine/guide-la-skateskateparks/jcr:content/root/responsivegrid/responsivegrid/byline*
...
Byline Sling Model은 AEM에서 제공하는 여러 Java API를 사용합니다. 이러한 API는 `core` 모듈의 POM 파일에 나열된 `dependencies`을 통해 사용할 수 있습니다.
...
1. pom 파일의 종속성 섹션에서 `uber-jar`에 대한 종속성을 찾습니다.
   ...
   [uber-jar](https://docs.adobe.com/content/help/en/experience-manager-65/developing/devtools/ht-projects-maven.html#experience-manager-api-dependencies)에는 AEM에서 노출한 모든 공개 Java API가 포함되어 있습니다. `core/pom.xml` 파일에 버전이 지정되지 않았습니다. 버전은 대신 프로젝트 `aem-guides-wknd/pom.xml`의 루트에 있는 상위 원자로 요소에서 유지 관리됩니다.
...
1. 열기 `BylineImpl.java`. `Byline.java` 인터페이스에 정의된 모든 메서드로 자동 채워집니다. 다음 클래스 수준 주석과 함께 `BylineImpl.java`을 업데이트하여 Sling 모델 주석을 추가합니다. 이 `@Model(..)`주석을 사용하면 클래스가 슬링 모델로 변경됩니다.
...
1. 이 리소스를 핵심 구성 요소 이미지 처리 모델로 변환하고 `getSrc()` 메서드가 비어 있지 않은지 확인합니다.
...
3. 코드 베이스를 로컬 AEM 인스턴스에 배포합니다. POM 파일에 대한 주요 변경 사항이 있으므로 프로젝트의 루트 디렉토리에서 전체 Maven 빌드를 수행합니다.
   ...
   >[!WARNING]
   >
   > WKND 프로젝트는 `ui.content`이 JCR의 모든 변경 사항을 덮어쓰도록 설정되므로 `ui.apps` 프로젝트만 배포하여 이전에 아티클 페이지에 추가된 Byline 구성 요소를 삭제하지 않도록 해야 합니다.
...
1. 업데이트를 배포한 후 [LA Skatetparks ](http://localhost:4502/editor.html/content/wknd/language-masters/en/magazine/guide-la-skateparks.html) 페이지로 이동하거나 장 앞 부분에 있는 Byline 구성 요소를 추가한 위치로 이동합니다.
...
스타일링 후 필라인 구성 요소는 다음의 미적 요소를 채택해야 합니다.

![byline mock 스타일](./assets/custom-component/byline-design.png)

*WKND 크리에이티브 팀이 정의한 대로 구성 요소 디자인*

### 기본 스타일 추가

Byline 구성 요소의 기본 스타일을 추가합니다. `/src/main/webpack/components/content` 아래의 **ui.frontend** 프로젝트에서:

1. `byline`이라는 새 폴더를 만듭니다.
1. `scss` 폴더 아래에 새 폴더를 만듭니다.`byline`
1. `byline.scss` 폴더 아래에 새 파일을 만듭니다.`byline/scss`
1. `styles` 폴더 아래에 새 폴더를 만듭니다.`byline/scss`
...
1. 기본 스타일을 포함하려면 **byline.scss**&#x200B;을(를) 채우는 방식으로 시작합니다.
...
1. `/src/main/webpack/site` 아래의 **ui.frontend** 프로젝트에서 `main.scss` 파일을 열고 `/* Components */` 섹션에 다음 줄을 추가합니다.

   ```scss
   @import '../components/content/byline/scss/byline.scss';
   ```

1. NPM을 사용하여 `ui.frontend` 모듈을 작성하고 컴파일합니다.
...
1. Maven을 사용하여 `ui.frontend` 프로젝트를 전환 방식으로 포함하는 `ui.apps` 프로젝트를 로컬 AEM 인스턴스에 빌드하고 배포합니다.
...
이 튜토리얼에 내장된 기능을 살펴보려면 아래 비디오를 참조하십시오.
...
[GitHub](https://github.com/adobe/aem-guides-wknd)에서 완료된 코드를 보거나 Git brach `custom-component/solution`에서 코드를 로컬로 검토 및 배포합니다.
...
Eclipse에 `src/main/java` 소스 폴더가 표시되지 않으면 src를 마우스 오른쪽 단추로 클릭하고 기본 및 java에 대한 폴더를 추가하여 폴더를 추가할 수 있습니다. 폴더를 추가하면 `src/main/java` 패키지가 표시됩니다.
...
>[!NOTE]
>
> 핵심 프로젝트에 추가된 새 종속성 중 일부에 대해 해결되지 않은 패키지 가져오기가 있는 경우 aem-guides-wknd maven 프로젝트를 업데이트하면 모든 하위 프로젝트를 업데이트할 수 있습니다. **aem-guides-wknd > Maven > 프로젝트 업데이트**&#x200B;를 마우스 오른쪽 단추로 클릭하여 이 작업을 수행할 수 있습니다.
