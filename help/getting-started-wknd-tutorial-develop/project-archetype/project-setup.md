...
이 장에서는 [AEM Project Archetype](https://github.com/adobe/aem-project-archetype)을(를) 사용하여 새 Adobe Experience Manager 프로젝트를 생성합니다. AEM 프로젝트에는 Sites 구현에 사용되는 전체 코드, 콘텐츠 및 구성이 포함됩니다. 이 장에서 생성된 프로젝트는 WKND Site 구현의 기반 역할을 하며 향후 장에서 빌드됩니다.
...
AEM용 Maven 다중 모듈 프로젝트를 제작하기 위한 두 가지 옵션이 있습니다. 이 자습서에서는 [Maven AEM Project Archetype **35**](https://github.com/adobe/aem-project-archetype)을(를) 사용합니다. 또한 Cloud Manager은 AEM 응용 프로그램 프로젝트 만들기를 시작할 수 있도록 [UI 마법사를 제공합니다](https://experienceleague.adobe.com/docs/experience-manager-cloud-manager/content/getting-started/project-creation/using-the-wizard.html). Cloud Manager UI에서 생성된 기본 프로젝트는 Archetype을 직접 사용하는 것과 동일한 구조를 갖습니다.
...
1. 다음 내용을 명령줄에 붙여 넣어 [일괄 처리 모드에서 프로젝트를 생성](https://maven.apache.org/archetype/maven-archetype-plugin/examples/generate-batch.html)합니다.
   ...
   >[!NOTE]
   >
   > AEM 6.5.14+를 타깃팅하려면 `aemVersion="cloud"`을(를) `aemVersion="6.5.14"`(으)로 바꾸십시오.
   >
...
1. 포트 **4502**&#x200B;에서 로컬로 실행 중인 AEM 작성자 인스턴스가 있는지 확인하십시오.
...
1. 다음 명령을 실행하여 전체 프로젝트를 빌드하고 AEM에 배포합니다.
   ...
   Maven 프로필 `autoInstallSinglePackage`이(가) 프로젝트의 개별 모듈을 컴파일하고 단일 패키지를 AEM 인스턴스에 배포합니다. 기본적으로 이 패키지는 포트 **4502**&#x200B;에서 로컬로 실행 중이고 자격 증명이 `admin:admin`인 AEM 인스턴스에 배포됩니다.
...
* [core](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/core.html) - Java 코드, 주로 백엔드 개발자입니다.
...
* [ui.apps](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/uiapps.html) - 구성 요소 및 대화 상자 정의를 포함하며, 컴파일된 CSS 및 JavaScript을 클라이언트 라이브러리로 임베드합니다.
* [ui.content](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/uicontent.html) - 구조적 콘텐츠와 편집 가능한 템플릿, 메타데이터 스키마(/content, /conf)와 같은 구성을 포함합니다.

* **모두** - 위의 모듈을 AEM 환경에 배포할 수 있는 단일 패키지로 결합하는 빈 Maven 모듈입니다.

![Maven 프로젝트 다이어그램](assets/project-setup/project-pom-structure.png)

Maven 모듈 **모두**&#x200B;에 대한 자세한 내용은 [AEM Project Archetype 설명서](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/overview.html)를 참조하세요.

### 핵심 구성 요소 포함 {#core-components}

[AEM 핵심 구성 요소](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/introduction.html)는 AEM용 표준화된 WCM(웹 콘텐츠 관리) 구성 요소 집합입니다. 이러한 구성 요소는 기본 기능 세트를 제공하며 개별 프로젝트에 대해 스타일링, 사용자 정의 및 확장됩니다.

AEM as a Cloud Service 환경에는 최신 버전의 [AEM 핵심 구성 요소](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/introduction.html?lang=ko-KR)가 포함되어 있습니다. 따라서 AEM as a Cloud Service에 대해 생성된 프로젝트에는 AEM 핵심 구성 요소 임베드가 **포함되지 않습니다**.

AEM 6.5/6.4 생성 프로젝트의 경우 Archetype은 프로젝트에 [AEM 핵심 구성 요소](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/introduction.html?lang=ko-KR)를 자동으로 임베드합니다. AEM 6.5/6.4에서는 최신 버전이 프로젝트와 함께 배포되도록 AEM 핵심 구성 요소를 포함하는 것이 좋습니다. 프로젝트에 [포함된 핵심 구성 요소에 대한 자세한 내용은 여기](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/using.html#core-components)에서 확인할 수 있습니다.
...
[구성 요소 기본 사항](component-basics.md) 자습서와 함께 간단한 `HelloWorld` 예제를 통해 Adobe Experience Manager(AEM) Sites 구성 요소의 기본 기술을 이해합니다.
...
**[core](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/core.html)** 모듈에 프로젝트와 연결된 모든 Java™ 코드가 포함되어 있습니다. **core** 모듈 빌드는 OSGi 번들을 AEM에 배포합니다. 이 모듈만 만들려면:
...
**[ui.apps](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/uiapps.html)** Maven 모듈에는 `/apps` 아래 사이트에 필요한 모든 렌더링 코드가 포함되어 있습니다. 여기에는 [clientlibs](https://experienceleague.adobe.com/docs/experience-manager-65/developing/introduction/clientlibs.html)라는 AEM 형식으로 저장된 CSS/JS가 포함됩니다. 동적 HTML 렌더링에 [HTL](https://experienceleague.adobe.com/docs/experience-manager-htl/content/overview.html) 스크립트도 포함됩니다. **ui.apps** 모듈은 JCR의 구조에 대한 맵으로 간주할 수 있지만 파일 시스템에 저장되고 소스 제어에 커밋될 수 있는 형식으로 간주할 수 있습니다. **ui.apps** 모듈에는 코드만 포함되어 있습니다.
...
1. 명령줄로 돌아가서 `ui.apps` 폴더 내에서 다음 명령을 실행합니다.
   ...
   `autoInstallPackagePublish` 프로필은 **4503** 포트에서 실행 중인 Publish 환경에 패키지를 배포하기 위한 것입니다. http://localhost:4503에서 실행 중인 AEM 인스턴스를 찾을 수 없는 경우 위의 오류가 발생합니다.
...
1. 마지막으로 다음 명령을 실행하여 **4504** 포트에서 `ui.apps` 패키지를 배포합니다.
   ...
   포트 **4504**&#x200B;에서 실행 중인 AEM 인스턴스를 사용할 수 없으면 빌드 오류가 다시 발생합니다. `aem.port` 매개 변수가 `aem-guides-wknd/pom.xml`의 POM 파일에 정의되어 있습니다.
