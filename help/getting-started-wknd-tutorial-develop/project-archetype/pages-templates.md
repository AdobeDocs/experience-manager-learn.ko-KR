...
1. 추가 **이미지** 구성 요소를 **main** 컨테이너.
   * 잠금 해제 **이미지** 구성 요소.
1. 추가 **이동 경로** 구성 요소 아래에 **이미지** 기본 컨테이너의 구성 요소입니다.
   * 다음에 대한 정책 만들기 **이동 경로** 구성 요소 이름 **문서 페이지 - 이동 경로**. 설정 **탐색 시작 수준** 끝 **4**.
1. 추가 **컨테이너** 구성 요소 아래에 **이동 경로** 구성 요소 및 내부 **main** 컨테이너. 이는 다음과 같이 작동합니다. **콘텐츠 컨테이너** 템플릿에 사용됩니다.
   * 잠금 해제 **콘텐츠** 컨테이너.
   * 정책을 다음으로 설정 **페이지 컨텐츠**.
...
1. 다음에 대한 참여 레이아웃 모드 **측면 레일 컨테이너**. 핸들을 오른쪽에서 왼쪽으로 드래그하고 컨테이너를 4열 너비로 축소합니다. 그런 다음 왼쪽 핸들을 왼쪽에서 오른쪽으로 한 열로 드래그하여 컨테이너를 3열 너비로 만들고 사이에 1열 간격을 둡니다. **콘텐츠 컨테이너**.
