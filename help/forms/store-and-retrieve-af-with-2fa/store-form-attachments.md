...
적응형 양식에 첨부 파일을 추가하면 첨부 파일이 CRX 저장소의 임시 위치에 저장됩니다. 사용 사례를 수행하려면 양식 첨부 파일을 CRX 저장소의 새 위치에 저장해야 합니다.

OSGi 서비스는 양식 첨부 파일을 CRX 저장소의 새 위치에 저장하도록 작성됩니다. CRX에서 첨부 파일의 새 위치를 사용하여 새 파일 맵이 만들어지고 호출 애플리케이션으로 반환됩니다.
다음은 서블릿에 전송되는 FileMap입니다. 키는 적응형 양식 필드이며 값은 첨부 파일의 임시 위치입니다. 서블릿에서는 첨부 파일을 추출하여 AEM 저장소의 새 위치에 저장하고 새 위치로 FileMap을 업데이트합니다
...
다음은 요청에서 첨부 파일을 추출하여 저장하는 코드입니다 **/content/afattachments** 폴더
...
[양식 데이터를 저장합니다](./store-form-data.md)