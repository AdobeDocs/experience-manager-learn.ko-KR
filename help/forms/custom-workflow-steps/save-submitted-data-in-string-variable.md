---
title: 제출된 데이터를 문자열 변수로 저장
description: 바인딩된 데이터를 추출하여 문자열 유형의 워크플로우 변수에 저장하는 사용자 지정 프로세스 단계입니다
...
# 바인딩된 데이터를 추출하여 문자열 변수에 저장합니다

이 기능을 사용하면 제출된 데이터를 이메일 본문에 포함할 수 있습니다. 사용자 지정 프로세스 단계에서는 **바인딩된 데이터** 적응형 양식 제출에서 을 채우며 유형 문자열의 변수를 데이터로 채웁니다. 그런 다음 이 문자열 변수를 사용하여 이메일 템플릿에 데이터를 삽입할 수 있습니다.
다음 스크린샷은 사용자 지정 프로세스 단계에 전달해야 하는 인수를 보여 줍니다
![프로세스 단계](assets/save-submitted-data-string.png)

다음은 매개 변수입니다

* `data.xml` - 제출된 데이터가 있는 파일입니다. 형식이 json인 경우 파일의 이름은 data.json일 수 있습니다.

사용자 지정 프로세스 단계에서는 바인딩된 데이터를 추출하여 워크플로우에 정의된 submittedDataString 변수에 저장합니다


[여기에서 사용자 지정 번들을 다운로드할 수 있습니다](assets/AEMFormsProcessStep.core-1.0.0-SNAPSHOT.jar)
