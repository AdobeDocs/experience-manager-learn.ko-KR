...
Workflow 데이터를 받은 편지함에 표시하려면 Workflow에서 변수를 정의하고 채워야 합니다. 사용자에게 작업을 할당하기 전에 변수 값을 설정해야 합니다. 신속한 시작을 위해 AEM 서버에 배포할 수 있는 샘플 워크플로우를 제공했습니다.
...
이 워크플로우에는 정의된 변수(isMarried 및 income)가 두 개 있으며 변수 설정 구성 요소를 사용하여 해당 값이 설정됩니다. 이러한 변수는 AEM 받은 편지함에 추가할 열로 사용할 수 있습니다

## 서비스 만들기

받은 편지함에 표시해야 하는 모든 열에 대해 서비스를 작성해야 합니다. 다음 서비스를 통해 isMarried 변수의 값을 표시하는 열을 추가할 수 있습니다
