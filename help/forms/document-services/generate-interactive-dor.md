...
일반적인 사용 사례는 적응형 양식 데이터가 있는 대화형 DoR을 다운로드할 수 있는 것입니다. 다운로드한 DoR은 Adobe Acrobat 또는 Adobe Reader를 사용하여 완료됩니다.
...
다음 코드는 PDF 다운로드 버튼이 트리거될 때 트리거됩니다
...
이전 단계의 XSD를 기반으로 적응형 양식을 만듭니다. 클라이언트 라이브러리 &quot;irs&quot;를 사용하도록 양식을 연결하십시오. 이 클라이언트 라이브러리에는 호출 애플리케이션으로 PDF을 반환하는 서블릿에 대해 POST 호출을 수행하는 코드가 있습니다.
...
이 샘플 코드에서는 xdp 이름과 기타 매개 변수가 요청 개체에서 추출됩니다. 양식이 XSD를 기반으로 하지 않으면 새 XML 문서가 만들어져 xdp와 병합됩니다. 그러나 양식이 XSD 기반인 경우 해당 노드가 적응형 양식의 제출 데이터에서 직접 추출되고 그에 따라 XML 문서가 생성되어 xdp 템플릿과 병합됩니다.
...
>[!NOTE]
>
>브라우저의 pdf 뷰어를 사용하여 다운로드한 PDF을 열면 pdf에 데이터가 표시되지 않습니다. Adobe Acrobat 또는 Adobe Reader를 사용하여 다운로드한 PDF을 엽니다.
