...
AEM에서 외부 데이터베이스와 통합할 수 있는 방법에는 여러 가지가 있습니다. 데이터베이스를 통합하는 가장 일반적인 방법 중 하나는 를 통해 Apache Sling 연결의 풀링된 데이터 소스 구성 속성을 사용하는 것입니다. [configMgr](http://localhost:4502/system/console/configMgr).
...
>[!NOTE]
>데이터 소스의 이름을 지정하십시오 **aemformtutorial**. 샘플 코드는 이름을 사용하여 데이터베이스에 연결합니다.
...
스키마를 만들 SQL 파일은 [여기에서 다운로드](assets/sign-multiple-forms.sql). 스키마와 테이블을 만들려면 MySql Workbench를 사용하여 이 파일을 가져와야 합니다.
