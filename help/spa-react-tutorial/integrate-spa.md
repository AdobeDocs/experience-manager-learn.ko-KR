...
description: Response로 기록된 단일 페이지 애플리케이션(SPA)의 소스 코드를 Adobe Experience Manager(AEM) 프로젝트와 통합하는 방법을 알아봅니다. 웹 팩 개발 서버와 같은 최신 프런트 엔드 툴을 사용하여 AEM JSON 모델 API를 통해 SPA을 신속하게 개발하는 방법을 살펴볼 수 있습니다.
sub-product: sites
...
Response로 기록된 단일 페이지 애플리케이션(SPA)의 소스 코드를 Adobe Experience Manager(AEM) 프로젝트와 통합하는 방법을 알아봅니다. 웹 팩 개발 서버와 같은 최신 프런트 엔드 툴을 사용하여 AEM JSON 모델 API를 통해 SPA을 신속하게 개발하는 방법을 살펴볼 수 있습니다.
...
그런 다음 `ui.frontend` 모듈을 검사하여 [AEM Project 원형](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/archetype/uifrontend-react.html)에 의해 자동으로 생성된 SPA을 파악합니다.
...
3. `dependencies` 아래에 `react-scripts`를 포함하여 `react`과(와) 관련된 여러 개가 표시됩니다.
...
5. `package.json` 파일에는 몇 개의 `scripts`이 정의되어 있습니다.
   ...
   다음은 반응형 앱 제작에서 [available](https://create-react-app.dev/docs/available-scripts)로 만든 표준 빌드 스크립트입니다.

   유일한 차이는 `build` 스크립트에 `&& clientlib`을 추가하는 것입니다. 이 추가 지침은 빌드 중에 컴파일된 SPA을 클라이언트 측 라이브러리로 `ui.apps` 모듈로 복사해야 합니다.
...
8. Inspect `ui.frontend/src/index.js`의 `index.js` 파일:
   ...
   `index.js` 는 SPA의 중심점입니다. `ModelManager` 는 AEM SPA Editor JS SDK에서 제공됩니다. JSON 컨텐츠)를 호출하고 애플리케이션에 삽입해야 합니다.`pageModel`

## 헤더 구성 요소 {#header-component} 추가

그런 다음 SPA에 새 구성 요소를 추가하고 변경 사항을 로컬 AEM 인스턴스에 배포합니다.

1. `ui.frontend` 모듈의 `ui.frontend/src/components` 아래에 `Header`라는 이름의 새 폴더를 만듭니다.
...
4. `ui.frontend/src/App.js` 파일을 엽니다. 애플리케이션 시작 지점입니다.
5. 정적 `Header`을 포함하려면 `App.js`을(를) 업데이트하십시오.
...
7. `ui.apps` 폴더로 이동합니다. `ui.apps/src/main/content/jcr_root/apps/wknd-spa-react/clientlibs/clientlib-react` 아래에서 컴파일된 SPA 파일이`ui.frontend/build` 폴더에서 복사되었음을 볼 수 있습니다.
...
9. 브라우저 탭을 열고 [http://localhost:4502/editor.html/content/wknd-spa-react/us/en/home.html](http://localhost:4502/editor.html/content/wknd-spa-react/us/en/home.html)으로 이동합니다. 이제 SPA에 표시되는 `Header` 구성 요소의 내용이 표시됩니다.

   ![초기 헤더 구현](./assets/integrate-spa/initial-header-implementation.png)

   6-8단계는 프로젝트의 루트에서 Maven 빌드를 트리거할 때 자동으로 실행됩니다(예: `mvn clean install -PautoInstallSinglePackage`). 이제 SPA 및 AEM 클라이언트측 라이브러리 간의 통합에 대한 기본 사항을 이해해야 합니다. 정적 `Header` 구성 요소 아래에서 AEM에서 `Text` 구성 요소를 편집하고 추가할 수 있습니다.
...
[webpack-dev-server](https://webpack.js.org/configuration/dev-server/)를 사용하여 SPA을 신속하게 개발할 수 있습니다. SPA은 AEM에서 생성된 JSON 모델을 기반으로 합니다. 이 연습에서는 AEM의 실행 중인 인스턴스의 JSON 콘텐츠가 개발 서버로 **proxied**&#x200B;됩니다.
...
3. 새 브라우저 탭(아직 열지 않은 경우)을 열고 [http://localhost:3000/content/wknd-spa-react/us/en/home.html](http://localhost:3000/content/wknd-spa-react/us/en/home.html)으로 이동합니다.
...
4. IDE로 돌아가 `ui.frontend/src/media`에 `media`이라는 새 폴더를 만듭니다.
...
8. [http://localhost:3000/content/wknd-spa-react/us/en/home.html](http://localhost:3000/content/wknd-spa-react/us/en/home.html)에서 브라우저로 돌아갑니다. 반영된 앱 변경 사항을 즉시 확인할 수 있습니다.

   ![헤더에 추가된 로고](./assets/integrate-spa/added-logo-localhost.png)

   컨텐츠를 프록시하고 있으므로 AEM에서 컨텐츠를 계속 업데이트하고 **webpack-dev-server**&#x200B;에 반영된 내용을 볼 수 있습니다.

9. 터미널에 `ctrl+c`이(가) 있는 웹 팩 개발 서버를 중지합니다.
...
3. `ui.frontend/public/mock-content` 아래에 `mock.model.json`이라는 이름의 새 파일을 만듭니다. 여기에 **1단계**&#x200B;의 JSON 출력을 붙여 넣습니다.
...
5. `ui.frontend/.env.development` 파일을 열고 다음 업데이트를 수행하여 `REACT_APP_PAGE_MODEL_PATH` 파일의 이전 값을 주석 지정합니다.
...
7. 이전에 만든 `mock.model.json` 파일을 약간 변경하십시오. **webpack-dev-server**&#x200B;에 즉시 반영되는 업데이트된 컨텐츠가 표시됩니다.
...
Available best practice는 각 구성 요소를 모듈화하고 자체 포함시키는 것입니다. 일반적인 권장 사항은 구성 요소 간에 동일한 CSS 클래스 이름을 다시 사용하지 않도록 하여 프리프로세서를 사용하는 것이 더 강력하지는 않음을 만드는 것입니다. 이 프로젝트는 [Sass](https://sass-lang.com/)을(를) 변수와 같은 유용한 몇 가지 기능에 사용합니다. 이 프로젝트는 [SUIT CSS 이름 지정 규칙](https://github.com/suitcss/suit/blob/master/doc/components.md)도 느슨하게 따릅니다. SUIT는 일관된 CSS 규칙을 만드는 데 사용되는 BEM 표기법인 블록 요소 수정자의 변형입니다.

1. 터미널 창을 열고 시작 시 **webpack-dev-server**&#x200B;를 중지합니다. `ui.frontend` 폴더 내부에서 [install Sass](https://create-react-app.dev/docs/adding-a-sass-stylesheet)에 다음 명령을 입력합니다.
...
5. `_variables.scss`이라는 `ui.frontend/src/styles` 아래에 새 파일을 만들고 다음 변수로 채웁니다.
...
6. `ui.frontend/src/index.css`에 있는 파일 `index.css`의 확장명을 **`index.scss`**&#x200B;로 다시 지정합니다. 내용을 다음으로 바꿉니다.
...
8. `ui.frontend/src/components/Header` 아래에 `Header.scss`이라는 이름의 새 파일을 만듭니다. 다음 항목으로 파일을 채웁니다.
...
9. `Header.js`을 업데이트하여 `Header.scss`을(를) 포함합니다.
...
10. 브라우저와 **webpack-dev-server**:[http://localhost:3000/content/wknd-spa-react/us/en/home.html](http://localhost:3000/content/wknd-spa-react/us/en/home.html)
   ...
   업데이트된 스타일이 추가되어 `Header` 구성 요소에 표시됩니다.
...
2. [http://localhost:4502/editor.html/content/wknd-spa-react/us/en/home.html](http://localhost:4502/editor.html/content/wknd-spa-react/us/en/home.html)으로 이동합니다. 로고와 스타일이 적용된 업데이트된 `Header`이 표시됩니다.
...
이 문제는 로컬 버전의 **Node.js** 및 **npm**&#x200B;이 [frontend-maven-plugin](https://github.com/eirslett/frontend-maven-plugin)에서 사용하는 버전과 다를 때 발생할 수 있습니다. `npm rebuild node-sass` 명령을 실행하면 문제를 일시적으로 수정하거나 `ui.frontend/node_modules` 폴더를 제거하고 다시 설치할 수 있습니다.
...
* 다음 실행 단계를 `npm run build` 단계 전에 `ui.frontend/pom.xml`에 추가합니다.
...
[SPA 구성 요소를 AEM 구성 요소에 매핑](map-components.md)  - AEM SPA Editor JS SDK를 사용하여 React 구성 요소를 Adobe Experience Manager(AEM) 구성 요소에 매핑하는 방법을 알아봅니다. 구성 요소 매핑을 사용하면 SPA SPA 편집기 내에서 AEM 구성 요소에 대한 동적 업데이트를 일반적인 AEM 작성과 비슷합니다.
