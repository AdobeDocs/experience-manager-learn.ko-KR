...
2. `ui.apps/src/main/content/jcr_root/apps/wknd-spa-react/components/card`으로 이동하여 `.content.xml` 파일을 봅니다.
   ...
   `sling:resourceSuperType` 속성은 `wknd-spa-react/components/image`을 가리킵니다. 즉, `Card` 구성 요소는 WKND SPA 이미지 구성 요소의 모든 기능을 상속합니다.
...
6. IDE 스위치에서 `ui.frontend` 모듈로 이동하고 `ui.frontend/src/components/Card`:

   ![반응형 구성 요소 시작](assets/extend-component/react-card-component-start.png)

7. Inspect 파일 `Card.js`
...
8. Inspect 메서드 `get imageContent()`:
   ...
   이 예에서는 기존 React Image 구성 요소 `Image`을(를) `Card` 구성 요소의 `this.props`으로 단순히 전달하여 다시 사용하도록 선택했습니다. 자습서의 후반부에 제목, 날짜 및 클릭유도문안 단추를 표시하도록 `get bodyContent()` 메서드가 구현됩니다.
...
6. **자산 메타데이터** 탭을 업데이트하여 **대체 텍스트** 및 **캡션**&#x200B;의 값을 추가합니다.
   ...
   현재 대화 상자를 업데이트한 후에는 추가 변경 사항이 표시되지 않습니다. 새 필드를 반응 구성 요소에 표시하려면 `Card` 구성 요소에 대한 슬링 모델을 업데이트해야 합니다.

7. 새 탭을 열고 [CRXDE-Lite](http://localhost:4502/crx/de/index.jsp#/content/wknd-spa-react/us/en/home/jcr%3Acontent/root/responsivegrid/card)로 이동합니다. `/content/wknd-spa-react/us/en/home/jcr:content/root/responsivegrid` 아래에 있는 컨텐트 노드를 Inspect하여 `Card` 구성 요소 컨텐츠를 찾습니다.
...
구성 요소 대화 상자의 값을 반응 구성 요소에 표시하려면 `Card` 구성 요소의 JSON을 채우는 Sling 모델을 업데이트해야 합니다. Adobe는 다음 두 가지 비즈니스 로직을 구현할 수 있는 기회를 가지고 있습니다.
...
7. `Card.java` 인터페이스에 정의된 추가 메서드를 구현합니다.
   ...
   >[!NOTE]
   >
   > [완성된 CardImpl.java는 여기에서 볼 수 있습니다](https://github.com/adobe/aem-guides-wknd-spa/blob/React/extend-component-solution/core/src/main/java/com/adobe/aem/guides/wknd/spa/react/core/models/impl/CardImpl.java).
...
3. 클릭유도문안 렌더링을 위해 `get ctaButton()` 메서드를 추가합니다.
...
4. `get lastModifiedDisplayDate()`에 대한 메서드를 추가하여 날짜를 나타내는 지역화된 문자열에 `this.props.cardLastModified`을(를) 변환합니다.
...
6. 제목, 클릭유도문안 및 마지막으로 수정한 날짜에 스타일을 지정하기 위해 sass 규칙이 이미 `Card.scss`에 추가되었습니다. 파일 맨 위에 있는 `Card.js`에 다음 줄을 추가하여 이러한 스타일을 포함하십시오.
...
8. [http://localhost:4502/editor.html/content/wknd-spa-react/us/en/home.html](http://localhost:4502/editor.html/content/wknd-spa-react/us/en/home.html)으로 이동하여 업데이트된 구성 요소를 확인합니다.
