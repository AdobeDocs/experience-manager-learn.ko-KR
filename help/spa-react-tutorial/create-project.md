...
description: AEM SPA 편집기와 통합된 반응형 애플리케이션의 시작점으로 Adobe Experience Manager(AEM) Maven 프로젝트를 사용하는 방법을 살펴볼 수 있습니다.
sub-product: sites
...
AEM SPA 편집기와 통합된 반응형 애플리케이션의 시작점으로 Adobe Experience Manager(AEM) Maven 프로젝트를 사용하는 방법을 살펴볼 수 있습니다.

## 목표

1. Maven 원형에서 구축된 새로운 AEM SPA 편집기 프로젝트의 구조를 이해할 수 있습니다.
2. 시작 프로젝트를 AEM의 로컬 인스턴스에 배포합니다.

## 구축 내용

이 장에서 새 AEM 프로젝트는 [AEM Project Tranype](https://github.com/adobe/aem-project-archetype)을 기반으로 배포됩니다. AEM 프로젝트는 매우 간단한 시작점으로 부트스트러피즈 SPA이 될 것입니다. 이 장에서 사용된 프로젝트는 WKND SPA의 구현 기반이 되며, 향후 장에 세워질 예정입니다.

![WKND SPA Responsive Starter Project](./assets/create-project/wknd-spa-react.png)

*WKND SPA의 사이트 계층 구조를 시작합니다.*

## 전제 조건

[로컬 개발 환경 설정](overview.md#local-dev-environment)에 대한 필수 도구 및 지침을 검토하십시오. **author** 모드에서 시작한 Adobe Experience Manager의 새 인스턴스가 로컬로 실행되고 있는지 확인합니다.

## 프로젝트 다운로드

AEM용 Maven 다중 모듈 프로젝트를 만드는 몇 가지 옵션이 있습니다. 이 자습서는 자습서 코드의 기초로 최신 [AEM 프로젝트 원형](https://github.com/adobe/aem-project-archetype)을 사용했습니다. 여러 버전의 AEM을 지원하기 위해 프로젝트 코드를 수정했습니다. [이전 버전과의 호환성에 대한 참고 사항](overview.md#compatibility)을 검토하십시오.

>[!CAUTION]
>
> 실제 구현에 사용할 새 프로젝트를 생성하기 위해 **최신** 버전의 [원형&lt;a1/> 버전을 사용하는 것이 좋습니다. ](https://github.com/adobe/aem-project-archetype) AEM 프로젝트는 원형형의 `aemVersion` 속성을 사용하여 AEM의 단일 버전을 대상으로 해야 합니다.
...
3. 다음 속성은 [AEM 프로젝트 원형](https://github.com/Adobe-Marketing-Cloud/aem-project-archetype/releases/tag/aem-project-archetype-14)에서 AEM 프로젝트를 생성할 때 사용되었습니다.
   ...
   >[!NOTE]
   >
   > `frontendModule=react` 속성을 확인합니다. 이렇게 하면 AEM SPA AEM 편집기와 함께 사용할 시작 [React code base](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/archetype/uifrontend-react.html)로 프로젝트를 부트스트래핑할 프로젝트 원형이 됩니다.
...
4. 로컬 AEM 인스턴스의 **[!UICONTROL 패키지 관리자]**&#x200B;로 이동합니다.[http://localhost:4502/crx/packmgr/index.jsp](http://localhost:4502/crx/packmgr/index.jsp).

5. `wknd-spa-react.all`, `wknd-spa-react.ui.apps` 및 `wknd-spa-react.ui.content`에 대한 세 개의 패키지가 표시됩니다.
...
6. 또한 `spa.project.core` 및 `core.wcm.components`에 대한 여러 패키지가 표시됩니다. 이러한 종속성은 원형 유형에 의해 자동으로 포함됩니다. [AEM 코어 구성 요소에 대한 자세한 내용은 ](https://docs.adobe.com/content/help/ko-KR/experience-manager-core-components/using/introduction.html)에서 참조할 수 있습니다.

   `spa.project.core` 는 SPA 편집기가 기대하는 JSON 모델 API를 생성하는 데 필요한 종속성이 있습니다.

## 컨텐츠 작성

그 다음 원형에서 생성된 시작 SPA을 열고 일부 컨텐츠를 업데이트합니다.
...
4. 브라우저의 개발자 도구를 열고 새로 고치는 동안 페이지의 네트워크 트래픽을 검사합니다. **XHR** 요청 보기:
   ...
   [http://localhost:4502/content/wknd-spa-react/us/en.model.json](http://localhost:4502/content/wknd-spa-react/us/en.model.json)에 대한 요청이 있어야 합니다. 여기에는 SPA을 구동하는 JSON으로 구성된 모든 컨텐츠가 포함되어 있습니다.
...
5. 새 탭에서 [http://localhost:4502/content/wknd-spa-react/us/en.model.json](http://localhost:4502/content/wknd-spa-react/us/en.model.json) 열기
   ...
   요청 `en.model.json`은 응용 프로그램을 구동할 컨텐트 모델을 나타냅니다. JSON 출력을 Inspect에 삽입하고 **[!UICONTROL Text]** 구성 요소를 나타내는 조각을 찾을 수 있어야 합니다.
   ...
   다음 장에서는 이 JSON 컨텐츠가 AEM 구성 요소에서 SPA 구성 요소로 어떻게 매핑되어 AEM SPA 편집기 경험의 기반이 되는지 살펴봅니다.
...
SPA은 아주 간단합니다. 다음 몇 장에서 더 많은 기능이 추가됩니다.

### 다음 단계 {#next-steps}

[SPA](integrate-spa.md)  통합 - SPA 소스 코드가 AEM Project와 어떻게 통합되어 있는지 알아보고 SPA을 신속하게 개발할 수 있는 툴을 이해합니다.
