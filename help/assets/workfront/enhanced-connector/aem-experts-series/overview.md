---
title: 강화 커넥터를 사용하여 Workfront와 AEM Assets 통합
description: Adobe Workfront 및 Experience Manager Assets 강화 커넥터 통합의 기본 사항에 대해 알아봅니다.
version: Experience Manager as a Cloud Service
topic: Content Management, Integrations
feature: Asset Management
role: Admin, User
level: Beginner
jira: KT-9754
thumbnail: kt-9754.jpeg
last-substantial-update: 2022-09-02T00:00:00Z
badgeIntegration: label="통합" type="positive"
badgeVersions: label="AEM Assets as a Cloud Service, AEM Assets 6.5" before-title="false"
exl-id: 4f279c0d-270f-433e-b047-88c02f5b1e78
duration: 49
source-git-commit: 866dca32844a7185bdf3980e38570da1d44d8b83
workflow-type: tm+mt
source-wordcount: '328'
ht-degree: 85%

---

# 강화 커넥터로 Workfront와 AEM Assets 통합

![AEM 전문가 시리즈](./assets/banner.png)

4부로 구성된 이 비디오 시리즈에서 Adobe의 Workfront 및 Experience Manager Assets 전문가와 함께 강화 커넥터를 사용하여 Workfront와 AEM Assets를 통합하는 방법과 관련된 사항을 시연하고 논의하십시오.

4부로 구성된 이 전문가 시리즈에서는 제품 관리자에서 컨설턴트, Adobe 파트너 등 Adobe 전문가 그룹이 모여 다음 주제에 대해 논의합니다.

1. [Workfront 및 AEM에서 초기 설정 구성 수행](./setup.md)
2. [Workfront 사용자 정의 양식과 AEM 간 메타데이터 동기화 구성](./custom-forms.md)
3. [AEM 태그, 프로젝트 연결 폴더, Adobe Asset Link 및 폴더 메타데이터 스키마를 사용하여 가치 극대화](./aem-tags-project-linked-folders-and-folder-metadata.md)
4. [고급 AEM 워크플로를 사용하여 AEM 및 Workfront 전반의 비즈니스 프로세스를 간소화하고 기타 고급 설정 사용](./advanced-settings-and-workflows.md)

<table>
  <td>
      <a href="./setup.md">
        <img alt="설정 및 초기 구성" 
             src="./assets/setup.png">
      </a>
      <div>
         <a href="./setup.md"><strong>설정 및 초기 구성</strong></a>
         <br/><em>1/4부</em>
      </div>
      <p>
        <br/>
         Workfront for Experience Manager 강화 커넥터를 설정하고 초기 구성을 수행하는 방법을 알아봅니다.
      </p>
   </td>
   <!-- Workfront custom forms and metadata mapping -->
   <td>
      <a href="./custom-forms.md">
        <img alt="Workfront 사용자 정의 양식 및 메타데이터 매핑" 
             src="./assets/custom-forms.png">
      </a>
      <div>
         <a href="./custom-forms.md"><strong>Workfront 사용자 정의 양식 및 메타데이터 매핑</strong></a>
         <br/><em>2/4부</em>
      </div>
      <p>
        <br/>
         Workfront 사용자 정의 양식에 대해 알아보고 이를 Experience Manager Assets 메타데이터 스키마에 매핑하여 데이터를 동기화하는 방법을 알아봅니다.
      </p>
    </td>
    <!-- AEM Tags, project linked folders, and folder metadata -->
    <td>
      <a href="./aem-tags-project-linked-folders-and-folder-metadata.md">
        <img alt="AEM 태그, 프로젝트 연결 폴더 및 폴더 메타데이터" 
             src="./assets/aem-tags.png">
      </a>
      <div>
         <a href="./aem-tags-project-linked-folders-and-folder-metadata.md"><strong>AEM 태그, 프로젝트 연결 폴더 및 폴더 메타데이터</strong></a>
         <br/><em>3/4부</em> 
      </div>
      <p>
        <br/>
            Workfront에서 AEM 분류 체계를 구동하고, 프로젝트에 연결된 폴더와 데이터를 AEM 자산 폴더 메타데이터에 사용하는 방법을 알아봅니다.
      </p>
   </td>   
   <!-- Advanced workflows -->
    <td>
      <a href="./advanced-settings-and-workflows.md">
        <img alt="고급 설정 및 워크플로" 
             src="./assets/advanced.png">
      </a>
      <div>
         <a href="./advanced-settings-and-workflows.md"><strong>고급 설정 및 워크플로</strong></a>
         <br/><em>4/4부</em>
      </div>
      <p>
        <br/>
            Workfront for AEM 강화 커넥터의 고급 설정에 대해 알아보고 고급 워크플로를 사용하여 데이터 동기화를 관리하는 방법에 대해 알아봅니다.
      </p>
   </td>
  </tr>  
</tbody></table>

## Workfront Fusion을 통해 연결

Workfront Fusion을 사용하여 Workfront과 AEM Assets을 연결하는 경우 프로젝트 생성 및 워크플로 오케스트레이션을 자동화하려면 Workfron Fusion 및 AEM Assets 통합 설명서를 참조하십시오.

[Workfront Fusion을 사용하여 Workfront 문제를 AEM 워크플로가 있는 프로젝트로 변환](https://experienceleague.adobe.com/en/docs/workfront/using/documents/wf-aem-integrations/wf-aem-essentials/use-fusion-to-create-project-with-workflows)
