...
>[!CAUTION]
>
>를 반드시 다운로드하십시오 [핵심 구성 요소의 최신 버전](https://github.com/adobe/aem-core-wcm-components) 를 사용하십시오.

이 비디오에서는 더 이상 최신 버전이 아닌 코어 구성 요소 v2.2.2를 사용합니다. 다음 섹션을 진행하려면 먼저 최신 버전을 사용하십시오.

* 다운로드 [자산 통찰력 샘플 이미지 컨텐츠](./assets/asset-insights-launch-tutorial/aem-assets-insights-sample.zip)
...
핵심 이미지 구성 요소는 ***data-asset-id*** 상위 속성 내의 속성 &lt;div> 이미지 태그로 이 기능을 활성화/비활성화합니다. 프록시 구성 요소는 다음 변경 사항으로 코어 구성 요소를 재정의합니다.

* 를 제거합니다. ***data-asset-id*** image.html 내에 있는  &lt;img> 요소의 상위 div
* 추가 ***data-aem-asset-id*** image.html  &lt;img> 내의 요소에 바로 액세스할 수 있습니다.
* 추가 ***data-trackable=&#39;true&#39;*** image. &lt;img> html 내의 요소에 대한 값
* ***data-aem-asset-id*** 및 ***data-trackable=&#39;true&#39;*** 동일한 노드 수준에서 유지됩니다.

>[!NOTE]
>
>*data-aem-asset-id=&#39;image.UUID&#39;* 및 *data-trackable=&#39;true&#39;* 은 자산 노출 횟수에 대해 존재해야 하는 주요 속성입니다. 자산 클릭 인사이트의 경우 태그에 있는 위의 데이터 속성  &lt;img> 외에 상위 태그에는 유효한 href 값이 있어야 합니다.
...
* Analytics 회사 이름
* Analytics 사용자 이름
* 공유 암호( *Adobe Analytics > 관리자 > 회사 설정 > 웹 서비스*).
* 보고서 세트(자산 보고에 사용되는 올바른 보고서 세트를 선택해야 합니다)

## 4부: Adobe Analytics 확장 추가에 Adobe Experience Platform Launch 사용 {#part-using-launch-by-adobe-for-adding-adobe-analytics-extension}
...
* **\&lt;code>assetAnalytics.core.assetClicked\&lt;code>** : asset-DOM-요소에 대해 &#39;click&#39; 이벤트가 전달될 때 호출됨, 이는 asset-DOM-요소에 유효한 외부 &#39;href&#39; 속성을 가진 상위 태그가 있는 경우에만 관련이 있습니다

마지막으로 Pagetracker는 초기화 함수를 다음과 같이 구현합니다.

* **\&lt;code>assetAnalytics.dispatcher.init()\&lt;code>** : 를 호출하여 Pagetracker 구성 요소를 초기화합니다. 이 ID는 웹 페이지에서 자산-인사이트 이벤트(노출 횟수 및/또는 클릭 수)를 생성하기 전에 호출해야 합니다.
* **\&lt;code>assetAnalytics.dispatcher.init()\&lt;code>** : 선택적으로 AppMeasurement 개체를 허용합니다. 제공된 경우 AppMeasurement 개체의 새 인스턴스를 만들지 않습니다.
...
두 개의 Google Chrome 브라우저 확장 프로그램은 Analytics를 디버깅하는 방법으로 비디오에서 참조됩니다. 다른 브라우저에서도 유사한 확장을 사용할 수 있습니다.
