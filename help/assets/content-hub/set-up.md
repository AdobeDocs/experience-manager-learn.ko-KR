---
title: AEM Assets Content Hub 설정
description: AEM as a Cloud Service에서 Adobe Experience Manager Assets Content Hub를 설정하는 방법을 알아봅니다.
version: Experience Manager as a Cloud Service
feature: Configuration, Asset Management
topic: Content Supply Chain, Administration
role: Admin
level: Beginner
doc-type: Feature Video
duration: 435
last-substantial-update: 2024-01-10T00:00:00Z
jira: KT-15989
thumbnail: KT-15989.jpeg
exl-id: 83ca35c5-1db4-4297-afc7-67b6e7d2f085
source-git-commit: 998a691bdc001032880d2d8e05f0bd18554c2848
workflow-type: tm+mt
source-wordcount: '108'
ht-degree: 16%

---

# AEM Assets Content Hub 설정

Adobe Cloud Manager 프로그램 및 Content Hub 추가 기능을 사용하여 Experience Manager Assets Assets를 설정하는 방법에 대해 알아봅니다. 제품 프로필을 사용하여 사용자의 액세스를 구성하는 방법도 알아봅니다.

>[!VIDEO](https://video.tv.adobe.com/v/3472942/?captions=kor&learn=on)


## AEM Assets Dynamic Media OpenAPI 설정

AEM Assets Dynamic Media OpenAPI에 라이센스가 부여된 경우 해당 API를 사용하여 Content Hub에서 스마트 자르기와 같은 동적 변환을 생성할 수 있습니다. 이러한 Dynamic Media OpenAPI 렌디션이 Content Hub에 표시되려면 먼저 초기 설정이 필요합니다.

>[!SLIDE](dm-open-api-setup)
