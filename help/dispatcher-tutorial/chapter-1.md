...
어떻게 되었습니까? Dispatcher는 렌더링하는 동안 다른 리소스에서 가져온 모든 콘텐츠 및 마크업이 포함된 페이지의 정적 버전을 저장합니다.
...
Dispatcher의 생성 날짜가 statfile보다 오래된 모든 파일은 마지막 활성화(및 무효화) 전에 렌더링되었으므로 &quot;유효하지 않음&quot;으로 간주됩니다. 이러한 파일은 물리적으로 여전히 파일 시스템에 존재하지만 Dispatcher는 이를 무시합니다. &quot;부실&quot; 상태입니다. 부실 리소스에 대한 요청이 이루어질 때마다 Dispatcher는 AEM 시스템에 페이지를 다시 렌더링하도록 요청합니다. 새로 렌더링된 페이지는 파일 시스템에 저장되며, 이제 새 생성 날짜가 표시되고 다시 새로 고쳐집니다.
...
AEM Author 및 Publish 를 즉시 설치하는 경우 토폴로지가 약간 이상합니다. 작성자는 게시 시스템에 콘텐츠를 전송하는 동시에 Dispatcher에 무효화 요청을 전송합니다. 게시 시스템과 Dispatcher는 모두 큐에 의해 작성자로부터 분리되므로 타이밍이 약간 유감스러울 수 있습니다. Dispatcher는 게시 시스템에서 콘텐츠가 업데이트되기 전에 작성자로부터 무효화 요청을 수신할 수 있습니다.
...
자동 다시 가져오기를 사용하면 이를 어느 정도 완화할 수 있습니다. 대부분의 무효화된 페이지는 자동 무효화 이후에도 여전히 물리적으로 Dispatcher에 저장됩니다. 그것들은 단지 _고려됨_ 오래됨. _자동 재에칭_ 는 시작하는 동안 이러한 부실 페이지를 몇 초 동안 계속 제공함을 의미합니다 _싱글_ 부실 콘텐츠를 다시 가져오도록 게시 시스템에 요청:
