...
이러한 정보 조각과 조각은 항상 매우 전문적인 수준에서 시작됩니다. - 사용자가 원하는 작업을 이미 알고 있으며 따라서 원하는 것을 얻는 방법에 대한 세부 정보만 제공한다고 가정할 경우 우리는 디스패처와 _무엇을 할 수 있고 할 수 없는 것에 관한 어떤 개념적 문서도 발견하지 못했습니다_ .
...
&quot;우선 _성공해서 빨리 만들어&quot; 라는 프로그래밍 조언을 들었을지도 모른다._. 완전히 틀린 것은 아니다. 그러나 올바른 컨텍스트가 없으면 잘못 해석되고 올바로 적용되지 않는 경향이 있습니다.

이러한 권고 사항은 개발자가 너무 빨리 코드를 최적화하지 못하도록 해야 하며, 이러한 코드는 실행되지 않거나 거의 실행되지 않으므로 최적화가 최적화에 투입되는 노력을 정당화할 수 있는 충분한 영향을 주지 않습니다. 또한 최적화를 통해 더 복잡한 코드가 생성되어 버그가 발생할 수 있습니다. 따라서 개발자는 각 단일 코드 줄을 미시적으로 최적화하는 데 많은 시간을 허비하지 않아도 됩니다. 적합한 데이터 구조, 알고리즘 및 라이브러리를 선택했는지 확인하고 프로파일러의 핫스팟 분석을 통해 최적화를 통해 전반적인 성능을 향상시킬 수 있는 상황을 확인할 수 있습니다.
...
파일 이름은 요청된 리소스의 URL에 의해 파생됩니다. 파일을 요청하면 파일이 `/foo/bar.html` 저장됩니다(예: /`var/cache/docroot/foo/bar.html`).
...
그러면 URL에 접미어가 붙은 `path/suffix.ext` 경우,
...
1. URL이 매우 길어질 수 있습니다. 로컬 파일 시스템에 &quot;경로&quot; 부분을 추가하면 일부 파일 시스템 `/docroot` 의 제한을 쉽게 초과할 수 있습니다. Windows에서 NTFS로 Dispatcher를 실행하면 문제가 될 수 있습니다. 그러나 리눅스는 안전하다.
...
리소스 `/home/path/suffix` 의 경로이므로 URL에 접미어가 없습니다.
...
요청을 `home.html` 먼저 하면 파일로 만들어집니다.

이후 요청에 따라 유효한 결과를 `home.html/suffix.html` 반환하지만 파일 시스템 `home.html` 의 파일 &quot;블록&quot;이 있을 `home.html` 경우, 폴더로 두 번째 파일을 만들 수 없으므로 캐시되지 `home.html/suffix.html` 않습니다.
...
반대로 회전을 하는 경우 먼저 요청하는 `home.html/suffix.html` 다음 `suffix.html` 은 폴더 `/home.html` 아래에 캐시됩니다. 그러나 나중에 리소스로 요청할 `home.html` 때 이 폴더는 삭제되고 파일 `home.html` 로 대체됩니다.
...
마지막 장에서는 Dispatcher가 요청을 캐시할 수 없을 때 많은 수의 예외가 표시되었습니다. 그러나 고려해야 할 것이 더 있습니다.Dispatcher가 요청을 캐싱할 _수_ 있다고 해서 _그럴 필요는 없습니다_.

요점은 다음과 같습니다.캐싱은 대개 쉽습니다. Dispatcher는 응답의 결과를 저장한 다음 동일한 요청이 수신될 때 반환해야 합니다. 오른쪽? 틀렸습니다!

어려운 부분은 캐시의 _무효화_ 또는 _플러싱입니다_ . Dispatcher는 리소스가 변경된 시기를 파악하고 다시 렌더링해야 합니다.
...
제거 `image.png` 및 `image.thumb.png` 해당 패턴과 일치하는 기타 모든 변환.
...
*홈 페이지로* 이동합니다. 홈 페이지는 Dispatcher가 Teaser를 비롯한 정적 .html 파일로 저장합니다.
...
아직도 확신하지 못합니까? 여러분은 *&quot;어떤 종류의 종속성 추적을 구현할 방법이 있을 것이라고 생각할 수도 있습니다*. 글쎄요, 또는 더 정확하게 거기에 *있었습니다*. AEM의 대 증조부인 코뮈니케는 페이지를 렌더링하는 데 사용된 _세션에서_ 종속성 추적기를 구현했습니다.
...
왜 이것이 &quot;.stat&quot;라고 부르는지 질문할 수 있습니다. 그리고 아마도 &quot;무효화&quot;는 아니겠죠? 파일 시스템에 이 파일을 저장한다면 Dispatcher가 정적인 웹 서버처럼 *정적으로* 제공할 수 있는 리소스를 결정하는 데 도움이 됩니다. 이러한 파일을 더 이상 동적으로 렌더링할 필요가 없습니다.

그러나 &#39;순수한 이름&#39;은 은유적이지 않다. 파일 수정 시간(다른 속성 포함)을 반환하는 Unix 시스템 호출 `stat()`에서 파생됩니다.
...
2. 에이전트가 HTTP 헤더 `CQ-Action-Scope: ResourceOnly`를 보내는 경우, 즉 이 단일 무효화 요청이 자동 무효화를 트리거하지 않습니다. 이 ( [https://github.com/cqsupport/webinar-dispatchercache/tree/master/src/refetching-flush-agent/refetch-bundle](https://github.com/cqsupport/webinar-dispatchercache/tree/master/src/refetching-flush-agent/refetch-bundle)) 코드 조각은 자신의 복제 에이전트에 적합한 시작점이 될 수 있습니다.

3. `ResourceOnly`은 자동 무효화만 방지합니다. 실제로 필요한 종속성 해결 및 무효화를 수행하려면 무효화 요청을 직접 트리거해야 합니다. 실제로 이러한 문제가 발생할 수 있는 방법에 대한 영감을 얻을 수 있도록 패키지 Dispatcher[](https://adobe-consulting-services.github.io/acs-aem-commons/features/dispatcher-flush-rules/index.html)플러시 규칙(https://adobe-consulting-services.github.io/acs-aem-commons/features/dispatcher-flush-rules/index.html)을 확인할 수 있습니다.
...
자산은 AEM의 DAM 영역에 업로드되며 응답형 이미지 구성 요소에서만 _참조됩니다_ .
...
구성 요소에는 두 개의 부분이 있습니다.첫 번째 부분은 이미지의 HTML 마크업을 렌더링하고 두 번째 부분은 참조된 이미지의 이진 데이터를 &quot;스풀&quot;합니다. 반응형 디자인이 적용된 최신 웹 사이트이므로 단순한 `<img src"…">` 태그를 렌더링하지 않고 태그로 이미지 세트를 렌더링하고 `<picture/>` 있습니다. 각 장치에 대해 두 개의 서로 다른 이미지를 DAM에 업로드하고 이미지 구성 요소에서 참조합니다.
...
쿼리 매개 변수를 본 적이 있습니까 `?ck=398547283745`? 이를 &#39;캐시 킬러(&quot;ck&quot;)라고 합니다. 쿼리 매개 변수를 추가하면 리소스는 검색되지 않습니다. 또한 임의 숫자를 매개 변수의 값(예: &quot;398547283745&quot;)으로 추가하면 URL이 고유해지고 AEM 시스템과 화면 사이의 다른 캐시도 되지 않도록 합니다. 일반적인 중간 용의자는 Dispatcher, CDN 또는 브라우저 캐시 앞에 있는 &quot;Varnish&quot; 캐시입니다. 다시 한 번:그러지 마 리소스를 가능한 한 많이 캐시해야 합니다. 캐시는 당신의 친구입니다 친구를 죽이지 마
...
쿼리 매개 변수가 있는 URL은 캐시할 수 없으므로 지문을 쿼리 매개 변수로 사용할 수 없습니다. 지문에 선택기 또는 접미어를 사용할 수 있습니다.

2018년 12월 31일, 23:59일 파일 `/content/dam/flower.jpg` 의 날짜가 `jcr:lastModified` 있다고 가정해 봅시다. 지문이 있는 URL입니다 `/content/home/jcr:content/par/respi.fp-2018-31-12-23-59.jpg`.
...
이 예에서는 구성 요소가 23:59에 렌더링되고 캐시되었습니다. 이제 00:00에 이미지가 변경되었습니다.  구성 요소 _는_ 마크업에 새로운 지문 URL을 생성합니다.

당신은 _그래야_&#x200B;한다고 생각할지 모르지만 그렇지 않아요이미지의 바이너리만 변경되고 포함 페이지가 터치되지 않았으므로 HTML 마크업을 다시 렌더링할 필요가 없습니다. 따라서 Dispatcher는 이전 지문과 이전 버전의 이미지를 제공하는 페이지를 제공합니다.
...
게으른 관리자는 일반적으로 &quot;jpg에 대한 자동 무효화를&#x200B;_설정하고 statfile 수준을 0으로 설정 - 모든 종류의 캐싱 문제에 항상 도움이 됩니다_.&quot; 기술 포럼에서 그 조언을 찾을 수 있으며 무효화 문제에 도움이 됩니다.

지금까지 상태 파일 수준에 대해 논의하지 않았습니다. 기본적으로 자동 무효화는 동일한 하위 트리에 있는 파일에만 작동합니다. 그러나 일반적으로 페이지와 자산이 같은 하위 트리에서 존재하지 않는다는 문제가 있습니다. 페이지는 아래에 있는 반면 `/content/mysite` 자산은 아래에 있습니다 `/content/dam`.
...
이렇게 하면 사용자 지정 Dispatcher Flashing 에이전트가 변경 사항이 있을 때 /content/site-a에 요청을 쉽게 보내고 무효화할 수 있습니다 `/content/dam/site-a`.
...
는 무효화된 리소스 중 하나와 관련이 없습니다. 기억? &quot;더미&quot; 리소스만 무효화했으며 &quot;홈&quot;이 유효하지 않다고 간주하기 위해 자동 무효화에 의존했습니다. 이미지 자체는 _물리적으로_ 삭제되지 않을 수 있습니다. 따라서 캐시는 확장되고 증가할 것입니다. 이미지가 변경되고 활성화되면 Dispatcher의 파일 시스템에 새 파일 이름이 생성됩니다.
...
[Versioned Clientlibs](https://adobe-consulting-services.github.io/acs-aem-commons/features/versioned-clientlibs/index.html) 는 이 방법을 사용하는 모듈입니다.

그러나 여기에서 여러분은 URL 지문을 가지고 있는 또 다른 원시적인 문제에 직면할 수 있습니다.URL을 컨텐츠에 연결합니다. 또한 URL을 변경하지 않으면 컨텐츠를 변경할 수 없습니다(또는 수정 날짜 업데이트). 이것이 그 지문들이 처음부터 고안된 것이다. 그러나 새로운 CSS 및 JS 파일과 새로운 URL이 포함된 새로운 릴리스를 출시한다는 점을 고려해 보십시오. 모든 HTML 페이지에는 여전히 이전 지문 URL에 대한 참조가 있습니다. 따라서 새로운 릴리스가 일관되게 작동하려면 모든 HTML 페이지를 한 번에 무효화해야 새로 지문을 인쇄한 파일에 대한 참조가 포함된 다시 렌더링할 수 있습니다. 동일한 라이브러리에 의존하는 여러 사이트가 있는 경우 상당한 양의 다시 렌더링이 가능하며 해당 사이트를 활용할 수 없습니다 `statfiles`. 따라서 롤아웃 후 게시 시스템에서 로드 픽스를 볼 수 있습니다. 캐시 온난화와 함께 녹색 환경을 구축하거나 Dispatcher 앞에서 TTL 기반 캐시를 사용하는 것으로 간주할 수 있습니다... 가능성은 무궁무진합니다.
...
특정 구성 요소에 대해 일부 특수 처리를 해야 하는 경우 해당 경로 및 선택기에 전용 서블릿을 등록하여 구성 요소를 대신하여 변환을 수행합니다. 우리는 여기서 &quot;.respi&quot;를 가지고 그것을 모범적으로 했습니다. selector. 전역 URL 공간(예:)에서 사용되는 선택기 이름을 추적하고 이름 충돌을 방지하기 위해 적절한 이름 지정 규칙을 `/content/dam`사용하는 것이 좋습니다.
...
또한 - 이미지가 사용되는 위치에 따라 다른 매개 변수를 사용할 수도 있습니다. Teaser의 작은 축소판은 화면 전체 영웅 배너에 사용된 것과 동일한 이미지보다 더 좋은 압축을 견딜 수 있습니다. 즉, 품질 매개 변수는 이미지가 아니라 이미지 및 컨텍스트에 따라 달라집니다. 저자의 취향에 따라
...
없는 것은 품질 값입니다. 구성 요소는 작성자가 입력한 속성을 알고 있습니다.. 다음과 같이 마크업을 렌더링할 때 이미지 렌더링 서블릿에 쿼리 매개 변수로 쉽게 전달할 수 있습니다. `flower.respi2.jpg?quality=60`
...
다시 캐시를 무시하고 게시 시스템에서 로드를 만듭니다. 그래서, 그것은 나쁜 생각일지도 모릅니다. 일부 매개 변수만 필터링하여 이를 줄일 수 있습니다. 귀하는 허용만을 원합니다 `q-20, q-40, q-60, q-80, q-100`.
...
여기 AEM이 브라우저에게 말하고 있습니다. &quot;난 없어 `q-41`. 하지만, `q-40` &quot;

이렇게 하면 대화에 요청 응답 루프가 추가되는데, 이는 약간의 오버헤드이지만, 전체 처리를 하는 것보다 더 싸습니다 `q-41`. 또한 에서 이미 캐싱된 파일을 활용할 수 있습니다 `q-40`. 하지만 302개의 응답이 Dispatcher에 캐시되지 않는다는 점을 이해해야 합니다. AEM에서 실행되는 로직에 대해 이야기하고 있습니다. 계속 반복해 따라서 슬림하고 빠르게 만드는 것이 좋습니다.
...
이 패턴을 _인버터_.. 반전된 스풀러는 모든 멋진 캐시 무효화 속성을 가지려면 이미지 리소스를 기반으로 해야 합니다.
...
우리는 전에 _성명_ 내용을 간단히 언급했었다. 자동 무효화와 관련이 있습니다.

자동 무효화되도록 구성된 Dispatcher 파일 시스템의 모든 캐시 파일은 마지막 수정 날짜가 `statfile's` 마지막으로 수정한 날짜보다 오래된 경우 유효하지 않은 것으로 간주됩니다.

>[!NOTE]
>
>마지막으로 수정한 날짜는 캐시된 파일이 클라이언트의 브라우저에서 파일을 요청하고 파일 시스템에서 생성된 날짜입니다. 리소스의 `jcr:lastModified` 날짜가 아닙니다.

상태 파일의 마지막 수정 날짜(`.stat`)는 디스패처에서 AEM의 무효화 요청을 받은 날짜입니다.
...
자동 무효화를 도입했을 때, 그리고 우리가 말한 stfile은 변경이 있을 때 *모든* 파일이 유효하지 않은 것으로 간주되고 어쨌든 모든 파일이 상호 의존적이라고 합니다.
...
Dispatcher는 사이트를 서로 구분하는 간단한 방법을 제공합니다.The `statfiles-level`.

파일 시스템의 수준을 정의하는 숫자입니다. 두 개의 하위 트리가 &quot;독립&quot;으로 간주됩니다.

상태 파일 수준이 0인 기본 사례를 살펴보겠습니다.

![/statefileslevel &quot;0&quot;:_ _.stat_ _is가 doroot에 만들어집니다. 무효화 도메인은 모든 사이트를 포함하여 전체 설치 전체에 걸쳐 있습니다.](assets/chapter-1/statfile-level-0.png)

`/statfileslevel "0":` 이 `.stat` 파일은 doroot에서 만들어집니다. 무효화 도메인은 모든 사이트를 포함한 전체 설치 전체에 적용됩니다.

어떤 파일이 무효화되었든, 디스패처 문서 루트 맨 위에 있는 `.stat` 파일은 항상 업데이트됩니다. 따라서 무효화할 `/content/site-b/home`경우 이제 문서 `/content/site-a` `.stat` 의 파일보다 더 오래된 파일이 모두 무효화됩니다. 사용자가 무효화할 때 필요한 것이 아닙니다 `site-b`.

이 예에서 는 로 `statfileslevel` 설정하는 것이 좋습니다 `1`.

이제 게시하여 아래 리소스 `/content/site-b/home` 나 다른 리소스를 무효화하면 파일 `/content/site-b`이 `.stat` 에 생성됩니다 `/content/site-b/`.

아래의 콘텐츠 `/content/site-a/` 는 영향을 받지 않습니다. 이 콘텐트는 의 `.stat` 파일과 비교됩니다 `/content/site-a/`. 두 개의 별도 무효화 도메인을 만들었습니다.
...
대규모 설치는 일반적으로 조금 더 복잡하고 더 깊이 조직되어 있습니다. 공통적 계획은 사이트를 브랜드, 국가 및 언어별로 구성하는 것이다. 이 경우 상태 파일 수준을 더 높게 설정할 수 있습니다. _1은_ 브랜드당 무효화 도메인, 국가당 _2_ 개 및 언어당 _3개_ 를 만듭니다.
...
전자는 `statfileslevel` 2 _점, 후자는_ 3점 __&#x200B;을 필요로 한다.

이상적 상황이 아닙니다 이 값을 _3_&#x200B;으로 설정하면 자동 무효화는 하위 분기 `/home`와 하위 분기 사이의 작은 사이트 내에서 작동하지 `/products` 않습니다 `/about`.

이 값을 _2_ 로 설정하면 더 큰 사이트에서 선언하거나 종속 `/canada/en` 을 지정할 수 `/canada/fr` 있음을 의미합니다. 따라서 각각의 무효화는 `/en` 또한 무효화될 것이다 `/fr`. 이로 인해 캐시 히트 비율이 약간 감소하지만 오래된 캐시된 컨텐츠를 제공하는 것보다 여전히 낫습니다.
...
지금 어느 정도가 적당합니까? 사이트 간 종속성 수에 따라 다릅니다. 페이지 렌더링을 위해 해결하는 포함은 &quot;하드 종속성&quot;으로 간주됩니다. Adobe는 _Teaser_ 구성 요소를 본 가이드의 시작 부분에 소개했을 때 이러한 _포함을_ 설명했습니다.

_하이퍼링크는_ 보다 부드러운 종속성 형식입니다. 하나의 웹 사이트 내에서 하이퍼링크가 만들어집니다... 웹 사이트 간에 링크가 있을 가능성은 별로 없습니다. [단순 하이퍼링크]는 일반적으로 웹 사이트 간 종속 관계를 만들지 않습니다. 사이트에서 facebook으로 설정한 외부 링크에 대해 생각해 보십시오.facebook에서 변경되는 사항이 있으면 페이지를 렌더링하지 않아도 됩니다.
...
각 페이지의 헤더 섹션에 있는 각 관련 페이지에 [태그를 설정하여, 시스템이 두 개 이상의 페이지에 있고, 동일한 컨텐츠가 있는 페이지가 실제로 두 개 이상 있다는 것을, 그리고 시스템을 &quot;게임&quot;하려고 시도하지 않는(](https://support.google.com/webmasters/answer/189077?hl=en)&quot;Google에 페이지의 현지화된 버전에 대해 알림&quot; 참조) `<link rel="alternate">` 등 낮은 등급으로 분류하지 않도록 할 수 있습니다.
...
이 구성표는 많은 링크뿐만 아니라 일부 문제를 만들었습니다. n개 언어로 __ p _하는_ 데 필요한 링크 수는 _p x(n<sup></sup>_ 2-n)입니다.각 페이지는 자체(_-n_)를 제외하고 서로 다른 페이지(_n x n_)로 링크됩니다. 이 구성표는 각 페이지에 적용됩니다. 4개 언어로 된 소규모 사이트가 20페이지이면 각각 _240개의_ 링크가 있습니다.
...
저희 프로젝트에서는 새로운 관련 페이지가 자주 등장했습니다. 하지만 그들은 &quot;대체&quot; 링크로 구현되지 않았다. 예를 들어 독일 웹 사이트에 `de-de/produkte` 페이지가 게시되었을 때 다른 사이트에서는 즉시 표시되지 않았습니다.
...
`shiny-brand` 이 `marque-brillant` 는 프랑스와 독일에서 `blitzmarke` 호출되었습니다.
...
그것은 그 `statfiles` 수준을 1로 설정하려는 의미일 것인데, 그것은 너무 큰 무효화 도메인을 초래할 것입니다.
...
콘텐츠를 받은 _후_ 게시 시스템에서 무효화 요청이 더 많은 설정으로 전송됩니다. &quot;[게시 인스턴스에서 발송자 캐시를 무효화&quot;](https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html#InvalidatingDispatcherCachefromaPublishingInstance)문서에서 세부 정보에 대해 설명합니다.
...
일반적으로 리소스 경로 및 접미어로 파생될 수 있는 정보를 사용하여 Apache 서버의 리소스 `mod_headers` 에 누락된 헤더를 다시 적용하기 시작했습니다. 그러나 그것이 항상 충분하지 않았다.

특히 디스패처가 브라우저에서의 첫 번째 _캐시되지_ 않는 응답은 전체 범위의 헤더가 있는 게시 시스템에서 나온 반면, 후속 응답은 제한된 헤더 집합을 가진 디스패처가 생성하는 경우가 있었습니다.
...
캐시-컨트롤 및 프라그마는 CDN과 같은 상위 캐싱 레이어로 전파되고 해석되는 공식 HTTP 헤더입니다. 헤더는 `Dispatcher` 디스패처가 캐시하지 않도록 하는 힌트일 뿐입니다. Dispatcher에게 캐시하지 않도록 하면서 여전히 상위 캐싱 레이어가 캐시하도록 하는 데 사용할 수 있습니다. 사실, 그것이 유용할 수 있는 사례를 찾기는 어렵습니다. 하지만 우리는 어딘가에 몇 개가 있다는 것을 확신한다.
...
일반적으로 Apache를 사용하거나 좀 더 개별적인 컨트롤을 필요로 하는 경우 AEM에서 `mod_expires` 오는 Cache-Control 및 Expires 헤더를 저장하여 정적으로 수행합니다.

브라우저에 캐시된 문서는 세 가지 수준의 최신 문서를 가질 수 있습니다.

1. _새 문서_ 보장 - 브라우저가 캐시된 문서를 사용할 수 있습니다.

2. _잠재적으로 부실_ - 브라우저가 캐시된 문서가 아직 최신 상태인지 서버에 먼저 물어봐야 합니다.
...
브라우저 캐시의 문서가 새 요청에 의해 전달되는 것과 동일한지 확인하기 위해 브라우저에서는 문서의 `Last-Modified` 날짜를 사용할 수 있습니다. 브라우저가 서버에 다음 사항을 요청합니다.

&quot;6월 10일부터 버전이&#x200B;_있습니다. 업데이트가 필요합니까?_&quot; 서버가

리소스를 다시 전송하지 않고 &quot;_304 - 사용 중인 버전이 여전히 최신_&#x200B;버전이거나, 서버에서

HTTP 헤더에 있는 &quot;_200 - 여기에 최신 버전_&quot;과 HTTP 본문에 있는 실제 최신 컨텐츠

두 번째 부분이 작동하려면 업데이트를 요청할 참조 지점이 있도록 `Last-Modified` 날짜를 브라우저로 전송해야 합니다.

앞서 설명했듯이, Dispatcher에서 생성된 `Last-Modified` 날짜는 브라우저가 파일을 요청할 때 캐시된 파일과 그 날짜가 생성되기 때문에 서로 다른 요청에 따라 달라질 수 있습니다. 대체 방법은 &quot;e-tags&quot;를 사용하는 것입니다. 이 숫자는 날짜 대신 실제 컨텐트(예: 해시 코드 생성)를 식별하는 숫자입니다.

ACS[커머스](https://adobe-consulting-services.github.io/acs-aem-commons/features/etag/index.html)패키지의 &quot;태그 지원 _&quot;은 이 방법을_ 사용합니다. 그러나 다음과 같은 비용이 발생합니다.E-Tag는 헤더로 전송되어야 하지만, 해시 코드의 처리를 위해서는 응답을 완전히 읽어야 하므로, 응답이 배달될 수 있으려면 먼저 기본 메모리에서 완전 버퍼링되어야 합니다. 이렇게 하면 웹 사이트에 캐시되지 않은 리소스가 있을 가능성이 크고 AEM 시스템에서 사용하는 메모리를 잘 지켜봐야 하는 경우 지연에 부정적인 영향을 줄 수 있습니다.

URL 지문을 사용하는 경우 매우 긴 만료 날짜를 설정할 수 있습니다. 브라우저에 지문을 남긴 리소스를 영구적으로 캐시할 수 있습니다. 새 버전이 새 URL로 표시되고 이전 버전은 업데이트할 필요가 없습니다.

스풀러 패턴을 소개할 때 URL 지문을 사용했어요 CSS, JS `/etc/design` (Static Files From the) is simply changed, and make them good 후보자 ID as stingle.
...
AEM UI로 구성된 [화강암 라이브러리]는 상당한 시간 동안 캐시될 수 있습니다. 또한 브라우저에서 사이트 정적 파일(글꼴, CSS 및 JavaScript)을 캐시할 수 있습니다. 페이지의 복사 텍스트와 같이 자주 변경되지 않으므로 15분 정도 캐시될 `/content/dam` 수 있습니다. 이미지는 AEM에서 대화형으로 편집되지 않습니다. AEM에 업로드되기 전에 먼저 편집하고 승인됩니다. 따라서 텍스트 만큼 자주 변경되지 않는다고 가정할 수 있습니다.
...
그러나 AEM에만 의존하지 마십시오. 이 경우 캐시의 루트 디렉토리 `/home.html` 와 같은 경로가 있을 수 있습니다. 지금, 그것은 마니스트 또는 독일어의 &quot;홈&quot;입니까 아니면 캐나다 웹 사이트입니까? 그리고 Dispatcher `/home.html` 에 파일이 있는 경우, Dispatcher는 Dispatcher에 대한 무효화 요청이 들어올 때 이 파일이 무효화되어야 한다는 것을 어떻게 알고 `/content/brand/fi/fi/home` 있습니까?

각 도메인에 대해 별도의 분리가 있는 프로젝트를 보았습니다. 디버깅하고 유지 관리하는 것은 악몽이었고 완벽하게 작동하는 것을 전혀 보지 못했습니다.

캐시를 다시 만들어 문제를 해결할 수 있어요 모든 도메인에 대한 단일 문서 루트가 있고 서버의 모든 파일이 다음으로 시작되므로 무효화 요청이 1:1로 처리될 수 있습니다 `/content`.

자르는 부분도 매우 쉽다.  AEM에서 구성에 따라 잘린 링크를 생성했습니다 `/etc/map`.
...
네, 하지만 이것은 AEM입니다. 기억 - 디스패처는 `404 – not found` 또는 응답을 캐시하지 `500 – internal server error` 않습니다.
...
물론 이것은 고객이 본 것이 아니다. Dispatcher에서 다음과 같이 `ErrorDocuments` 등록했습니다.
...
첫째, `error-404.html` 항상 같은 페이지입니다. 그래서, &quot;프로덕텐&quot;에 대한 당신의 검색&#x200B;_이_&#x200B;결과를 낳지 않았다는 것과 같은 개인적인 메시지는 없습니다. 우리는 그것을 가지고 쉽게 살 수 있었다.

둘째,... 만약 내부 서버 오류가 발견된다면 - 아니면 AEM 시스템의 작동 중단이 발생한다면, AEM에게 오류 페이지를 렌더링하도록 요청할 방법이 없습니다, 그렇죠? 지시문에 정의된 대로 필요한 후속 요청도 `ErrorDocument` 실패합니다. Adobe는 정의된 위치에서 오류 페이지를 주기적으로 가져와서 지시문에 정의된 정적 파일 위치에 저장하는 cron-job `wget` 을 실행하여 이 문제를 `ErrorDocuments` 해결했습니다.
...
>[!NOTE]
>
>이 패턴을 사용하려면 _게이트웨이가_ 필요합니다. 이 _는_ 각 요청을 _가로채서_ 실제권한(발송자에 대한 요청 허용 또는 거부)을수행합니다. SSO 시스템이 _인증자인_&#x200B;경우, 3옵션을 구현해야 하는 사용자의 ID만 설정합니다. SSO 시스템 핸드북에 &quot;SAML&quot; 또는 &quot;OAauth&quot;와 같은 용어를 읽은 경우, 이 용어는 옵션 3을 구현해야 하는 강력한 지표입니다.
...
**옵션 3**. &quot;사용 권한 감지 캐싱&quot;은 흥미로운 방법입니다. Dispatcher가 리소스를 캐싱하고 있지만 전달하기 전에 AEM 시스템에서 그럴 수 있는지 묻습니다. 이렇게 하면 Dispatcher에서 Publish로 추가 요청이 생성되지만, 이미 캐시되어 있는 경우 일반적으로 Publish 시스템이 페이지를 다시 렌더링하는 데 사용되지 않습니다. 하지만 이 방법을 사용하려면 몇 가지 사용자 지정 구현이 필요합니다. 아티클 권한 [감지 캐싱에서 자세한 내용을 살펴보십시오](https://helpx.adobe.com/experience-manager/dispatcher/using/permissions-cache.html).
...
이러한 &quot;캐시 무효화 폭풍&quot;의 문제를 완화하려면 해석에 대해 덜 까다로울 수 `statfile` 있습니다.

Dispatcher가 자동 무효화에 대해 `grace period` 를 사용하도록 설정할 수 있습니다. 이렇게 하면 내부적으로 수정 날짜에 약간의 시간이 더 `statfiles` 추가됩니다.

오늘 12시 `statfile` 에 수정 시간이 있고 2분 `gracePeriod` 으로 정하죠 그러면 12:01과 12:02에 모든 자동 무효화된 파일이 유효한 것으로 간주됩니다. 12:02 이후에 다시 렌더링됩니다.

참고자료구성은 좋은 이유 `gracePeriod` 로 2분을 제시한다. &#39;2분? 거의 아무것도 아니에요 콘텐츠가 표시되기까지는 10분이 걸릴 것 같습니다.&quot;  따라서 10분 후에 컨텐츠가 나타난다고 가정할 때 더 긴 기간을 설정하려고 할 수 있습니다.

>[!WARNING]
>
>이것이 `gracePeriod` 작동하는 방식이 아닙니다. 유예 기간은 문서가 무효화되는 시점이 _아니라_ 기간 동안 무효화가 발생하지 않습니다. 이 프레임 내에 속하는 각 후속 무효화는 _시간_ 기간을 늘립니다. 이 값은 무한하게 길어질 수 있습니다.

예를 통해 실제 작동 방식 `gracePeriod` 을 설명하겠습니다.
...
또한 컨텐츠는 무효화되지 않습니다. 유예 기간 *내* 각 무효화는 유예 시간을 효과적으로 연장합니다. 실효 `gracePeriod` 에 대비해 설계됐지만 결국 비를 맞으러 가야 돼 그래서 보호소에 영원히 숨지 못하게 `gracePeriod` 상당히 짧게 해
...
일반 복제 간격이 길이보다 짧으면 예기치 않게 길어질 `gracePeriod` 수 있습니다 `gracePeriod`.
...
무효화가 발생한 후 실행할 &quot;사용자 지정 무효화 스크립트&quot;(참조)를 사용합니다. 이 스크립트는 `statfile's` 마지막 수정 날짜를 읽고 다음 간격 정지까지 반올림했습니다. Unix 셸 명령 `touch --time`에서 시간을 지정하겠습니다.
...
자동 다시 반입을 사용하면 어느 정도까지는 완화할 수 있습니다. 대부분의 무효화된 페이지는 자동 무효화 후에도 디스패처에 물리적으로 저장됩니다. 그것들은 단지 _오래된 것으로_ 여겨진다. _자동_ 리퍼칭(Automatic Referencing)은 부실 컨텐츠를 다시 가져오기 위해 게시 시스템에 _단일_ 요청을 시작하는 동안 몇 초 동안 이러한 오래된 페이지를 계속 제공함을 의미합니다.
...
디스패처는 유지 관리 목적으로만 작성된 요청으로부터 게시 시스템을 보호함으로써 약간의 추가 보안을 제공합니다. 예를 들어, 사용자 `/crx/de` 또는 `/system/console` URL을 노출하지 않으려는 경우
...
Dispatcher를 필터로 사용하는 또 다른 방법은 `dispatcher.any`
...
버전 B는 작은 따옴표 `'` 를 사용하여 _정규 표현식 패턴을 표시합니다_. &quot;모든 문자&quot;는 를 사용하여 표현됩니다 `.*`.

_글로빙 패턴_, 반면 큰따옴표는 `"` 사용하며 간단한 자리표시자만 사용할 수 `*`있습니다.

이러한 차이점을 알고 있다면 사소한 일이지만 그렇지 않을 경우 견적을 손쉽게 취합하여 구성을 디버깅하는 화창한 오후를 보낼 수 있습니다. 이제 경고를 받겠습니다.

&quot;구성 `'/url'` 에서 인지하고 있습니다...하지만 필터 `'/glob'` 에 있는 것은 무엇인가요?

이 지시문은 메서드 및 경로를 포함하여 전체 요청 문자열을 나타냅니다. 그건

`"GET /content/foo/bar.html HTTP/1.1"`

패턴과 비교할 문자열입니다. 초보자는 초보 부분, 즉 `method` (GET, POST, ...)을 잊어버리는 경향이 있다. 그러니까,
...
각각의 정의 앞 `/005` 에 있는 &quot;라인 번호&quot;에 대한 마지막 말
...
* `Deactive:` 삭제 `/path-pattern.*`및 삭제 `/path-pattern/*`
* `Delete:`   삭제 `/path-pattern.*`및 삭제 
`/path-pattern/*`
* `Test:`   &quot;확인&quot;을 반환하지만 아무 작업도 수행하지 않습니다.

`CQ-Handle: <path-pattern>` - 무효화할 컨텐츠 리소스 경로 참고, `<path-pattern>` 은 &quot;패턴&quot;이 아니라 &quot;경로&quot;입니다.

`CQ-Action-Scope: ResourceOnly` - 선택 사항:이 헤더를 설정하면 파일이 `.stat` 터치되지 않습니다.
...
모든 지시문이 있는 발송자 설명서 설명: [https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html](https://helpx.adobe.com/kr/experience-manager/dispatcher/using/dispatcher-configuration.html을 참조하십시오.)
...
프레젠테이션 &quot;컨텐츠 무효화의 저평가된 힘&quot;, &quot;adaptTo()&quot; 컨퍼런스는 2018년 [https://adapt.to/2018/en/schedule/the-underappreciated-power-of-content-invalidation.html](https://adapt.to/2018/en/schedule/the-underappreciated-power-of-content-invalidation.html)
