---
title: "1장 - Dispatcher 개념, 패턴 및 안티패턴"
description: 이 장에서는 Dispatcher의 내역 및 역학을 간략하게 소개하고 AEM 개발자가 구성 요소를 디자인하는 방법에 영향을 미치는 방법에 대해 설명합니다.
feature: Dispatcher
topic: Architecture
role: Architect
level: Beginner
...
이 장에서는 Dispatcher의 내역 및 역학을 간략하게 소개하고 AEM 개발자가 구성 요소를 디자인하는 방법에 영향을 미치는 방법에 대해 설명합니다.

## 개발자가 인프라에 관심을 가져야 하는 이유

Dispatcher는 모든 AEM 설치는 아니지만 대부분의 필수적인 부분입니다. Dispatcher 구성 방법과 팁 및 요령에 대해 설명하는 많은 온라인 문서를 찾을 수 있습니다.

그러나 이러한 정보 조각은 항상 매우 기술적인 수준에서 시작합니다. 즉, 원하는 작업을 이미 알고 있다고 가정하고 원하는 작업을 수행하는 방법에 대한 세부 정보만 제공합니다. 우리는 다음을 설명하는 개념적인 논문을 발견하지 못했다 _(이)가 무엇이며 왜 이(가)_ dispatcher로 수행할 수 있는 작업과 수행할 수 없는 작업에 대해 설명합니다.

### 패턴 방지: 사후 사고로서의 Dispatcher

이러한 기본 정보의 부족은 많은 AEM 프로젝트에서 보았던 많은 안티 패턴으로 이어집니다.

1. Dispatcher가 Apache 웹 서버에 설치되므로 이를 구성하는 것은 프로젝트의 &quot;Unix gods&quot;의 작업입니다. 필사의 자바 개발자는 그것에 대해 걱정할 필요가 없다.
...
프로그래밍 조언을 들으셨을 수도 있습니다 _&quot;우선 작동하게 만든 다음 빠르게 만듭니다.&quot;_&#x200B;을 따르지 않는 경우입니다. 완전히 틀린 것은 아니다. 그러나 올바른 문맥이 없으면 잘못 해석되어 올바르게 적용되지 않는 경향이 있습니다.
...
기억? 이 페이지 맨 위에는 Dispatcher가 AEM 애플리케이션의 필수 부분이라고 설명되어 있습니다. 웹 애플리케이션에 대한 액세스는 매우 임의적입니다. 사용자가 예측할 수 없는 시간에 오고갑니다. 결국 - 모든 콘텐츠가 Dispatcher에서 캐시됩니다(또는 캐시되어야 합니다). 따라서 주의 깊게 살펴보았다면 캐싱이 &quot;아키텍처&quot; 아티팩트로 보일 수 있으므로 팀, 개발자 및 관리자 모두 이해해야 한다는 것을 깨달았을 것입니다.

개발자가 실제로 Dispatcher를 구성해야 한다는 것은 아닙니다. 이러한 사용자는 개념(특히 경계)을 알고 있어야 Dispatcher에서도 코드를 활용할 수 있습니다.

Dispatcher는 코드 속도를 동적으로 향상시키지 않습니다. 개발자는 Dispatcher를 염두에 두고 구성 요소를 만들어야 합니다. 따라서, 그는 그것이 어떻게 작동하는지 알 필요가 있습니다.

## Dispatcher 캐싱 - 기본 원칙

### 캐싱 Http로서의 Dispatcher - 로드 밸런서

Dispatcher란 무엇이며 처음에 &quot;Dispatcher&quot;라고 하는 이유는 무엇입니까?
...
웹 초기에는 사이트에 수백 명의 방문자가 있을 것으로 예상됩니다. 한 Dispatcher의 설정으로 여러 AEM 게시 서버에 대한 요청의 로드를 &quot;발송&quot;하거나 밸런싱했으며 일반적으로 충분했습니다. 따라서 이름이 &quot;Dispatcher&quot;입니다. 그러나 최근에는 이 설정이 더 이상 자주 사용되지 않습니다.
...
1. 사용자가 페이지를 요청합니다.
2. Dispatcher는 이미 해당 페이지의 렌더링된 버전이 있는지 확인합니다. 이 페이지에 대한 첫 번째 요청이고 Dispatcher가 로컬 캐시 복사본을 찾을 수 없다고 가정해 보겠습니다.
3. Dispatcher가 게시 시스템에서 페이지를 요청합니다
4. 게시 시스템에서 페이지는 JSP 또는 HTL 템플릿에 의해 렌더링됩니다
5. 페이지가 디스패처에 반환됩니다
6. Dispatcher가 페이지를 캐시합니다
7. Dispatcher가 페이지를 브라우저에 반환합니다
8. 동일한 페이지를 두 번 요청한 경우 게시 인스턴스에서 다시 렌더링하지 않아도 Dispatcher 캐시에서 직접 제공할 수 있습니다. 따라서 게시 인스턴스에서 사용자 및 CPU 주기에 대한 대기 시간이 절약됩니다.
...
* `path/to/resource` 는 리소스가 CRX에 저장되고 이후에 Apache 서버의 파일 시스템에 저장되는 경로입니다
...
* `selectors` 는 리소스가 렌더링되는 방법을 결정하기 위해 Sling에 사용되는 여러 선택기를 나타냅니다. URL에는 임의 개수의 선택기가 포함될 수 있습니다. 그것들은 일정 기간별로 분리되어 있다. 선택기 섹션은 예를 들어 &quot;french.mobile.fancy&quot;와 같은 것일 수 있습니다. 선택기에는 문자, 숫자 및 대시만 포함되어야 합니다.

* `html` 는 &quot;선택기&quot;의 마지막 부분이므로 확장이라고 합니다. AEM/Sling에서는 렌더링 스크립트도 부분적으로 결정합니다.

* `path/suffix.ext` 는 URL의 접미사가 될 수 있는 경로와 유사한 표현식입니다.  AEM 스크립트에서 리소스를 렌더링하는 방법을 추가로 제어할 수 있습니다. 이 부분에 대해서는 나중에 전체 섹션이 작성될 예정입니다. 지금은 추가 매개 변수로 사용할 수 있음을 알고 있으면 됩니다. 접미사에는 확장명이 있어야 합니다.

* `?parameter=value&otherparameter=value` 는 URL의 쿼리 섹션입니다. 임의의 매개 변수를 AEM에 전달하는 데 사용됩니다. 매개 변수가 있는 URL은 캐시할 수 없으므로 매개 변수는 반드시 필요한 경우로 제한해야 합니다.

* `#fragment`: URL의 조각 부분이 AEM으로 전달되지 않습니다. 이 부분은 브라우저에서만 사용됩니다. JavaScript 프레임워크에서는 &quot;라우팅 매개 변수&quot;로 또는 페이지의 특정 부분으로 이동합니다.

Apache에서 (*아래 다이어그램 참조*),

* `pagename.selectors.html` 는 캐시의 파일 시스템에서 파일 이름으로 사용됩니다.

URL에 접미사가 있는 경우 `path/suffix.ext` 그런 다음,

* `pagename.selectors.html` 이(가) 폴더로 생성됨

* `path` 폴더의 폴더 `pagename.selectors.html` 폴더

* `suffix.ext` 은(는) 의 파일입니다. `path` 폴더를 삭제합니다. 참고: 접미사에 확장명이 없으면 파일이 캐시되지 않습니다.
...
1. URL은 매우 길어질 수 있습니다. 의 &quot;경로&quot; 부분 추가 `/docroot` 로컬 파일 시스템에서 일부 파일 시스템의 한도를 쉽게 초과할 수 있습니다. Windows에서 NTFS로 Dispatcher를 실행하는 것은 어려울 수 있습니다. 그러나 Linux를 사용하면 안전합니다.
...
`http://domain.com/home.html` 은(는) **캐시 가능**

`http://domain.com/home` 은(는) **캐시 불가능**

URL에 접미사가 포함된 경우에도 동일한 규칙이 적용됩니다. 접미사를 사용하려면 확장명이 있어야 합니다.

**예**

`http://domain.com/home.html/path/suffix.html` 은(는) **캐시 가능**

`http://domain.com/home.html/path/suffix` 은(는) **캐시 불가능**
...
다음 `/home/path/suffix` 는 리소스의 경로이므로 URL에 접미사가 없습니다.
...
AEM에서 절대적으로 유효합니다. 로컬 개발 시스템에는 Dispatcher가 없어도 문제가 발생하지 않습니다. 또한 UAT 또는 로드 테스트에서도 문제가 발생하지 않을 가능성이 높습니다. 우리가 직면한 문제는 너무 미묘해서 대부분의 시험을 통과했다.  피크 타임에 있고 해결할 시간이 제한되어 있고 서버 액세스 권한 또는 이를 수정할 리소스가 없을 때 이 문제가 사용자에게 매우 큰 영향을 미칩니다. 우리는 그곳에 갔다...

그래서... 뭐가 문제야?

`home.html` 파일 시스템에서 파일 또는 폴더가 될 수 있습니다. AEM에서와 동시에 둘 다 실행할 수는 없습니다.

요청하시면 `home.html` 먼저, 파일로 만들어집니다.

에 대한 후속 요청 `home.html/suffix.html` 유효한 결과를 반환하지만, 파일로 `home.html` 파일 시스템에서 위치를 &quot;차단&quot;합니다.  `home.html` 을(를) 폴더로 두 번 만들 수 없으므로 `home.html/suffix.html` 은(는) 캐시되지 않습니다.
...
반대로 하면, 먼저 `home.html/suffix.html` 그러면 `suffix.html` 폴더에 캐시됨 `/home.html` 처음에는. 그러나 이 폴더는 삭제되고 파일로 대체됩니다. `home.html` 나중에 요청할 때 `home.html` as a resource.
...
따라서 캐시된 사항의 결과는 전적으로 임의적이며 수신 요청의 순서에 따라 달라집니다. 문제를 더 까다롭게 하는 것은 일반적으로 디스패처가 두 개 이상 있다는 사실입니다. 또한 성능, 캐시 적중률 및 동작은 Dispatcher마다 다를 수 있습니다. 웹 사이트가 응답하지 않는 이유를 확인하려면 잘못된 캐싱 순서가 있는 올바른 Dispatcher를 보고 있는지 확인해야 합니다. 운이 좋게도 더 유리한 요청 패턴이 있는 Dispatcher를 보면 문제를 찾는 데 어려움을 겪을 수 있습니다.
...
마지막 장에 대한 간략한 요약과 몇 가지 더 많은 예외를 검토하겠습니다. Dispatcher는 캐시 가능으로 구성된 경우 및 GET 요청인 경우 URL을 캐시할 수 있습니다. 다음 예외 중 하나에서는 캐시할 수 없습니다.

**캐시 가능한 요청**

* Dispatcher 구성에서 요청을 캐시할 수 있도록 구성됨
* 요청은 일반 GET 요청입니다.
...
마지막 장에서는 Dispatcher가 요청을 캐시할 수 없는 경우 많은 예외를 나열했습니다. 그러나 Dispatcher가 다음과 같은 이유로 더 많은 고려 사항이 있습니다. _can_ 요청을 캐시합니다. 그렇다고 해서 요청을 캐시하는 것은 아닙니다. _다음과 같아야 함_.

중요한 점은 캐싱은 일반적으로 쉽다는 것입니다. Dispatcher는 응답 결과를 저장하고 다음에 동일한 요청이 들어올 때 반환하면 됩니다. 오른쪽? 틀렸어요!

어려운 부분은 _무효화_ 또는 _플러싱_ 캐시에서 사용됩니다. Dispatcher는 리소스가 언제 변경되었는지 파악하고 다시 렌더링해야 합니다.
...
Dispatcher의 파일 시스템에서.

이제 사용자가 해당 파일의 새 버전을 업로드하고 활성화합니다. 궁극적으로 무효화 요청이 AEM에서 Dispatcher로 전송됩니다.
...
무효화는 쉽습니다. Dispatcher의 특수 &quot;/invalidate&quot; URL에 대한 간단한 GET 요청입니다. HTTP-body가 필요하지 않습니다. &quot;payload&quot;는 &quot;invalidate-path&quot; 헤더일 뿐입니다. 또한 헤더의 invalidate-path는 Dispatcher가 캐시한 파일 또는 파일이 아니라 AEM이 알고 있는 리소스입니다. AEM은 리소스만 알고 있습니다. 리소스가 요청될 때 런타임에 확장, 선택기 및 접미사가 사용됩니다. AEM은 리소스에서 사용된 선택기에 대한 부기를 수행하지 않으므로 리소스 경로는 리소스를 활성화할 때 확실히 알 수 있습니다.

이 정도면 충분합니다. 리소스가 변경된 경우 해당 리소스의 모든 렌디션도 변경되었다고 안전하게 가정할 수 있습니다. 이 예제에서 이미지가 변경된 경우 새 썸네일도 렌더링됩니다.

Dispatcher는 캐시된 모든 렌디션을 사용하여 리소스를 안전하게 삭제할 수 있습니다. It will do something like,

`$ rm /content/dam/path/to/image.*`

제거 중 `image.png` 및 `image.thumb.png` 및 해당 패턴과 일치하는 다른 모든 렌디션.
...
AEM에서는 매력적으로 작동하지만 게시 인스턴스에서 Dispatcher를 사용하는 경우 이상한 일이 발생합니다.
...
*대상* 홈 페이지입니다. 홈 페이지는 티저와 파일의 헤드라인을 포함하여 정적 .html 파일로 Dispatcher에 의해 저장됩니다.
...
![홈 페이지에 오래된 포함 콘텐츠를 저장하는 Dispatcher](assets/chapter-1/dispatcher-storing-stale-content.png)

*홈 페이지에 오래된 포함 콘텐츠를 저장하는 Dispatcher*

<br> 

무슨 일이 있었던 거야? Dispatcher는 렌더링하는 동안 다른 리소스에서 가져온 모든 콘텐츠 및 마크업이 포함된 페이지의 정적 버전을 저장합니다.

Dispatcher는 파일 시스템 기반 웹 서버일 뿐 아니라 빠르고 간단합니다. 포함된 리소스가 변경되는 경우 이를 인식하지 못합니다. 포함 페이지가 렌더링될 때 있던 콘텐츠에 계속 연결됩니다.

&quot;겨울 특수&quot; 페이지는 아직 렌더링되지 않았으므로 Dispatcher에 정적 버전이 없으므로 요청 시 새로 렌더링될 때 새 티저와 함께 표시됩니다.

Dispatcher는 리소스가 변경될 때 이 리소스를 사용한 모든 페이지를 렌더링하고 플러시하는 동안 해당 리소스가 접촉하는 모든 리소스를 추적한다고 생각할 수 있습니다. 그러나 Dispatcher는 페이지를 렌더링하지 않습니다. 렌더링은 Publish 시스템에서 수행됩니다. Dispatcher는 렌더링된 .html 파일에 어떤 리소스가 들어가는지 모릅니다.

아직도 납득이 안 가요? 당신은 아마 *&quot;일종의 종속성 추적을 구현하는 방법이 있어야 합니다.&quot;*. 있기는 하지만 더 정확하게 말하자면 *다음과 같음*. Communiqué 3 AEM의 고조부는 _session_ 페이지를 렌더링하는 데 사용되었습니다.
...
![기본 탐색은 필연적으로 &quot;NavTitles&quot;를 가져와서 모든 페이지의 컨텐츠를 서로 결합합니다.](assets/chapter-1/nav-titles.png)

*기본 탐색은 필연적으로 &quot;NavTitles&quot;를 가져와서 모든 페이지의 컨텐츠를 서로 결합합니다.*
...
이제 수천 페이지의 대형 사이트가 있는 경우 모든 페이지를 루프하고 물리적으로 삭제하는 데 시간이 꽤 걸릴 수 있습니다. 이 기간 동안 Dispatcher는 의도하지 않게 오래된 콘텐츠를 제공할 수 있습니다. 더 심각한 것은 캐시 파일에 액세스하는 동안 충돌이 발생할 수 있기 때문이며, 페이지를 삭제하는 동안 페이지가 요청되거나 즉시 후속 활성화 후 발생한 두 번째 무효화로 페이지가 다시 삭제될 수 있습니다. 얼마나 엉망진창인지 생각해 보세요. 다행히도 이런 일은 일어나지 않습니다. Dispatcher는 이를 방지하기 위해 영리한 트릭을 사용합니다. 수백, 수천 개의 파일을 삭제하는 대신 파일이 게시될 때 파일 시스템의 루트에 단순한 빈 파일을 배치하여 따라서 모든 종속 파일이 유효하지 않은 것으로 간주됩니다. 이 파일을 &quot;statfile&quot;이라고 합니다. statfile은 빈 파일입니다. statfile에 중요한 것은 파일의 생성 날짜입니다.

Dispatcher의 생성 날짜가 statfile보다 오래된 모든 파일은 마지막 활성화(및 무효화) 전에 렌더링되었으므로 &quot;유효하지 않음&quot;으로 간주됩니다. 이러한 파일은 물리적으로 여전히 파일 시스템에 존재하지만 Dispatcher는 이를 무시합니다. &quot;부실&quot; 상태입니다. 부실 리소스에 대한 요청이 이루어질 때마다 Dispatcher는 AEM 시스템에 페이지를 다시 렌더링하도록 요청합니다. 새로 렌더링된 페이지는 파일 시스템에 저장되며, 이제 새 생성 날짜가 있으며 다시 새로 고쳐집니다.

![.stat 파일의 생성 날짜는 오래된 콘텐츠 및 최신 콘텐츠를 정의합니다.](assets/chapter-1/creation-date.png)

*.stat 파일의 생성 날짜는 오래된 콘텐츠 및 최신 콘텐츠를 정의합니다.*

<br> 

왜 &quot;.stat&quot;라고 하는지 물어볼 수 있습니까? 그리고 아마도 &quot;.invalidated&quot;는 아닐까요? 파일 시스템에 해당 파일이 있으면 Dispatcher가 리소스를 결정할 수 있습니다 *정적으로* 정적 웹 서버에서와 같이 제공됩니다. 이러한 파일은 더 이상 동적으로 렌더링할 필요가 없습니다.

그러나 그 이름의 본성은 덜 은유적이다. Unix 시스템 호출에서 파생됩니다 `stat()`: 다른 속성에서 파일의 수정 시간을 반환합니다.

#### 단순 유효성 검사와 자동 유효성 검사 혼합

하지만 기다려... 아까 말하기를, 단일 리소스가 물리적으로 삭제된다고 했습니다. 이제 더 최신 상태 파일이 Dispatcher의 관점에서 사실상 무효화된다고 가정합니다. 왜 물리적 삭제가 먼저 이루어졌을까요?
...
Dispatcher는 파일 확장명에 따라 자동 무효화 스키마를 적용할지 여부를 결정합니다.
...
자체 포함된 리소스는 해당 리소스의 경로에서 제공되어야 합니다. 그것은 무효화에 많은 도움이 됩니다. 단순하게 유지하고, &quot;/x/y/z&quot;에서 제공되는 &quot;resource /a/b/c&quot;와 같은 매핑 체계를 만들지 마십시오. 구성 요소가 기본 Dispatcher의 자동 무효화 설정에서 작동하도록 합니다. Dispatcher에서 초과 무효화가 있는 잘못 설계된 구성 요소를 복구하지 마십시오.
...
2. 에이전트가 HTTP 헤더를 전송하는 경우 `CQ-Action-Scope: ResourceOnly`즉, 이 단일 무효화 요청이 자동 무효화를 트리거하지 않습니다. 이 ( [https://github.com/cqsupport/webinar-dispatchercache/tree/master/src/refetching-flush-agent/refetch-bundle](https://github.com/cqsupport/webinar-dispatchercache/tree/master/src/refetching-flush-agent/refetch-bundle)) 코드 조각은 자체 복제 에이전트의 좋은 시작점이 될 수 있습니다.

3. `ResourceOnly`는 자동 무효화만 방지합니다. 필요한 종속성 해결 및 무효화를 실제로 수행하려면 무효화 요청을 직접 트리거해야 합니다. 패키지 Dispatcher 플러시 규칙([https://adobe-consulting-services.github.io/acs-aem-commons/features/dispatcher-flush-rules/index.html](https://adobe-consulting-services.github.io/acs-aem-commons/features/dispatcher-flush-rules/index.html)) 실제로 그러한 일이 어떻게 일어날 수 있는지에 대한 영감을 얻습니다.

종속성 해결 스키마를 빌드하지 않는 것이 좋습니다. 그러나 너무 많은 노력과 적은 이득이 있을 뿐이며, 앞서 말한 바와 같이, 여러분이 틀릴 수도 있는 것은 너무 많습니다.

대신 다른 리소스에 종속되지 않고 자동 무효화 없이 무효화될 수 있는 리소스를 확인해야 합니다. 하지만 이러한 경우에는 사용자 정의 복제 에이전트를 사용할 필요가 없습니다. Dispatcher 구성에서 이러한 리소스를 자동 무효화에서 제외하는 사용자 지정 규칙을 만들면 됩니다.
...
## Dispatcher를 염두에 두고 구성 요소 구축

### 실제 예제에 Dispatcher 역학 적용

마지막 장에서는 Dispatcher의 기본 역학 방법, 일반적인 작동 방식과 제한 사항에 대해 설명했습니다.
...
에셋은 AEM의 DAM 영역에만 업로드됩니다 _참조됨_ 응답형 이미지 구성 요소

응답 구성 요소는 마크업의 렌더링과 이진 이미지 데이터 전달을 모두 처리합니다.

여기에서 구현하는 방식은 많은 프로젝트에서 확인한 일반적인 패턴이며, AEM 핵심 구성 요소 중 하나라도 이 패턴을 기반으로 합니다. 따라서 개발자인 사용자가 이러한 패턴을 조정할 수 있습니다. 캡슐화에 있어 유리한 지점이 있지만, Dispatcher에서 사용할 수 있도록 하려면 많은 노력이 필요합니다. 우리는 나중에 문제를 완화하는 몇 가지 옵션에 대해 논의할 것이다.
...
원래 &#39;스풀링&#39;이라는 용어는 실제로는 프린터와 같이 공유가 느린 오프라인 주변기기를 의미하므로 여기서는 올바르게 적용되지 않습니다. 하지만 우리는 어쨌든 이 용어를 좋아하는데, 왜냐하면 그것은 온라인 세계에서 드물기 때문에 구별할 수 있기 때문이다. 그리고 각각의 패턴은 구별할 수 있는 이름을 가져야 합니다. 그렇죠? 이것이 패턴인지 안티 패턴인지 결정하는 것은 여러분에게 달려 있습니다.

#### 구현

반응형 이미지 구성 요소는 다음과 같이 구현됩니다.

구성 요소에는 두 개의 부분이 있습니다. 첫 번째 부분은 이미지의 HTML 마크업을 렌더링하고, 두 번째 부분은 참조된 이미지의 이진 데이터를 &quot;스풀&quot;합니다. 반응형 디자인의 최신 웹 사이트이므로 간단하게 렌더링하고 있지 않습니다 `<img src"…">` 태그로 변환되지만 `<picture/>` 태그에 가깝게 배치하십시오. 각 장치에 대해 서로 다른 두 이미지를 DAM에 업로드하고 이미지 구성 요소에서 참조합니다.

구성 요소에는 전용 선택기를 사용하여 각각 지정된 세 개의 렌더링 스크립트(JSP, HTL 또는 서블릿으로 구현됨)가 있습니다.

1. `/respi.jsp` - HTML 마크업을 렌더링하는 선택기 없음
2. `/respi.img.java` 데스크탑 버전을 렌더링하려면
3. `/respi.img.mobile.java` 모바일 버전을 렌더링합니다.


구성 요소는 홈 페이지의 parsys에 배치됩니다. CRX에서의 결과 구조가 아래에 예시되어 있다.
...
이제 사용자가 Dispatcher를 통해 페이지 및 에셋을 요청합니다. 그러면 아래 그림과 같이 Dispatcher 파일 시스템에 파일이 생성됩니다.
...
캐시되지 않은 새 페이지가 요청될 때마다 자산은 다른 URL의 AEM에서 가져옵니다. Dispatcher 캐싱과 브라우저 캐싱이 없으므로 게재 속도가 빨라집니다.
...
AEM 개발자의 관점에서 볼 때 이 패턴은 매우 우아해 보였다. 그러나 Dispatcher를 식으로 간주하면 순진한 접근 방식이 충분하지 않을 수 있다는 점에 동의할 수 있습니다.
...
다음과 같은 쿼리 매개 변수를 본 적이 있습니까? `?ck=398547283745`? 이를 캐시 킬러(&quot;ck&quot;)라고 합니다. 쿼리 매개 변수를 추가하면 리소스가 캐시되지 않는다는 아이디어입니다. 또한 매개 변수 값으로 난수를 추가하는 경우(예: &quot;398547283745&quot;) URL은 고유해지며 AEM 시스템과 화면 간에 다른 캐시 중 어느 것도 캐시할 수 없도록 합니다. 일반적인 중간 용의자는 Dispatcher, CDN 또는 브라우저 캐시 앞에 &quot;Varnish&quot; 캐시가 있습니다. 다시: 그러지 마. 리소스는 가능한 한 오래 캐싱하려고 합니다. 캐시는 당신의 친구입니다. 친구를 죽이지 마세요

#### 자동 무효화

>[!WARNING]
>
>이건 무늬를 없애는 겁니다. 디지털 에셋에 사용하지 마십시오. Dispatcher의 기본 구성을 유지해 보십시오. > 은 &quot;.html&quot; 파일에만 자동 무효화됩니다.
...
Unix 타임스탬프는 실제 구현에 적합합니다. 가독성을 높이기 위해 이 자습서에서는 읽기 쉬운 형식을 사용합니다. `2018 31.12 23:59 or fp-2018-31-12-23-59`.

쿼리 매개 변수가 있는 URL은 캐시할 수 없으므로 지문을 쿼리 매개 변수로 사용해서는 안 됩니다. 선택기나 접미사를 지문에 사용할 수 있습니다.

파일을 다음과 같이 가정합니다. `/content/dam/flower.jpg` 다음 포함 `jcr:lastModified` 2018년 12월 31일, 23:59. 지문이 있는 URL은 `/content/home/jcr:content/par/respi.fp-2018-31-12-23-59.jpg`.

이 URL은 참조된 리소스(`flower.jpg`) 파일이 변경되지 않았습니다. 따라서 무한한 시간 동안 캐시될 수 있으며 캐시 킬러가 아닙니다.

이 URL은 응답형 이미지 구성 요소에서 만들고 제공해야 합니다. 기본 AEM 기능이 아닙니다.

그것이 기본 개념입니다. 그러나 간과하기 쉬운 몇 가지 세부 사항이 있습니다.

이 예에서 구성 요소는 23:59에 렌더링되고 캐시되었습니다. 이제 이미지가 변경되었습니다. 예를 들어 00:00에 변경하겠습니다.  구성 요소 _의지_ 마크업에 새 지문 URL을 생성합니다.

그렇게 생각할지도 모르지 _다음과 같아야 함_... 하지만 그렇지 않습니다. 이미지의 이진만 변경되었으며 포함 페이지를 터치하지 않았으므로 HTML 마크업을 다시 렌더링할 필요가 없습니다. 따라서 Dispatcher는 이전 지문이 있는 페이지를 제공하며 따라서 이전 버전의 이미지가 제공됩니다.
...
이제 홈 페이지(또는 해당 사이트의 다른 페이지)를 다시 활성화하면 statfile이 업데이트되고 Dispatcher는 home.html이 오래된 것으로 간주하여 이미지 구성 요소의 새 지문으로 다시 렌더링합니다.
...
지연 관리자는 일반적으로 &quot;_자동 무효화를 jpgs로 설정하고 statfile-level을 0으로 설정합니다. 이 값은 항상 모든 종류의 문제를 캐시하는 데 도움이 됩니다._.&quot; 기술 포럼에서 이러한 조언을 찾을 수 있으며, 이는 무효화 문제에 도움이 됩니다.

지금까지 statfile 수준에 대해 논의하지 않았습니다. 기본적으로 자동 무효화는 동일한 하위 트리에 있는 파일에만 작동합니다. 그러나 페이지와 에셋이 일반적으로 동일한 하위 트리에 있지 않다는 문제가 있습니다. 페이지는 아래 어딘가에 있습니다. `/content/mysite` 반면에 자산은 아래에 있습니다. `/content/dam`.
...
어쨌든 - &quot;.jpg&quot; 또는 &quot;.png&quot;가 새로운 URL을 사용하여 다시 렌더링할 수 있도록 변경된 경우 어떻게든 HTML 페이지를 무효화하도록 Dispatcher에 알려야 합니다.
...
이렇게 하면 사용자 지정 Dispatcher 플러시 에이전트에서 다음에 변경 사항이 발생할 때 /content/site-a로 쉽게 및 무효화 요청을 보낼 수 있습니다 `/content/dam/site-a`.

실제로 동일한 사이트, 동일한 &quot;하위 트리&quot;에 있는 한 Dispatcher에 무효화하도록 지시하는 경로가 중요하지 않습니다. 실제 리소스 경로를 사용할 필요도 없습니다. &quot;가상&quot;일 수도 있습니다.
...
6. 이미지가 실제로 요청되면 새 렌디션이 생성되어 Dispatcher에 저장됩니다
...
무효화된 리소스와 관련이 없습니다. 기억? &quot;더미&quot; 리소스만 무효화하고 자동 무효화에 의존하여 &quot;홈&quot;을 무효로 간주했습니다. 이미지 자체는 다음과 같을 수 없습니다. _물리적으로_ 삭제됨. 따라서 캐시는 증가하고 확장되며 확장됩니다. 이미지를 변경하고 활성화하면 Dispatcher 파일 시스템에서 새 파일 이름을 가져옵니다.
...
각 요청은 Dispatcher를 우회하여 게시 인스턴스에 로드됩니다. 또한 더 나쁜 것은 Dispatcher에 따라 파일을 생성한다는 것입니다.

따라서... 간단한 캐시 킬러로 지문을 사용하는 대신 이미지의 jcr:lastModified 날짜를 확인하고 예상 날짜가 아닌 경우 404를 반환해야 합니다. 이 작업에는 게시 시스템에서 시간이 걸리고 CPU 사이클이 필요하며, 처음부터 방지하려는 작업입니다.

#### 고주파수 릴리스에서 URL 지문 주의

DAM에서 가져온 자산뿐만 아니라 JS 및 CSS 파일과 관련 리소스에 대해서도 지문 스키마를 사용할 수 있습니다.

[버전이 지정된 Clientlibs](https://adobe-consulting-services.github.io/acs-aem-commons/features/versioned-clientlibs/index.html) 는 이 접근 방식을 사용하는 모듈입니다.

그러나 여기서는 URL 지문을 갖는 또 다른 주의 사항을 직면할 수 있습니다. 이는 URL을 콘텐츠에 연결합니다. URL을 변경하지 않으면 콘텐츠를 변경할 수 없습니다(즉, 수정 날짜를 업데이트). 그것이 우선 지문이 설계된 것입니다. 하지만 새로운 CSS 및 JS 파일과 새로운 지문이 있는 새로운 URL을 사용하여 새로운 릴리스를 롤아웃하는 것을 고려하십시오. 모든 HTML 페이지에는 여전히 이전 지문 URL에 대한 참조가 있습니다. 따라서 새 릴리스가 일관되게 작동하게 하려면 모든 HTML 페이지를 한 번에 무효화하여 새로 지문 처리된 파일에 대한 참조를 사용하여 다시 렌더링하도록 해야 합니다. 동일한 라이브러리에 의존하는 사이트가 여러 개 있는 경우 상당한 양의 재렌더링이 가능할 수 있으며, 여기에서는 를 활용할 수 없습니다. `statfiles`. 따라서 롤아웃 이후 Publish 시스템에서 로드 피크를 확인할 수 있도록 준비하십시오. 캐시 워밍이 있는 파란색-녹색 배포를 고려하거나 Dispatcher 앞에 TTL 기반 캐시를 고려할 수 있습니다. 가능성은 무한합니다.

#### 짧은 휴식

와우, 고려해야 할 세부 사항이 많네요, 그렇죠? 그리고 쉽게 이해되고 테스트되고 디버깅되는 것을 거부합니다. 그리고 이 모든 것이 겉으로 보기에 우아한 해결책을 위한 것입니다. 확실히, 그것은 우아하다 - 하지만 오직 AEM 전용 관점에서만. Dispatcher와 함께 그것은 불쾌해집니다.
...
Dispatcher가 리소스의 종속성에 대해 알지 못합니다. 의미 체계도 거의 없는 단일 파일일 뿐입니다.
...
![구성 요소가 아니라 이미지에 바인딩된 서블릿을 사용하여 이미지 스풀링.](assets/chapter-1/spooling-image.png)

*구성 요소가 아니라 이미지에 바인딩된 서블릿을 사용하여 이미지 스풀링.*

<br> 

Assets 원본 리소스 경로를 사용하여 데이터를 렌더링합니다. 원본 이미지를 그대로 렌더링해야 하는 경우 에셋에 AEM 기본 렌더러를 사용할 수 있습니다.

특정 구성 요소에 대해 특수 처리를 수행해야 하는 경우 해당 경로 및 선택기에 전용 서블릿을 등록하여 구성 요소를 대신하여 변환합니다. 우리는 여기서 &quot;.respi&quot;로 모범적으로 그것을 했다. 선택기. 전역 URL 공간에서 사용되는 선택기 이름을 추적하는 것이 좋습니다(예: `/content/dam`) 좋은 이름 지정 규칙을 사용하여 이름 지정 충돌을 피할 수 있습니다.
...
없는 것은 품질의 가치입니다. 구성 요소는 작성자가 입력한 속성을 알고 있습니다... 다음과 같이 마크업이 렌더링될 때 쿼리 매개 변수로 이미지 렌더링 서블릿에 쉽게 전달할 수 있습니다. `flower.respi2.jpg?quality=60`:
...
이것은 나쁜 생각이다. 기억? 쿼리 매개 변수가 있는 요청은 캐시할 수 없습니다.
...
이 경우 다시 캐시를 무시하고 게시 시스템에 로드를 생성합니다. 그래서, 그것은 나쁜 생각일지도 모릅니다. 매개 변수의 작은 하위 집합만 필터링하여 이를 완화할 수 있습니다. 만 허용하시겠습니까? `q-20, q-40, q-60, q-80, q-100`.
...
1. 이 필터는 CRX에 너무 많이 액세스하지 않고 메모리 및 시간을 낭비하지 않고 매우 효율적으로 구현됩니다.
...
더 좋은 방법은 를 반환하는 것입니다. `301 – Moved permanently`:
...
여기 AEM이 브라우저에 알려줍니다. &quot;난... `q-41`. 하지만, 저기.. `q-40` &quot;.

이 경우 대화에 추가 요청 응답 루프가 추가되므로 약간의 오버헤드가 발생하지만 전체 처리를 수행하는 것보다 비용이 저렴합니다 `q-41`. 또한 이미 아래에 캐시된 파일을 활용할 수 있습니다 `q-40`. 그러나 302 응답은 Dispatcher에 캐시되지 않으며 AEM에서 실행되는 논리에 대해 논의하고 있음을 이해해야 합니다. 계속해서... 그래서 날씬하고 빠르게 만드는 게 좋죠.
...
실제로 각 수준에 약간의 보안이 제공됩니다. 하지만 어떤 수준에서 요청이 필터링되는지 확인할 수 있는 수단이 있는지 확인하십시오. 체인의 어떤 필터가 요청을 차단하는지 확인하려면 Publish 시스템, Dispatcher 및 WAF의 로그 파일에 직접 액세스할 수 있어야 합니다.
...
이 패턴을 _반전 스풀러_.. 모든 캐시 무효화 속성을 사용하려면 반전 스풀러가 이미지 리소스를 기반으로 해야 합니다.
...
`/content/dam/flower` 는 이미지 리소스의 경로입니다

`.respi3` 는 이미지를 전달할 올바른 서블릿을 선택하는 선택기입니다

`.content-mysite-home-jcrcontent-par-respi` 는 추가 선택기입니다. 이미지 변환에 필요한 속성을 저장하는 구성 요소에 대한 경로를 인코딩합니다. 선택기는 경로보다 작은 문자 범위로 제한됩니다. 여기서 인코딩 방식은 단지 예시적인 것이다. &quot;/&quot;를 &quot;-&quot;로 대체합니다. 경로 자체에도 &quot;-&quot;를 포함할 수 있다는 것은 고려되지 않습니다. 좀 더 정교한 인코딩 방식은 실제 사례에서 권장될 것이다. Base64는 정상이어야 합니다. 그러나 이로 인해 디버깅이 좀 더 어려워집니다.

`.jpg` 는 파일 접미사입니다

### 결론

와우... 스풀러 얘기가 생각보다 길어지고 복잡해졌어요. 우리는 당신에게 변명의 여지를 줍니다. 하지만 Dispatcher-land에서 잘 작동하는 것과 그렇지 않은 것에 대한 직관을 기를 수 있도록 좋은 것과 나쁜 것 등 여러 측면을 제시할 필요가 있다고 생각했습니다.
...
우리는 이미 다음을 간단히 언급했습니다. _statfile_ 전. 자동 무효화와 관련되어 있습니다.

자동 무효화되도록 구성된 Dispatcher의 파일 시스템에 있는 모든 캐시 파일은 마지막 수정 날짜가 다음보다 오래된 경우 유효하지 않은 것으로 간주됩니다. `statfile's` 마지막 수정 날짜.

>[!NOTE]
>
>마지막으로 수정된 날짜는 캐시된 파일이며 클라이언트의 브라우저에서 파일이 요청되어 파일 시스템에서 최종적으로 생성된 날짜입니다. 다음 항목이 아닙니다. `jcr:lastModified` 리소스의 날짜입니다.

statfile의 마지막 수정 날짜(`.stat`)는 Dispatcher에서 AEM의 무효화 요청이 수신된 날짜입니다.

Dispatcher가 두 개 이상 있는 경우 이로 인해 이상한 영향이 발생할 수 있습니다. 브라우저에 Dispatcher 최신 버전이 있을 수 있습니다(Dispatcher가 두 개 이상인 경우). 또는 Dispatcher가 다른 Dispatcher에서 발급한 브라우저 버전이 오래되어 새 복사본을 불필요하게 전송한다고 생각할 수 있습니다. 이러한 효과는 성능 또는 기능 요구 사항에 큰 영향을 주지 않습니다. 브라우저에 최신 버전이 있으면 시간이 지남에 따라 레벨이 조정됩니다. 하지만 브라우저 캐싱 동작을 최적화하고 디버깅하는 경우에는 다소 혼란스러울 수 있습니다. 조심하세요.

#### /statfileslevel을 사용하여 무효화 도메인 설정

자동 무효화와 상태 파일을 도입했을 때, *모두* 변경 사항이 있고 모든 파일이 상호 종속되어 있는 경우 파일이 유효하지 않은 것으로 간주됩니다.

정확하지 않습니다. 일반적으로 공통 기본 탐색 루트를 공유하는 모든 파일은 상호 종속됩니다. 그러나 하나의 AEM 인스턴스가 여러 웹 사이트를 호스팅할 수 있음 - *독립* 웹 사이트. 일반적인 탐색을 공유하지 않음 - 사실상, 아무 것도 공유하지 않음.

A사이트에 변경 사항이 있어 B사이트를 무효화시키는 것은 아깝지 않을까요? 네, 그렇습니다. 그리고 그렇게 될 필요는 없습니다.

Dispatcher는 사이트를 서로 분리하는 간단한 방법을 제공합니다. `statfiles-level`.

이 숫자는 파일 시스템에서 두 개의 하위 트리가 &quot;독립&quot;으로 간주되는 수준을 정의하는 숫자입니다.

statfileslevel이 0인 기본 사례를 살펴보겠습니다.

![/statfileslevel &quot;0&quot;: _._ docroot에 만들어집니다(_S). 무효화 도메인은 모든 사이트를 포함한 전체 설치에 걸쳐 있습니다](assets/chapter-1/statfile-level-0.png)

`/statfileslevel "0":` 다음 `.stat` 파일이 docroot에 만들어집니다. 무효화 도메인은 모든 사이트를 포함한 전체 설치에 걸쳐 있습니다.

무효화된 파일의 이름은 `.stat` dispatchers docroot의 맨 위에 있는 파일은 항상 업데이트됩니다. 따라서 무효화할 때 `/content/site-b/home`, 또한 의 모든 파일 `/content/site-a` 은(는) 현재 다음보다 나이가 많으므로 무효화됩니다. `.stat` docroot에 있는 파일입니다. 무효화할 때 필요한 것은 확실히 아닙니다. `site-b`.

이 예제에서는 `statfileslevel` 끝 `1`.

이제 을 게시하여 무효화하면 `/content/site-b/home` 또는 아래에 있는 기타 모든 리소스 `/content/site-b`, `.stat` 파일 생성 위치: `/content/site-b/`.

아래 콘텐츠 `/content/site-a/` 은 영향을 받지 않습니다. 이 콘텐츠는 과 비교됩니다. `.stat` 파일 위치: `/content/site-a/`. 두 개의 별도 무효화 도메인을 만들었습니다.

![statfileslevel &quot;1&quot;은 다른 무효화 도메인을 생성합니다](assets/chapter-1/statfiles-level-1.png)

*statfileslevel &quot;1&quot;은 다른 무효화 도메인을 생성합니다*

<br> 

대규모 설치는 일반적으로 조금 더 복잡하고 심층적으로 구성됩니다. 일반적인 스키마는 브랜드, 국가 및 언어별로 사이트를 구성하는 것입니다. 이 경우 statfileslevel을 더 높게 설정할 수 있습니다. _1_ 는 브랜드당 무효화 도메인을 만들고, _2_ 국가 및 _3_ 언어별로.
...
전자는 다음을 필요로 합니다. `statfileslevel` / _2_, 반면에 후자는 _3_.

이상적인 상황은 아니야 을 로 설정하면 _3_&#x200B;하위 분기 사이의 작은 사이트 내에서는 자동 무효화가 작동하지 않습니다 `/home`, `/products` 및 `/about`.

설정 _2_ 즉, 선언하는 더 큰 사이트에서 `/canada/en` 및 `/canada/fr` 의존적이긴 하지만 아닐 수도 있죠 따라서 의 각 무효화 `/en` 이(가) 무효화됩니다. `/fr`. 이렇게 하면 캐시 적중률이 약간 줄어들지만 오래된 캐시된 콘텐츠를 전달하는 것보다 훨씬 좋습니다.
...
어느 수준이 적당할까요? 사이트 간 종속성 수에 따라 다릅니다. 페이지 렌더링에 대해 확인하는 포함은 &quot;엄격한 종속성&quot;으로 간주됩니다. 우리는 그러한 사례를 입증했다 _포함_ 을(를) 소개했을 때 _티저_ 이 안내서의 시작 부분에 있는 콘텐츠.

_하이퍼링크_ 는 더 부드러운 종속성 형식입니다. 한 웹 사이트 내에서 하이퍼링크를 사용할 가능성이 높으며 웹 사이트 간에 링크가 있을 가능성도 낮지 않습니다. 단순 하이퍼링크는 일반적으로 웹 사이트 간에 종속성을 만들지 않습니다. 사이트에서 facebook으로 설정한 외부 링크만 생각해 보십시오. facebook에서 변경되는 내용이 있거나 그 반대로 변경되는 경우 페이지를 렌더링하지 않아도 됩니다.
...
투명하게 만들어 순위가 낮아지는 것을 방지하고, 실제로 동일한 내용의 페이지가 두 개 이상 있으며, 시스템을 &quot;게임&quot;하려고 하지 않을 수 있습니다(참조) [&quot;Google에 페이지의 현지화된 버전에 대해 알려주십시오.&quot;](https://support.google.com/webmasters/answer/189077?hl=en)) 로 설정합니다. `<link rel="alternate">` 각 페이지의 헤더 섹션에서 각 관련 페이지에 태그 추가:
...
![모두 상호 연결](assets/chapter-1/inter-linking-all.png)
...
이 계획은 많은 링크뿐만 아니라 몇 가지 문제를 만들었습니다. 에 필요한 링크 수 _p_ 위치: _n_ 언어는 입니다. _p x (n<sup>2</sup>-n)_: 각 페이지는 서로 다른 페이지로 연결됩니다(_n x n_&#x200B;자신을 제외한 ( )_-n_). 이 구성표는 각 페이지에 적용됩니다. 4개 언어로 된 20페이지의 작은 사이트가 있는 경우 각각 _240_ 링크.
...
우리 프로젝트에서는 새로운 상대 페이지가 자주 떴다. 그러나 그들은 &quot;대체&quot; 연결로 구체화되지 않았다. 예를 들어, `de-de/produkte` 페이지가 독일 웹 사이트에 게시되었습니다, 그것은 즉시 다른 사이트에 볼 수 없었습니다.
...
`shiny-brand` 이(가) 호출됨 `marque-brillant` 프랑스에서 `blitzmarke` 독일에서:
...
That would have mean의도한 to set설정 the `statfiles` 수준 1 - 너무 큰 무효화 도메인으로 이어졌을 것입니다.

그 부지를 재구조화하면 그것을 고칠 수 있었을 것이다. 모든 브랜드를 하나의 공통 루트 아래에 병합합니다. 하지만 그 당시에는 능력이 없었기 때문에 - 그것은 레벨 2만 주었을 것입니다.

우리는 레벨 3을 고수하기로 결정했고 항상 최신 &quot;대체&quot; 링크를 가지고 있지 않은 가격을 지불했습니다. 이를 줄이기 위해 Dispatcher에서 &quot;리퍼&quot; 크론 작업이 실행되었으며, 이는 어쨌든 1주 이상 지난 파일을 정리합니다. 결국 모든 페이지가 어느 시점에 다시 렌더링되었습니다. 그러나 그것은 각각의 프로젝트에서 개별적으로 결정되어야 하는 상충관계입니다.

## 결론

Dispatcher가 일반적으로 작동하는 방식에 대한 몇 가지 기본 원칙을 다뤘으며 이를 제대로 수행하기 위해 구현 노력을 조금 더 기울여야 하는 위치와 절충점을 만들 수 있는 위치에 대한 몇 가지 예를 제공했습니다.

Dispatcher에 구성되는 방법에 대한 자세한 내용은 살펴보지 않았습니다. 우리는 당신이 너무 일찍 콘솔에 당신을 잃지 않고 먼저 기본 개념과 문제를 이해 하기를 원했습니다. 또한 실제 구성 작업이 잘 문서화되어 있습니다. 기본 개념을 이해한다면 다양한 스위치가 사용되는 용도를 알고 있어야 합니다.
...
AEM Author 및 Publish를 즉시 설치하는 경우 토폴로지가 약간 이상합니다. 작성자는 게시 시스템에 콘텐츠를 전송하는 동시에 Dispatcher에 무효화 요청을 전송합니다. 게시 시스템과 Dispatcher는 모두 큐에 의해 작성자로부터 분리되므로 타이밍이 약간 유감스러울 수 있습니다. Dispatcher는 게시 시스템에서 콘텐츠가 업데이트되기 전에 작성자로부터 무효화 요청을 수신할 수 있습니다.

그동안 클라이언트가 해당 콘텐츠를 요청하면 Dispatcher가 오래된 콘텐츠를 요청하고 저장합니다.

보다 신뢰할 수 있는 설정은 Publish 시스템에서 무효화 요청을 보냅니다. _이후_ 그들은 콘텐츠를 받았습니다. 문서 &quot;[게시 인스턴스에서 Dispatcher 캐시 무효화](https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html#InvalidatingDispatcherCachefromaPublishingInstance)&quot;&quot;는 세부 정보를 설명합니다.
...
옛날에는 Dispatcher가 파일 시스템에 일반 파일을 저장하고 있었습니다. 고객에게 전달할 HTTP 헤더가 필요한 경우 파일 또는 위치에서 얻은 작은 정보를 기반으로 Apache를 구성하여 이 작업을 수행했습니다. 이는 HTTP 헤더에 크게 의존하는 AEM에서 웹 애플리케이션을 구현할 때 특히 문제가 되었습니다. AEM 전용 인스턴스에서는 모든 것이 제대로 작동했지만 Dispatcher를 사용했을 때는 작동하지 않았습니다.

일반적으로 Apache 서버의 리소스에 누락된 헤더를 다시 적용하기 시작했습니다. `mod_headers` 정보를 사용하면 리소스 경로 및 접미사를 통해 파생될 수 있습니다. 그러나 그것이 항상 충분하지는 않았다.

특히 짜증나는 것은 디스패처를 처음 사용하는 경우에도 마찬가지였다 _캐시되지 않음_ 브라우저에 대한 응답은 전체 범위의 헤더가 있는 Publish 시스템에서 온 반면, 후속 응답은 제한된 헤더 세트로 Dispatcher에 의해 생성되었습니다.

Dispatcher 4.1.11 이상에서 Dispatcher는 게시 시스템에서 생성된 헤더를 저장할 수 있습니다.

이렇게 하면 Dispatcher에서 헤더 논리를 복제하지 않아도 되고 HTTP 및 AEM의 전체 표현력이 해제됩니다.
...
모든 페이지와 이미지를 일반적으로 캐시할 수 있지만, 경우에 따라 예외를 만듭니다. 예를 들어 PNG 이미지는 캐시하지만 CAPTCHA(각 요청에서 변경된다고 가정함)를 표시하는 PNG 이미지는 캐시하지 않으려고 합니다. Dispatcher가 captcha를 captcha로 인식하지 못할 수 있지만 AEM에서는 확실히 인식합니다. Dispatcher에 응답과 함께 헤더에 따라 을 전송하여 해당 요청을 캐시하지 않도록 요청할 수 있습니다.
...
Cache-Control 및 Pragma는 CDN과 같은 상위 캐싱 계층에 전파되고 해석되는 공식 HTTP 헤더입니다. 다음 `Dispatcher` 헤더는 Dispatcher가 캐시하지 않도록 하는 힌트일 뿐입니다. 상위 캐싱 계층에서 캐시하도록 허용하면서 Dispatcher에 캐시하지 않도록 하는 데 사용할 수 있습니다. 사실, 그것이 유용할지도 모르는 사례를 찾는 것은 어렵습니다. 하지만 우리는 어딘가에는 몇 가지가 있다고 확신한다.
...
일반적으로 Apache의 `mod_expires` 또는 더 많은 개별 제어가 필요한 경우 AEM에서 제공되는 Cache-Control 및 Expires 헤더를 저장하여 사용할 수 있습니다.

브라우저에서 캐시된 문서는 세 가지 수준의 최신 상태를 가질 수 있습니다.

1. _보장된 최신_ - 브라우저에서 캐시된 문서를 사용할 수 있습니다.

2. _잠재적으로 부실_ - 브라우저는 먼저 캐시된 문서가 최신 상태인지 서버에 물어야 합니다.
...
브라우저 캐시의 문서가 새 요청에 전달될 문서와 같은지 확인하려면 브라우저에서 `Last-Modified` 문서 날짜. 브라우저가 서버에 다음과 같은 질문을 합니다.

&quot;_6월 10일 버전이 있는데... 업데이트가 필요한가요?_&quot;서버가 다음으로 응답할 수 있습니다.

&quot;_304 - 버전이 아직 최신 상태입니다._&quot; 리소스를 다시 전송하지 않으면 서버가

&quot;_200 - 최신 버전_ HTTP 헤더 및 HTTP 본문의 실제 최신 콘텐츠

두 번째 부분이 작동하도록 하려면 `Last-Modified` 업데이트하도록 요청하는 참조 지점이 있도록 브라우저에 대한 날짜입니다.

앞에서 설명했듯이 `Last-Modified` 날짜는 Dispatcher에 의해 생성되며 브라우저에 의해 파일이 요청될 때 캐시된 파일( 및 날짜입니다 )이 생성되므로 요청마다 달라질 수 있습니다. 대안은 &quot;e-tags&quot;를 사용하는 것입니다. 이 숫자는 날짜 대신 실제 콘텐츠를 식별하는 숫자입니다(예: 해시 코드를 생성하여).

&quot;[Etag 지원](https://adobe-consulting-services.github.io/acs-aem-commons/features/etag/index.html)의 &quot; _ACS Commons 패키지_ 은 이 접근 방식을 사용합니다. 그러나 가격과 함께 제공됩니다. E-Tag는 헤더로 전송해야 하지만 해시 코드 계산에서는 응답을 완전히 읽어야 하므로 응답이 배달되기 전에 기본 메모리에서 완전히 버퍼링되어야 합니다. 이 경우 웹 사이트에 캐시되지 않은 리소스가 더 있을 수 있고 AEM 시스템에서 사용하는 메모리를 주시해야 하는 경우 지연에 부정적인 영향을 미칠 수 있습니다.

URL 지문을 사용하는 경우 매우 긴 만료 날짜를 설정할 수 있습니다. 브라우저에서 지문 리소스를 영원히 캐시할 수 있습니다. 새 버전이 새 URL로 표시되어 이전 버전을 업데이트할 필요가 없습니다.

스풀러 패턴을 소개할 때 URL 지문을 활용했습니다. 에서 들어오는 정적 파일 `/etc/design` (CSS, JS)는 거의 변경되지 않으며, 지문으로 사용하기에 적합한 후보입니다.
...
AEM UI를 구성하는 Granite 라이브러리는 상당히 오랜 시간 동안 캐시할 수 있습니다. 브라우저에서 사이트의 정적 파일(글꼴, CSS 및 JavaScript)을 캐시할 수도 있습니다. 의 짝수 이미지 `/content/dam` 일반적으로 페이지의 복사 텍스트만큼 자주 변경되지 않으므로 약 15분 동안 캐시될 수 있습니다. 이미지는 AEM에서 대화식으로 편집되지 않습니다. 이러한 구성 요소는 AEM에 업로드되기 전에 먼저 편집되고 승인됩니다. 따라서 텍스트가 텍스트만큼 자주 변경되지 않는다고 가정할 수 있습니다.
...
그러나 AEM에만 의존하지 마십시오. 그럴 경우 다음과 같은 경로가 있습니다. `/home.html` 캐시의 루트 디렉터리에 있습니다. 이제, 그것이 피니시 또는 독일어 또는 캐나다 웹사이트의 &quot;홈&quot;입니까? 파일이 있는 경우 `/home.html` Dispatcher에서에 대한 무효화 요청이 있을 때 Dispatcher가 이를 무효화해야 한다는 것을 어떻게 알 수 있습니까? `/content/brand/fi/fi/home` 들어와요.

각 도메인에 대해 별도의 docroot가 있는 프로젝트를 보았습니다. 디버깅하고 유지하는 것은 악몽이었고 - 실제로 우리는 그것이 완벽하게 실행되는 것을 보지 못했습니다.

우리는 캐시를 재구성하여 문제를 해결할 수 있다. 모든 도메인에 대해 단일 docroot가 있었으며 서버의 모든 파일이 로 시작될 때 무효화 요청이 1:1로 처리될 수 있었습니다 `/content`.

자르는 부분도 아주 쉬웠어요.  의 해당 구성으로 인해 AEM이 잘린 링크를 생성함 `/etc/map`.

이제 요청이 있을 때 `/home.html` 는 Dispatcher에 도달하고 있으며 가장 먼저 경로를 내부적으로 확장하는 재작성 규칙을 적용합니다.
...
파일 시스템에는 이제 일반 파일이 있습니다. `/content`- 작성자 및 게시에서도 찾을 수 있는 기반 경로 - 이는 디버깅에 많은 도움이 되었습니다. 올바른 무효화는 말할 것도 없고, 더 이상 문제가 되지 않았습니다.
...
하나의 공통 docroot가 갖는 또 다른 좋은 기능을 가지고있다. Dispatcher에서 문제가 발생하면 을 실행하여 전체 캐시를 정리할 수 있습니다.
...
예 - 하지만 이것은 AEM 부분만 해당됩니다. 기억하십시오. Dispatcher가 캐싱하지 않습니다. `404 – not found` 또는 `500 – internal server error` 응답.
...
물론 이것은 고객이 본 것이 아닙니다. Dispatcher에서 등록했습니다. `ErrorDocuments` 다음과 같이:
...
이제 AEM 시스템은 Dispatcher에 문제가 있음을 알리고 Dispatcher가 반짝이고 멋진 버전의 오류 문서를 제공할 수 있습니다.

여기에 두 가지 사항이 명시되어야 합니다.

첫 번째, `error-404.html` 는 항상 동일한 페이지입니다. 따라서 &quot;다음에 대한 검색&quot;과 같은 개별 메시지는 없습니다._프로덕텐_&quot;결과를 생성하지 않았습니다.&quot; 우리는 쉽게 그것을 받아들일 수 있습니다.

둘째... 내부 서버 오류가 표시되거나 AEM 시스템이 중단되는 경우 AEM에 오류 페이지를 렌더링하도록 요청할 방법이 없습니다. 에 정의된 대로 필요한 후속 요청 `ErrorDocument` 지시문도 실패할 것입니다. 를 통해 정의된 위치에서 오류 페이지를 정기적으로 가져오는 크론 작업을 실행하여 이 문제를 해결했습니다. `wget` 및에 정의된 정적 파일 위치에 저장합니다. `ErrorDocuments` 지시문입니다.
...
Dispatcher는 기본적으로 리소스를 전달할 때 권한을 확인하지 않습니다. 이 기능은 일부러 이렇게 구현됩니다. 공개 웹 사이트를 빠르게 하기 위해서입니다. 로그인하여 일부 리소스를 보호하려면 기본적으로 세 가지 옵션이 있습니다.

1. Protect은 요청이 캐시에 도달하기 전에 리소스를 호출합니다(예: Dispatcher 앞의 SSO(Single Sign-On) 게이트웨이에 의해 또는 Apache 서버의 모듈로)
...
>[!NOTE]
>
>이 패턴에는 다음이 필요합니다. _게이트웨이_ that _가로채기_ 각 요청 및 실제 작업 수행 _authorization_ - Dispatcher에 대한 요청 부여 또는 거부. SSO 시스템이 _인증자_&#x200B;를 설정하는 경우 옵션 3을 구현해야 하는 사용자의 ID만 설정됩니다. SSO 시스템의 안내서에서 &quot;SAML&quot; 또는 &quot;OAauth&quot;와 같은 용어를 읽는 경우 옵션 3을 구현해야 한다는 강력한 표시기입니다.
...
**옵션 3**. &quot;권한 구분 캐싱&quot;은 흥미로운 접근 방식입니다. Dispatcher가 리소스를 캐시하고 있지만 전달하기 전에 AEM 시스템에 캐시할 수 있는지 여부를 묻습니다. 이렇게 하면 Dispatcher에서 게시로 추가 요청이 작성되지만 일반적으로 페이지가 이미 캐시된 경우 게시 시스템이 페이지를 다시 렌더링하지 않습니다. 그러나 이 접근 방식에서는 몇 가지 사용자 지정 구현이 필요합니다. 문서에서 세부 정보 찾기 [권한 구분 캐싱](https://helpx.adobe.com/experience-manager/dispatcher/using/permissions-cache.html).
...
![자주 활성화하면 대부분의 경우 캐시가 잘못됨](assets/chapter-1/frequent-activations.png)

*자주 활성화하면 대부분의 경우 캐시가 잘못됨*

<br> 

이 &quot;캐시 무효화 폭풍&quot;의 문제를 완화하기 위해 간혹 호출될 때 이 문제에 대해 덜 엄격할 수 있습니다. `statfile` 해석.

를 사용하도록 Dispatcher를 설정할 수 있습니다 `grace period` 자동 무효화. 이렇게 하면 내부적으로 `statfiles` 수정일.

예를 들어, `statfile` 에는 오늘 12:00의 수정 시간이 있으며 `gracePeriod` 가 2분으로 설정되어 있습니다. 그런 다음 자동 무효화된 모든 파일은 12:01 및 12:02에 유효한 것으로 간주됩니다. 12:02 이후에 다시 렌더링됩니다.

참조 구성은 `gracePeriod` 2분 동안이요, 좋은 이유로요. &quot;2분? 거의 아무것도 아니에요. 내용이 나오기까지 10분 정도 기다릴 수 있습니다...  따라서 더 긴 기간을 설정하고 싶을 수 있습니다. 최소한 이 10분 후에 콘텐츠가 나타난다고 가정해 보겠습니다.

>[!WARNING]
>
>이렇게 하는 것이 아닙니다 `gracePeriod` 은(는) 작동합니다. 유예 기간은 다음과 같습니다 _아님_ 문서의 무효화가 보장되지만 무효화가 발생하지 않는 시간. 이 프레임 내에 있는 후속 무효화 각각 _장수_ 시간대 - 무한정 길 수 있습니다.

방법을 설명해 보겠습니다. `gracePeriod` 실제로 은 예제를 사용하여 작동합니다.
...
그리고 그 내용은 무효화되지 않습니다. 각 무효화 *다음 범위 내* gracePeriod는 유예 시간을 효과적으로 연장합니다. 다음 `gracePeriod` 무효화 폭풍을 막아줄 계획이지만 결국엔 비를 맞아야 해 `gracePeriod` 은신처에 영원히 숨는 것을 막기에는 상당히 짧다.

#### 결정론적 유예 기간

무효화 폭풍을 어떻게 견뎌낼 수 있는지 다른 아이디어를 소개하고자 합니다. 그것은 단지 생각일 뿐이다. 프로덕션에서 시도한 적은 없지만, 아이디어를 공유하기에 충분히 흥미로운 컨셉을 찾았습니다.

다음 `gracePeriod` 는 정규 복제 간격이 보다 짧은 경우 예상할 수 없을 정도로 길 수 있습니다. `gracePeriod`.

대체 아이디어는 다음과 같습니다. 고정된 시간 간격에서만 무효화하십시오. 사이의 시간은 항상 오래된 콘텐츠를 제공하는 것을 의미합니다. 무효화는 결국 발생하지만, 여러 무효화가 하나의 &quot;일괄&quot; 무효화에 수집되므로 Dispatcher는 그동안 캐시된 일부 콘텐츠를 제공하고 Publish 시스템에 숨 쉴 수 있는 공기를 제공할 수 있습니다.

구현은 다음과 같습니다.

무효화가 발생한 후에 실행되는 &quot;사용자 지정 무효화 스크립트&quot;(참조)를 사용합니다. 이 스크립트는 `statfile's` 마지막 수정 날짜 및 다음 간격 중지로 반올림합니다. Unix shell 명령 `touch --time`시간을 지정하겠습니다.

예를 들어 유예 기간을 30초로 설정하면 Dispatcher가 상태 파일의 마지막 수정 날짜를 다음 30초로 반올림합니다. 사이에 발생하는 무효화 요청은 다음 전체 30초를 동일하게 설정합니다.
...
무효화 요청과 다음 라운드 30초 슬롯 사이에 발생하는 캐시 히트는 오래된 것으로 간주됩니다. 게시에는 업데이트가 있지만 Dispatcher는 여전히 이전 콘텐츠를 제공합니다.
...
임의 무효화 요청이 Dispatcher로 전송되어 가장 인기 있는 한 페이지를 포함한 모든 페이지가 무효화됩니다.
...
자동 다시 가져오기를 사용하면 이를 어느 정도 완화할 수 있습니다. 대부분의 무효화된 페이지는 자동 무효화 이후에도 여전히 물리적으로 Dispatcher에 저장됩니다. 그것들은 단지 _고려됨_ 오래됨. _자동 재에칭_ 는 시작하는 동안 이러한 부실 페이지를 몇 초 동안 계속 제공함을 의미합니다 _싱글_ 게시 시스템에 오래된 콘텐츠를 다시 가져오도록 요청:
...
다시 가져오기를 활성화하려면 Dispatcher에 자동 무효화 이후 다시 가져올 리소스를 알려주어야 합니다. 활성화한 모든 페이지는 인기 있는 페이지를 포함하여 다른 모든 페이지를 자동 무효화한다는 것을 기억하십시오.

다시 가져오기는 실제로 Dispatcher에 각 (!)을 알려 줍니다. 가장 방문 빈도가 높은 항목과 가장 방문 빈도가 높은 항목을 다시 가져오려는 무효화 요청.
...
Dispatcher가 이러한 요청을 볼 때 평소와 같이 자동 무효화를 트리거하고 게시 시스템에서 새로운 콘텐츠를 다시 가져오는 요청을 즉시 대기열에 넣습니다.

이제 요청 본문을 사용하므로 HTTP 표준에 따라 콘텐츠 유형 및 콘텐츠 길이도 설정해야 합니다.

Dispatcher는 또한 자동 무효화로 인해 이러한 리소스가 유효하지 않은 것으로 간주되더라도 직접 전달할 수 있음을 알 수 있도록 내부적으로 URL에 따라 표시합니다.
...
Dispatcher는 유지 관리 목적으로만 사용되는 요청에서 게시 시스템을 차단하여 약간의 추가 보안을 제공합니다. 예를 들어, 당신은 당신을 노출하고 싶지 않습니다 `/crx/de` 또는 `/system/console` URL을 공개합니다.
...
자주 표시되는 것은 Dispatcher 구성에서 더 취약한 리소스에 대한 액세스를 방지하는 Apache 재작성 규칙 세트입니다.

그러나 다른 접근 방식을 고려할 수도 있습니다.

Dispatcher 구성에 따라 Dispatcher 모듈은 특정 디렉터리에 바인딩됩니다.
...
우선 처리기의 바인딩 범위를 좁힐 수 있습니다. `SetHandler` 핸들러를 디렉터리에 바인딩하기만 하면 핸들러를 URL 또는 URL 패턴에 바인딩할 수 있습니다.
...
이렇게 하는 경우 항상 Dispatcher-handler를 Dispatcher의 무효화 URL에 바인딩해야 합니다. 이렇게 하지 않으면 AEM에서 Dispatcher로 무효화 요청을 보낼 수 없습니다.

Dispatcher를 필터로 사용하는 또 다른 대안은 `dispatcher.any`
...
다행히도 이 변경 사항은 이후 버전의 Dispatcher에서 변경되었습니다. 이제 POSIX 정규 표현식도 사용할 수 있으며 요청의 다양한 부분에 액세스하여 필터를 정의할 수 있습니다. Dispatcher로 방금 시작한 사용자를 당연시할 수 있습니다. 하지만 여러분이 글로브만 가지고 있다면, 그것은 놀라운 일이고 쉽게 간과될 수 있습니다. globs와 regexes의 구문 외에도 너무 비슷하다. 동일한 작업을 수행하는 두 버전을 비교해 보겠습니다.
...
버전 B는 작은 따옴표를 사용합니다. `'` 을(를) 표시하려면 _정규 표현식 패턴_. &quot;모든 문자&quot;는 `.*`.

_글로빙 패턴_, 반대로 큰따옴표 사용 `"` 다음과 같은 간단한 자리 표시자만 사용할 수 있습니다. `*`.

이러한 차이점을 알고 있다면 사소한 것이지만, 그렇지 않은 경우 구성 디버깅 시 따옴표를 쉽게 혼동하고 화창한 오후를 보낼 수 있습니다. 이제 경고 받으세요.

&quot;I recogny `'/url'` 구성에서... 하지만 그게 뭔데요 `'/glob'` 필터에서 질문할 수 있습니까?

해당 지시문은 메서드 및 경로를 포함하여 전체 요청 문자열을 나타냅니다. 그것은 유효할 수 있다

`"GET /content/foo/bar.html HTTP/1.1"`

패턴을 비교할 문자열입니다. 초보자는 첫 번째 부분인 을 잊어버리는 경향이 있습니다. `method` (GET, POST, ...) 그래서, 패턴이
...
&quot;줄 번호&quot;에 대한 마지막 단어 하나 `/005` 각 정의 앞에
...
>[!WARNING]
>
>보다 정교한 설정은 필터링 규칙을 main에 포함된 여러 파일로 분할합니다 `dispatcher.any` 구성 파일입니다. 그러나 새 파일에는 새 네임스페이스가 도입되지 않습니다. 따라서 한 파일에 규칙 &quot;001&quot;이 있고 다른 파일에 규칙 &quot;001&quot;이 있는 경우 오류가 발생합니다. 의미상 강력한 이름을 생각해 내야 하는 이유가 더 있다.
...
대부분의 경우 AEM 및 Dispatcher는 즉시 작동합니다. 따라서 최상위 애플리케이션 빌드를 위한 무효화 프로토콜에 대한 포괄적인 Dispatcher 프로토콜 사양은 찾지 못할 것입니다. 그 정보는 공개적이지만, 약간 많은 자원에 흩어져 있습니다.
...
`POST /dispatcher/invalidate.cache HTTP/1.1` - 첫 번째 줄은 Dispatcher 제어 끝점의 URL이며 변경되지 않을 수 있습니다.

`CQ-Action: <action>` - 어떻게 해야 합니까? `<action>` 다음 중 하나입니다.
...
`CQ-Handle: <path-pattern>` - 무효화할 콘텐츠-리소스 경로. 참고, `<path-pattern>` 는 실제로 &quot;경로&quot;이지 &quot;패턴&quot;이 아닙니다.

`CQ-Action-Scope: ResourceOnly` - 선택 사항: 이 헤더가 설정되면 `.stat` 파일이 터치되지 않았습니다.
...
자동 재가져오기 URL 목록을 정의하는 경우 이러한 헤더를 설정합니다. `<bytes in request body>` 는 HTTP 본문의 문자 수입니다
...
모든 지시문이 포함된 Dispatcher 설명서: [https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html](https://helpx.adobe.com/kr/experience-manager/dispatcher/using/dispatcher-configuration.html)
...
프레젠테이션 &quot;콘텐츠 무효화의 저평가된 능력&quot;, &quot;adaptTo()&quot; 컨퍼런스(Potsdam 2018) [https://adapt.to/2018/en/schedule/the-underappreciated-power-of-content-invalidation.html](https://adapt.to/2018/en/schedule/the-underappreciated-power-of-content-invalidation.html)
