...
프록시 서버를 추가하면 일반적으로 성능이 향상됩니다. 그러나 실제로 서버를 추가하면 성능이 저하될 수 있는 시나리오가 있습니다. 어떻게? 매분마다 새로운 기사와 페이지를 소개하는 뉴스 포털이 있다고 가정해 보겠습니다. Dispatcher은 &quot;자동 무효화&quot;에 의해 무효화됩니다. 페이지가 게시될 때마다 동일한 사이트의 캐시에 있는 모든 페이지가 무효화됩니다. 이는 유용한 기능입니다. 이 시리즈의 [챕터 1](chapter-1.md)에서 다루었지만 웹 사이트에서 변경 내용이 자주 있으면 캐시를 무효화하는 경우가 많습니다. Publish 인스턴스당 하나의 Dispatcher만 있는 경우 페이지를 요청하는 첫 번째 방문자가 해당 페이지의 재캐싱을 트리거합니다. 두 번째 방문자는 이미 캐시된 버전을 가져옵니다.
