...
>[!VIDEO](https://video.tv.adobe.com/v/3447439?captions=kor&quality=12&learn=on)
...
**위젯**&#x200B;은(는) **확장 포인트를 사용하여 콘텐츠 조각 편집기의** RTE`rte`에 추가됩니다. `rte` 확장 지점의 `getWidgets()` 메서드를 사용하면 하나 이상의 위젯이 추가됩니다. `{` 특수 키를 눌러 컨텍스트 메뉴 옵션을 연 다음 원하는 위젯을 선택하여 사용자 지정 대화 상자 UI를 로드함으로써 트리거됩니다.
...
+ `getWidgets()` 특성이 있는 `id, label and url`의 위젯 정의 함수입니다.
+ 대화 상자 UI를 로드할 상대 URL 경로(`url`)인 `/index.html#/discountCodes` 특성 값입니다.
...
### `discountCodes`에 `App.js` 경로 추가{#add-widgets-route}
...
+ `addDiscountCode` 개체를 사용하는 `guestConnection` 함수는 실행할 RTE 명령을 제공합니다. 이 경우 RTE에 삽입할 실제 할인 코드의 `insertContent` 명령 및 HTML 코드 조각입니다.
