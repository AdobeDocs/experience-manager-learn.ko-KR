...
1. AEM은 컨텐츠 조각에 대한 속성 업데이트를 as a Cloud Service으로 유지하고 Adobe I/O Runtime 작업에 대한 실패 응답의 성공을 반환합니다.
...
1. 두 번째 경로 세트는 URL을 확장의 모달 내용을 렌더링하는 React 구성 요소에 매핑합니다. 다음 `:selection` 매개 변수는 구분된 목록 컨텐츠 조각 경로를 나타냅니다.
...
벌크 속성 업데이트 양식을 제출하면 사용자 지정 `onSubmitHandler()` 현재 AEM 호스트(도메인)와 사용자의 AEM 액세스 토큰을 전달하여 Adobe I/O Runtime 작업을 호출하고, 이 토큰을 호출하면 [AEM 컨텐츠 조각 API](https://experienceleague.adobe.com/docs/experience-manager-65/developing/platform/content-fragments-api.html) 컨텐츠 조각을 업데이트하려면
...
AEM 확장 앱 빌더 앱은 0개 또는 여러 Adobe I/O Runtime 작업을 정의하거나 사용자 지정할 수 있습니다.
