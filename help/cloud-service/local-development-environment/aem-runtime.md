...
1. [최신 Java™ JDK 21 다운로드 및 설치](https://experience.adobe.com/#/downloads/content/software-distribution/en/general.html?fulltext=java*+21*&orderby=%40jcr%3Acontent%2Fjcr%3AlastModified&orderby.sort=desc&layout=list&p.offset=0&p.limit=11)
1. 다음 명령을 실행하여 Oracle Java™ 21 JDK가 설치되어 있는지 확인합니다.
...
>[!CAUTION]
>
>이전 버전의 AEM SDK을 실행하는 경우 [Java 11 JDK를 다운로드](https://experience.adobe.com/#/downloads/content/software-distribution/en/general.html?fulltext=java*+11*&orderby=%40jcr%3Acontent%2Fjcr%3AlastModified&orderby.sort=desc&layout=list&p.offset=0&p.limit=11)해야 할 수 있습니다. 그러나 최신 버전의 AEM SDK을 사용하는 것이 좋습니다.
...
1. 명령줄에서 다음을 실행하여 로컬 AEM 작성자 서비스를 시작합니다.
   ...
   *두 번 클릭*&#x200B;하여 AEM을 Cloud Service Quickstart Jar [로 시작할 수](#troubleshooting-double-click)없습니다.
1. 웹 브라우저의 [http://localhost:4502](http://localhost:4502)에서 로컬 AEM 작성자 서비스에 액세스합니다. 처음 시작할 때 설치가 완료될 때까지 몇 분 정도 기다려야 할 수 있습니다. 일반적으로 브라우저 탭이 자동으로 열립니다.
...
1. 다음 명령을 사용하여 AEM 프로젝트 또는 샘플 [AEM WKND Sites 프로젝트](https://github.com/adobe/aem-guides-wknd)를 복제하고 로컬 AEM 런타임에 배포할 수 있습니다.
...
1. `http://localhost:4503/<your-page-path>.html`에서 게시된 콘텐츠를 볼 수 있습니다. 일반적으로 게시된 콘텐츠를 보기 위해 Publish 서비스에 로그인할 필요가 없습니다. 그러나 문제가 발생하거나 로그, 구성 등을 검토하는 경우 [http://localhost:4503/libs/granite/core/content/login.html](http://localhost:4503/libs/granite/core/content/login.html)에서 Publish 서비스에 로그인할 수 있습니다.
...
이것은 AEM as a Cloud Service에 Java™ JDK 21이 필요하며 Java™ 11 또는 8과 같은 다른 버전을 실행 중이기 때문입니다. 이 문제를 해결하려면 [Oracle Java™ JDK 21](https://experience.adobe.com/#/downloads/content/software-distribution/en/general.html?fulltext=Java*+21*&orderby=%40jcr%3Acontent%2Fjcr%3AlastModified&orderby.sort=desc&layout=list&p.offset=0&p.limit=11)을(를) 다운로드하여 설치하십시오.

Oracle Java™ 21 JDK가 설치되면 명령줄에서 명령을 실행하여 활성 버전인지 확인합니다.
