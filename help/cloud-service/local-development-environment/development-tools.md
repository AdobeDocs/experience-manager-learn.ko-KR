...
Homebrew를 사용하는 경우 아래 섹션에 있는 __Install using Homebrew__ 지침을 따르십시오. Homebrew를 사용하고 있지 않은 __인 경우 OS별 링크를 사용하여 도구를 설치하십시오.__

## Git 설치

[](https://git-scm.com/) Adobe  [Cloud Manager에서 사용하는 소스 제어 관리 시스템](https://docs.adobe.com/content/help/en/experience-manager-cloud-manager/using/requirements/source-code-repository.html)으로, 개발을 위해 필요합니다.
...
[Node.](https://nodejs.org) jsis a JavaScript runtime environment used to work with the front-end assets of an AEM project&#39;s  __ui.frontendsub-project__ . Node.js는 JavaScript 종속성을 관리하는 데 사용되는 사실상의 Node.js 패키지 관리자인 [npm](https://www.npmjs.com/)과 함께 배포됩니다.
...
Apache Maven은 AEM Project Maven Tranype에서 생성된 AEM 프로젝트를 구축하는 데 사용되는 오픈 소스 Java 명령줄 도구입니다. 모든 주요 IDE의 ([IntelliJ IDEA](https://www.jetbrains.com/idea/), [Visual Studio Code](https://code.visualstudio.com/), [Eclipse](https://www.eclipse.org/) 등) Maven이 통합되어 있습니다.
...
[Adobe I/O CLI](https://github.com/adobe/aio-cli) 또는 `aio`는 [클라우드 관리자](https://github.com/adobe/aio-cli-plugin-cloudmanager) 및 [Asset compute](https://github.com/adobe/aio-cli-plugin-asset-compute)를 포함하여 다양한 Adobe 서비스에 대한 명령줄 액세스를 제공합니다. Adobe I/O CLI는 개발자에게 다음과 같은 기능을 제공하는 Cloud Service으로 AEM에서 개발 시 중요한 역할을 합니다.
...
1. Adobe I/O CLI가 npm 모듈이므로 [Node.js가](#node-js)로 설치되었는지 확인합니다.
   + `node --version`을(를) 실행하여 확인
1. `npm install -g @adobe/aio-cli`을(를) 실행하여 전역 `aio` npm 모듈을 설치합니다.

### Adobe I/O CLI Cloud Manager 플러그인 설정{#aio-cloud-manager}

Creative Cloud Manager 플러그인을 사용하면 aio CLI가 `aio cloudmanager` 명령을 통해 Adobe Cloud Manager와 상호 작용할 수 있습니다.
...
Creative Cloud Manager 플러그인을 사용하면 aio CLI에서 `aio asset-compute` 명령을 통해 Asset compute 작업자를 생성하고 실행할 수 있습니다.
...
Adobe I/O CLI가 Cloud Manager와 통신하려면 Cloud Manager 통합이 Adobe I/O 콘솔에서 생성되어야 하며 인증이 성공적으로 이루어지도록 자격 증명을 얻어야 합니다.
...
1. 새로 만들거나 기존 [Adobe I/O 프로그램](https://www.adobe.io/apis/experienceplatform/console/docs.html#!AdobeDocs/adobeio-console/master/projects.md) 열기
   + Adobe I/O 콘솔 프로그램은 통합 관리 방법에 따라 통합, 생성 또는 사용 및 기존 프로그램의 구성 그룹화합니다.
...
1. `config.json` 파일을 Adobe I/O CLI로 로드
   + `$ aio config:set jwt-auth PATH_TO_CONFIG_JSON_FILE --file --json`
1. `private.key` 파일을 Adobe I/O CLI로 로드
...
__[Visual Studio 코드](https://code.visualstudio.com/)__ (VS 코드)는 프런트 엔드 개발자를 위한 무료 오픈 소스 도구입니다. Visual Studio 코드는 Adobe 도구 __[repo](https://github.com/Adobe-Marketing-Cloud/tools/tree/master/repo#integration-into-visual-studio-code)__&#x200B;의 도움으로 AEM과 컨텐츠 동기화를 통합하도록 설정할 수 있습니다.

Visual Studio 코드는 프런트 엔드 개발자에게 주로 프런트 엔드 코드를 생성하는 데 이상적인 제품입니다.JavaScript, CSS 및 HTML VS 코드에는 [extensions](https://code.visualstudio.com/docs/java/java-tutorial)을 통해 Java가 지원되지만, 더 많은 Java에 따라 제공되는 일부 고급 기능이 없을 수 있습니다.
...
__[Eclipse ](https://www.eclipse.org/ide/)__ IDE는 Java 개발을 위해 널리 사용되는 IDE로, Adobe에서 제공하는   __[AEM Developer ](https://eclipse.adobe.com/aem/dev-tools/)__ Toolsplug를 지원하며 저작에 IDE 내 GUI를 제공하고 로컬 AEM 인스턴스와 JCR 컨텐츠를 동기화할 수 있습니다.
