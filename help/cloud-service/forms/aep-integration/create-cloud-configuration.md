---
title: AEP 및 AEM 클라우드 구성 만들기
description: AEM Forms에서 클라우드 구성을 만드는 방법을 알아보고 Adobe Developer Console 자격 증명을 사용하여 Adobe Experience Platform과 안전하게 연결합니다.
version: Experience Manager as a Cloud Service
feature: Adaptive Forms
topic: Integrations
role: Admin
level: Intermediate
doc-type: Technical Video
duration: 81
last-substantial-update: 2026-03-16T00:00:00Z
jira: KT-20539
source-git-commit: 4c5bec95d74d3611dc96d1d23dcd4d3d708e0243
workflow-type: tm+mt
source-wordcount: '85'
ht-degree: 2%

---


# AEP 및 AEM 통합을 위한 클라우드 구성 만들기

AEM Forms에서 Adobe Experience Platform과 안전하게 통합되는 클라우드 구성을 설정하는 방법에 대해 알아봅니다. Adobe Developer Console 자격 증명을 사용하여 양식에 대한 데이터를 원활하게 제출하고 검색할 수 있습니다. 이 안내서에서는 연결을 효율적으로 구성하는 단계별 지침을 제공합니다.

>[!VIDEO](https://video.tv.adobe.com/v/3481496/?learn=on&enablevpops)


## 다음 단계

[양식 만들기 및 AEP 제출](./create-integration-with-aep.md)
