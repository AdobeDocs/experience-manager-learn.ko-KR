...
AEM as a Cloud Services은 사용자 정의 로그 파일을 지원하지 않지만 사용자 정의 로깅을 지원합니다.
...
AEM Publish Dispatcher만 Apache 웹 서버 및 Dispatcher 로그를 제공합니다. 이러한 측면은 AEM 게시 계층에만 있고 AEM 작성자 계층에는 없으므로 AEM Publish 계층에는 없습니다.
...
+ 환경 변수는 를 통해서만 프로그래밍 방식으로 관리할 수 있습니다. [ADOBE I/O CLI](https://github.com/adobe/aio-cli-plugin-cloudmanager#aio-cloudmanagerset-environment-variables-environmentid) 또는 [Cloud Manager API](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/deploying/configuring-osgi.html#cloud-manager-api-format-for-setting-properties).
