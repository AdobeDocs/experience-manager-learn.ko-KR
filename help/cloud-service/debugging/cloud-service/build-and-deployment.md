...
+ __원인:__ 파이프라인의 대상 환경은 전환 상태에 있으며 이 상태에서 새 빌드를 수락할 수 없습니다.
+ __해결 방법:__ 상태가 실행(또는 업데이트 사용 가능) 상태로 해결될 때까지 기다립니다. 환경이 삭제되는 경우 환경을 다시 만들거나 다른 환경을 선택하여 구축할 수 있습니다.
...
+ __오류 메시지:__ 잘못된 파이프라인:XXXXXX. reason=Branch=xxxx가 저장소에 없습니다.
   ![잘못된 파이프라인:XXXXXX. 이유=분기=xxxx가 저장소에 없습니다.](./assets/build-and-deployment/validation__branch-not-found.png)
+ __원인:__ 파이프라인이 사용하도록 구성된 Git 분기가 삭제되었습니다.
+ __해결 방법:__ 동일한 이름을 사용하여 누락된 Git 분기를 다시 만들거나 다른 기존 분기에서 빌드하도록 파이프라인을 다시 구성합니다.
...
+ Maven [Central에서 사용할 수 없는](https://search.maven.org/) 마비사 종속성이 사용되고 종속성이 포함된 Maven 저장소가 둘 중 하나입니다.
   + 비공개 내부 Maven 저장소 또는 Maven 리포지토리와 같이 Cloud Manager에서 연결할 수 없는 경우 인증이 필요하며 잘못된 자격 증명이 제공됩니다.
   + 프로젝트의 URL에 명시적으로 등록되어 있지 않습니다 `pom.xml`. 빌드 시간이 증가함에 따라 Maven 리포지토리를 비롯한 사용 안 함
+ 시간 문제로 인해 단위 테스트가 실패했습니다. 이것은 단위 테스트가 시간 민감할 때 발생할 수 있습니다. 강력한 표시기가 테스트 코드 `.sleep(..)` 에 의존하고 있습니다.
...
코드에 치명적인 보안 취약점이 있는 경우 코드 스캔을 수행하면 빌드 오류가 발생합니다. 가벼운 위반은 무시될 수 있지만, 이를 수정하는 것이 좋습니다. 코드 스캔은 불완전하며 [잘못된 양성으로 이어질 수 있습니다](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/developing/understand-test-results.html#dealing-with-false-positives).

코드 스캔 문제를 해결하려면 Cloud Manager에서 제공하는 CSV 형식의 보고서를 **다운로드 세부** 정보 버튼을 통해 다운로드하고 모든 항목을 검토하십시오.

자세한 내용은 AEM 관련 규칙을 참조하십시오. Cloud Manager 문서의 사용자 [정의 AEM 관련 코드 스캔 규칙을 참조하십시오](https://docs.adobe.com/content/help/en/experience-manager-cloud-manager/using/how-to-use/custom-code-quality-rules.html).
...
+ __원인:__ AEM 프로젝트의 모든 패키지에 여러 코드 패키지가 들어 있으며, 코드 패키지 중 두 개 이상에서 동일한 OSGi 구성이 제공되어 충돌이 발생하므로 이미지 빌드 단계에서 사용해야 하는 항목을 결정할 수 없어 빌드가 실패합니다. OSGi 공장 구성에는 고유한 이름이 있을 경우 적용되지 않습니다.
+ __해결 방법:__ AEM 응용 프로그램의 일부로 배포되는 모든 코드 패키지(포함된 타사 코드 패키지 포함)를 검토하여 실행 모드를 통해 대상 환경에서 해결되는 중복 OSGi 구성을 찾습니다. &quot;mergeConfigurations 플래그를 true로 설정&quot;에 대한 오류 메시지 지침은 AEM에서 클라우드 서비스로 사용할 수 없으므로 무시해야 합니다.

#### 원인 2

+ __원인:__ AEM 프로젝트에 동일한 코드 패키지가 두 번 잘못 포함되어 해당 패키지에 포함된 모든 OSGi 구성이 중복됩니다.
+ __해결 방법:__ 모든 프로젝트에 포함된 패키지의 모든 pom.xml 패키지를 검토하고 구성 `filevault-package-maven-plugin` 설정이 되어 있는지 [](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/developing/aem-project-content-package-structure.html#cloud-manager-target) `<cloudManagerTarget>none</cloudManagerTarget>`확인합니다.
...
+ __해결 방법:__ 참조 스크립트 OSGi 구성이 배포되면 AEM SDK의 로컬 빠른 시작을 검토하여 오류가 있는지 여부를 확인합니다.
...
+ __해결 방법:__ 참조 스크립트에 의존하는 컨텐츠가 있는지 확인합니다. 종종, 이러한 누락되지만 필수 컨텐츠 구조를 정의하는 디렉티브가 누락된 잘못 정의된 리포인트 스크립트를 나타냅니다. 이것은 AEM을 삭제하고, Jar를 압축 해제하고, 리포인트 스크립트가 포함된 리포인트 OSGi 구성을 설치 폴더에 추가하고, AEM을 시작하여 로컬로 재현할 수 있습니다. 이 오류는 AEM SDK 로컬 quickstart의 error.log에 나타납니다.
...
다음 경우 이미지 빌드 단계가 실패할 수 있습니다.

+ 배포 응용 프로그램이 (OSGi 번들) 프로젝트에서 핵심 구성 요소 `core` 종속 버전 버전을 업데이트합니다.
...
+ __원인:__ 프로젝트의 OSGi 번들(프로젝트에서 정의됨)은 `core` Cloud Service으로 AEM에 배포된 버전과 다른 버전 수준에서 핵심 구성 요소의 Java 클래스를 가져옵니다.
+ __해상도:__
...
배포 대상 단계를 디버깅하기 전에 [AEM을 Cloud Service 로그으로](./logs.md) 숙지하십시오. 로그에는 발행물에 배포와 관련이 있을 수 있는 창의 시작 및 종료에 대한 정보가 포함되어 있습니다. `aemerror` Cloud Manager의 배포 대상 단계에서 다운로드 로그 단추를 통해 사용할 수 있는 로그는 `aemerror` 로그가 아니며 응용 프로그램 시작과 관련된 자세한 정보는 포함하지 않습니다.
...
+ __원인:__ Cloud Manager 파이프라인은 대상 환경에 배포된 이전 버전보다 이전 버전의 AEM을 보유합니다. 이 문제는 파이프라인이 다시 사용되고 최신 버전의 AEM을 실행 중인 새 환경을 가리킬 때 발생할 수 있습니다. 환경의 AEM 버전이 파이프라인의 AEM 버전보다 높은지 확인하여 이 값을 식별할 수 있습니다.
...
+ __해결 방법:__ OSGi Bundle의 라이프사이클에서 초기에 실행되는 코드에 대한 구현을 검토하고, Cloud Manager에서 보듯이 실패 시간(GMT에 기록 시간)에 AEM 작성자 및 게시 서비스에 대한 `aemerror` 로그를 검토하고, 사용자 정의 로그 실행 프로세스를 나타내는 로그 메시지를 찾습니다.

### 호환되지 않는 코드 또는 구성

대부분의 코드 및 구성 위반은 빌드 초기에 발견되지만 사용자 지정 코드 또는 구성이 Cloud Service으로 AEM과 호환되지 않고 컨테이너에서 실행될 때까지 감지되지 않을 수 있습니다.

+ __원인:__ 사용자 지정 코드는 OSGi 번들 또는 구성 요소 라이프사이클에서 초기에 트리거된 큰 쿼리 또는 컨텐츠 트래픽과 같은 긴 작업을 호출할 수 있으며 AEM의 시작 시간이 크게 지연됩니다.
+ __해결 방법:__ Cloud Manager에서 보듯이 AEM 작성자 및 게시 서비스에 대한 `aemerror` 로그를 시간(GMT로 로그인)에 따라 검토합니다.
...
`/var` 을 변경할 수 있습니다. 컨텐츠 패키지 `/var` (예: `ui.content`) Cloud Manager를 통해 배포되면 배포 단계가 실패할 수 있습니다.
...
1. 배포에 포함된 하나 이상의 콘텐츠 패키지를 찾아 씁니다 `/var`.
...
+ __원인:__ AEM 게시 서비스에 콘텐츠 패키지를 배포하는 데 사용되는 AEM 복제 서비스 사용자는 AEM 게시 `/var` 에 쓸 수 없습니다. 이로 인해 콘텐츠 패키지를 AEM 게시 서비스에 배포하지 못합니다.
+ __해결 방법:__ 이 문제를 해결하기 위한 다음 방법은 기본 설정의 순서로 나열되어 있습니다.
   1. 리소스가 필요하지 않은 경우 `/var` 애플리케이션의 일부로 배포된 컨텐츠 패키지에서 `/var` 리소스를 제거합니다.
   2. 리소스가 `/var` 필요한 경우 리포인트를 사용하여 노드 구조를 [정의합니다](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/deploying/overview.html#repoinit). 참조 스크립트는 AEM 작성자, AEM 게시 또는 둘 모두를 OSGi 실행 모드를 통해 타깃팅할 수 있습니다.
   3. 리소스를 AEM 작성자만 `/var` 필요로 하고 리포인트를 사용하여 합리적으로 모델링할 수 [없는](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/deploying/overview.html#repoinit)경우 AEM 작성자 실행 모드 폴더( [)의 패키지에](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/developing/aem-project-content-package-structure.html#embeddeds) 포함시켜 AEM 작성자 `all` 에만 설치되는 개별 컨텐츠 패키지로`<target>/apps/example-packages/content/install.author</target>`이동합니다.
