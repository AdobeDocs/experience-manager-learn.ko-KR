...
- WKND Sites 프로젝트용 새 Cloud Manager Git 저장소를 생성하고 로컬 Git 저장소에 원격으로 추가합니다.
   ...
   - **저장소 이름**(예: &quot;wknd-site-tutorial&quot;)을 입력하고 **저장**&#x200B;을 클릭합니다. 저장소가 생성될 때까지 기다립니다.
     ...
     ![저장소 추가](./assets/development-agent-troubleshoot-ci-cd-pipeline/add-repository.png)

   - 오른쪽 상단의 **저장소 정보 액세스**&#x200B;를 클릭하고 저장소 URL을 복사합니다.

     ![저장소 정보 액세스](./assets/development-agent-troubleshoot-ci-cd-pipeline/access-repo-info.png)
...
- **비프로덕션 파이프라인 추가** 대화 상자에서 다음을 구성합니다.
   ...
   - **구성** 단계:
     ...
     ![비프로덕션 파이프라인 구성 추가](./assets/development-agent-troubleshoot-ci-cd-pipeline/add-non-production-pipeline-configuration.png)
   ...
   - **Source 코드** 단계:
     ...
     ![비프로덕션 파이프라인 Source 코드 추가](./assets/development-agent-troubleshoot-ci-cd-pipeline/add-non-production-pipeline-source-code.png)

- 파이프라인 항목의 세 점 메뉴에서 **실행**&#x200B;을 클릭하여 새로 만든 코드 품질 파이프라인을 실행합니다.

  ![코드 품질 파이프라인 실행](./assets/development-agent-troubleshoot-ci-cd-pipeline/run-code-quality-pipeline.png)
...
![파이프라인 실행 실패](./assets/development-agent-troubleshoot-ci-cd-pipeline/failed-pipeline-execution.png)
...
- **Prompt**&#x200B;이라는 자연어로 파이프라인 실패 세부 정보를 입력하십시오. 예:
  ...
  ![AEM 개발 에이전트 호출](./assets/development-agent-troubleshoot-ci-cd-pipeline/invoke-aem-development-agent.png)
...
- 추론이 완료되면 **전체 화면으로 열기** 아이콘을 클릭하여 자세한 문제 해결 프로세스를 확인합니다.
  ...
  ![전체 화면으로 열기](./assets/development-agent-troubleshoot-ci-cd-pipeline/open-in-full-screen.png)
...
- 이 경우 에이전트는 문제를 해결하기 위해 구현(`getName()` 메서드)을 변경하거나 단위 테스트(`getNameTest()` 메서드)를 업데이트할 것을 올바르게 제안했습니다. 개발자에게 실행 가능한 코드 변경 사항을 제공하면서 환각을 피하고 수동 루프 접근 방식을 사용했습니다.
  ...
  ![코드 변경 내용 복사](./assets/development-agent-troubleshoot-ci-cd-pipeline/copy-code-changes.png)
