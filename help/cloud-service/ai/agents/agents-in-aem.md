---
title: AEM의 에이전트
description: Adobe Experience Manager Agent에서 AEM의 작업을 자동화하여 워크플로를 간소화하는 방법을 살펴볼 수 있습니다.
version: Experience Manager as a Cloud Service
role: Leader, User, Developer
level: Beginner
doc-type: Feature Video
duration: 140
last-substantial-update: 2026-01-05T00:00:00Z
jira: KT-20040
exl-id: e5b50205-d348-46b3-b564-50c3967c73b0
source-git-commit: bf6a6f467c3ded713552e0077b13c41fb45511cb
workflow-type: tm+mt
source-wordcount: '98'
ht-degree: 0%

---

# AEM의 에이전트

[AEM AI Assistant](../ai-assistant/overview.md)를 통해 노출되는 에이전트를 사용하여 작업을 자동화하고 워크플로를 간소화하는 방법에 대해 알아봅니다. 에이전트의 정의, 작동 방식, AEM에서 수행하는 작업에 적합한 위치에 대해 알아봅니다.

>[!VIDEO](https://video.tv.adobe.com/v/3478462/?captions=kor&learn=on&enablevpops)

샘플 프롬프트를 포함하여 AEM에서 사용 가능한 에이전트 및 해당 기능의 전체 목록에 대해서는 [AEM의 에이전트 설명서](https://experienceleague.adobe.com/ko/docs/experience-manager-cloud-service/content/ai-in-aem/agents/overview)를 참조하십시오.

*AEM의 에이전트에 액세스하려면 [설치 가이드](../setup.md#agents-in-aem)를 따라 AEM 환경에서 액세스 권한을 설정했는지 확인하십시오.*
