...
2. `ui.apps/src/main/content/jcr_root/apps/wknd-spa-angular/components/card`으로 이동하여 `.content.xml` 파일을 봅니다.
   ...
   속성 `sling:resourceSuperType`은 `wknd-spa-angular/components/image`을(를) 가리킵니다. 즉, `Card` 구성 요소는 WKND SPA 이미지 구성 요소의 모든 기능을 상속합니다.
...
7. Inspect에서 `card.component.ts` 파일을 찾습니다.
   ...
   `src`, `alt` 및 `title`에 대한 클래스의 3개의 `@Input` 매개 변수를 검토합니다. 이러한 값은 Angular 구성 요소에 매핑될 AEM 구성 요소의 JSON 값이 필요합니다.
...
8. `card.component.html` 파일을 엽니다.
   ...
   이 예에서는 `card.component.ts`의 `@Input` 매개 변수를 단순히 전달하여 기존 각도 이미지 구성 요소 `app-image`을 다시 사용하도록 선택했습니다. 자습서의 후반부에 추가 속성이 추가되고 표시됩니다.
...
6. **자산 메타데이터** 탭을 업데이트하여 **대체 텍스트** 및 **캡션**&#x200B;의 값을 추가합니다.
   ...
   현재 대화 상자를 업데이트한 후 추가 변경 사항이 표시되지 않습니다. 새 필드를 각도 구성 요소에 표시하려면 `Card` 구성 요소에 대한 슬링 모델을 업데이트해야 합니다.
...
궁극적으로 구성 요소 대화 상자의 값을 각 구성 요소에 표시하려면 `Card` 구성 요소에 대한 JSON을 채우는 Sling 모델을 업데이트해야 합니다. 또한 다음과 같은 두 가지 비즈니스 로직을 구현할 수 있습니다.
...
4. `card.component.html`을(를) 열고 다음 마크업을 추가하여 제목, 작업 호출 및 마지막으로 수정한 날짜를 표시합니다.
   ...
   >[!NOTE]
   >
   > 완료된 [각 카드 구성 요소 코드는 여기에서 볼 수 있습니다](https://github.com/adobe/aem-guides-wknd-spa/tree/Angular/extend-component-solution/ui.frontend/src/app/components/card).
