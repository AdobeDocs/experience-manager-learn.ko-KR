...
description: AEM SPA 편집기와 통합된 각도 애플리케이션의 시작점으로 Adobe Experience Manager(AEM) Maven 프로젝트를 사용하는 방법을 알아봅니다.
sub-product: sites
...
AEM SPA 편집기와 통합된 각도 애플리케이션의 시작점으로 Adobe Experience Manager(AEM) Maven 프로젝트를 사용하는 방법을 알아봅니다.
...
이 장에서 새 AEM 프로젝트는 [AEM Project Tranype](https://github.com/adobe/aem-project-archetype)을 기반으로 배포됩니다. AEM 프로젝트는 Angular SPA의 매우 간단한 시작점으로 부트스트윗됩니다. 이 장에서 사용된 프로젝트는 WKND SPA의 구현 기반이 되며, 향후 장에 세워질 예정입니다.
...
3. 다음 속성은 [AEM 프로젝트 원형](https://github.com/Adobe-Marketing-Cloud/aem-project-archetype/releases/tag/aem-project-archetype-14)에서 AEM 프로젝트를 생성할 때 사용되었습니다.
   ...
   >[!NOTE]
   >
   > `frontendModule=angular` 속성을 확인합니다. 이렇게 하면 AEM SPA Editor와 함께 사용할 시작 [Angular code base](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/archetype/uifrontend-angular.html)로 프로젝트를 부트스트랩해야 합니다.
...
6. 또한 `spa.project.core` 및 `core.wcm.components`에 대한 여러 패키지가 표시됩니다. 이러한 종속성은 원형 유형에 의해 자동으로 포함되는 종속입니다. [AEM 코어 구성 요소에 대한 자세한 내용은 ](https://docs.adobe.com/content/help/ko-KR/experience-manager-core-components/using/introduction.html)에서 참조할 수 있습니다.
...
2. 페이지의 소스를 보고 텍스트 컨텐트 **[!DNL Hello World]** 또는 다른 컨텐트를 찾을 수 없음을 확인합니다. 대신 다음과 같은 HTML이 표시됩니다.
   ...
   `clientlib-angular.min.js` 는 페이지로 로드되고 컨텐츠 렌더링을 담당하는 Angular SPA입니다.
...
4. 브라우저의 개발자 도구를 열고 새로 고치는 동안 페이지의 네트워크 트래픽을 검사합니다. **XHR** 요청 보기:
   ...
   [http://localhost:4502/content/wknd-spa-angular/us/en.model.json](http://localhost:4502/content/wknd-spa-angular/us/en.model.json)에 대한 요청이 있어야 합니다. 여기에는 SPA을 구동하는 JSON으로 구성된 모든 컨텐츠가 포함되어 있습니다.
...
5. 새 탭에서 [http://localhost:4502/content/wknd-spa-angular/us/en.model.json](http://localhost:4502/content/wknd-spa-angular/us/en.model.json)을 엽니다.
   ...
   다음 장에서는 JSON 컨텐츠가 AEM 구성 요소에서 SPA 구성 요소로 어떻게 매핑되어 AEM SPA 편집기 경험의 기반이 되는지 살펴봅니다.
...
지금은 간단하지만 다음 몇 장 더 많은 기능이 추가될 예정입니다.
