...
3. `CustomComponent.java`을 업데이트하여 `ComponentExporter` 인터페이스를 확장합니다.
   ...
   `CustomComponent` 인터페이스에는 단일 getter 메서드 `getMessage()`이 포함되어 있습니다. JSON 모델을 통해 작성 대화 상자의 값을 표시하는 메서드입니다. 빈 매개 변수 `()`이 있는 getter 메서드만 JSON 모델에 내보내집니다.

4. `core/src/main/java/com/adobe/aem/guides/wknd/spa/angular/core/models/impl/CustomComponentImpl.java`에서 `CustomComponentImpl.java` 을 엽니다.
...
5. 정적 변수 `RESOURCE_TYPE`을 업데이트하여 이전 연습에서 만든 AEM 구성 요소 `wknd-spa-angular/components/custom-component`를 가리킵니다.
   ...
   구성 요소의 리소스 유형은 Sling 모델을 AEM 구성 요소에 바인딩하고 궁극적으로 Angular 구성 요소에 매핑됩니다.
...
7. 작성자 대화 상자에서 지속되는 `message` 속성 값을 반환하려면 `getMessage()` 메서드를 업데이트합니다. `@ValueMap` 주석을 사용하여 JCR 값 `message`을 Java 변수에 매핑합니다.
   ...
   >[!NOTE]
   >
   > [완성된 CustomComponentImpl.java를 여기에서 볼 수 있습니다](https://github.com/adobe/aem-guides-wknd-spa/blob/Angular/custom-component-solution/core/src/main/java/com/adobe/aem/guides/wknd/spa/angular/core/models/impl/CustomComponentImpl.java).
...
2. `@Input() message: string;` 줄을 준수합니다. 변환된 대문자 값은 이 변수에 매핑되어야 합니다.
...
4. `cutom.component.html` 을 열고 `{{message}}` 값이 `<h2>` 태그 옆에 표시되는지 확인합니다.
5. `custom.component.css` 을 열고 다음 규칙을 추가합니다.
...
1. [http://localhost:4502/system/console/status-slingmodels](http://localhost:4502/system/console/status-slingmodels)로 이동하여 새 Sling 모델의 등록을 확인합니다.
   ...
   위의 두 행이 표시되고 `CustomComponentImpl` 구성 요소가 `wknd-spa-angular/components/custom-component` 구성 요소와 연결되며 Sling 모델 익스포터를 통해 등록되었음을 나타냅니다.
...
5. [http://localhost:4502/content/wknd-spa-angular/us/en.model.json](http://localhost:4502/content/wknd-spa-angular/us/en.model.json)로 이동하여 JSON 모델을 봅니다. `wknd-spa-angular/components/custom-component` 검색:
