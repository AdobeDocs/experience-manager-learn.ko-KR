...
항상 [GitHub에서](https://github.com/adobe/aem-guides-wknd-spa/tree/Angular/custom-component-solution) 완료된 코드를 보거나 분기로 전환하여 로컬로 코드를 체크 아웃할 수 `Angular/custom-component-solution`있습니다.
...
2. 새 폴더 `ui.apps/src/main/content/jcr_root/apps/wknd-spa-angular/components` 로 이동하여 만듭니다 `custom-component`.
...
2. 다음 위치에서 Java 인터페이스 `CustomComponent.java` 를 엽니다 `core/src/main/java/com/adobe/aem/guides/wknd/spa/angular/core/models/CustomComponent.java`.
...
3. 인터페이스 `CustomComponent.java` 를 확장하도록 `ComponentExporter` 업데이트합니다.
   ...
   인터페이스에는 단일 getter 메서드가 포함되어 있습니다 `CustomComponent` `getMessage()`. JSON 모델을 통해 작성 대화 상자의 값을 노출하는 방법입니다. 빈 매개 변수를 사용하는 getter 메서드만 JSON 모델에 `()` 내보내집니다.

4. 에서 `CustomComponentImpl.java` 엽니다 `core/src/main/java/com/adobe/aem/guides/wknd/spa/angular/core/models/impl/CustomComponentImpl.java`.
...
5. 정적 변수 `RESOURCE_TYPE` 를 업데이트하여 이전 연습 `wknd-spa-angular/components/custom-component` 에서 만든 AEM 구성 요소를 가리킵니다.
...
7. 작성 대화 상자 `getMessage()` 에 의해 지속된 `message` 속성의 값을 반환하려면 메서드를 업데이트합니다. 주석을 `@ValueMap` 사용하면 JCR 값이 Java 변수 `message` 에 매핑됩니다.
   ...
   >[!NOTE]
   >
   > 여기서 [완성된 CustomComponentImpl.java를 볼 수 있습니다](https://github.com/adobe/aem-guides-wknd-spa/blob/Angular/custom-component-solution/core/src/main/java/com/adobe/aem/guides/wknd/spa/angular/core/models/impl/CustomComponentImpl.java).

## 각도 구성 요소 업데이트

사용자 지정 구성 요소의 각 코드가 이미 생성되었습니다. 그런 다음 각도 구성 요소를 AEM 구성 요소에 매핑하려면 몇 가지 업데이트를 수행합니다.

1. 모듈에서 파일을 `ui.frontend` 엽니다 `ui.frontend/src/app/components/custom/custom.component.ts`
2. 선을 `@Input() message: string;` 지켜라. 변형된 대문자 값이 이 변수에 매핑되어야 합니다.
...
4. 열고 값 `cutom.component.html` 이 태그 `{{message}}` 와 함께 표시되는지 `<h2>` 확인합니다.
5. 다음 규칙 `custom.component.css` 을 열고 추가합니다.
   ...
   구성 요소가 비어 있을 때 AEM 편집기 자리 표시자가 제대로 표시되도록 하려면 `:host-context` 또는 다른 `<div>` 자리 표시자를 설정해야 합니다 `display: block;`.
...
1. http://localhost:4502/system/console/status-slingmodels으로 이동하여 새 Sling Model의 등록을 [확인합니다](http://localhost:4502/system/console/status-slingmodels).
   ...
   위의 두 줄은 구성 요소와 `CustomComponentImpl` 연결되어 있고 Sling 모델 내보내기 도구를 통해 등록되었음을 나타내는 `wknd-spa-angular/components/custom-component` 것입니다.
...
5. http://localhost:4502/content/wknd-spa-angular/us/en.model.json으로 이동하여 JSON 모델을 [봅니다](http://localhost:4502/content/wknd-spa-angular/us/en.model.json). Search for `wknd-spa-angular/components/custom-component`:
