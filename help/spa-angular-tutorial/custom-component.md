...
2. `ui.apps/src/main/content/jcr_root/apps/wknd-spa-angular/components`으로 이동하고 `custom-component`이라는 새 폴더를 만듭니다.
...
3. `CustomComponent.java`을 업데이트하여 `ComponentExporter` 인터페이스를 확장합니다.
   ...
   `CustomComponent` 인터페이스에는 단일 getter 메서드 `getMessage()`가 포함되어 있습니다. JSON 모델을 통해 작성 대화 상자의 값을 노출하는 방법입니다. 빈 매개 변수 `()`이(가) 있는 getter 메서드만 JSON 모델에서 내보내집니다.
...
5. 정적 변수 `RESOURCE_TYPE`을 업데이트하여 이전 연습에서 만든 AEM 구성 요소 `wknd-spa-angular/components/custom-component`을 가리킵니다.
...
6. `getExportedType()` 메서드를 `CustomComponentImpl` 클래스에 추가하여 구성 요소 리소스 유형을 반환합니다.
   ...
   이 방법은 `ComponentExporter` 인터페이스를 구현할 때 필요하며 Angular 구성 요소에 매핑을 허용하는 리소스 유형이 표시됩니다.
...
7. `getMessage()` 메서드를 업데이트하여 작성자 대화 상자에서 지속된 `message` 속성의 값을 반환합니다. `@ValueMap` 주석을 사용하여 JCR 값 `message`을 Java 변수에 매핑합니다.
   ...
   메시지 값을 대문자로 반환하는 일부 추가 &quot;비즈니스 논리&quot;가 추가됩니다. 이를 통해 작성자 대화 상자에 저장된 원시 값과 스링 모델에 의해 노출된 값 간의 차이를 확인할 수 있습니다.

   >[!NOTE]
   >
   > [완성된 CustomComponentImpl.java를 여기](https://github.com/adobe/aem-guides-wknd-spa/blob/Angular/custom-component-solution/core/src/main/java/com/adobe/aem/guides/wknd/spa/angular/core/models/impl/CustomComponentImpl.java)에서 볼 수 있습니다.

## 각도 구성 요소 업데이트

사용자 지정 구성 요소의 각 코드가 이미 생성되었습니다. 그런 다음 각도 구성 요소를 AEM 구성 요소에 매핑하려면 몇 가지 업데이트를 수행합니다.

1. `ui.frontend` 모듈에서 `ui.frontend/src/app/components/custom/custom.component.ts` 파일을 엽니다.
2. `@Input() message: string;` 줄을 관찰합니다. 변형된 대문자 값이 이 변수에 매핑되어야 합니다.
...
4. `cutom.component.html`을 열고 `{{message}}`의 값이 `<h2>` 태그와 함께 표시되는지 확인합니다.
...
6. Maven 기술을 사용하여 프로젝트 디렉토리의 루트에서 로컬 AEM 환경에 모든 업데이트를 배포합니다.
...
1. [http://localhost:4502/system/console/status-slingmodels](http://localhost:4502/system/console/status-slingmodels)으로 이동하여 새 Sling 모델의 등록을 확인합니다.
   ...
   `CustomComponentImpl`이(가) `wknd-spa-angular/components/custom-component` 구성 요소와 연결되어 있고 Sling Model Exporter를 통해 등록되었음을 나타내는 위의 두 줄이 표시됩니다.
...
축하합니다. 사용자 정의 AEM 구성 요소를 만드는 방법과 Sling Models 및 대화 상자가 JSON 모델에서 어떻게 작동하는지 알아보았습니다.
