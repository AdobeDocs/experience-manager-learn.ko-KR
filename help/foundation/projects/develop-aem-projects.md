...
1. 프로젝트 작업 만들기 워크플로 프로세스는 워크플로의 단계로 작업을 만들도록 설계되었습니다. 작업을 완료한 후에만 워크플로가 진행됩니다. 프로젝트 만들기 작업 단계의 강력한 측면은 워크플로 메타데이터 값을 읽고 이를 사용하여 작업을 동적으로 만들 수 있다는 것입니다.
   ...
   먼저 기본적으로 생성되는 참가자 단계를 삭제합니다. 구성 요소 메뉴의 사이드 킥에서 **&quot;프로젝트&quot;** 하위 제목 을 클릭하고 **&quot;프로젝트 작업 만들기&quot;** 모델에 연결합니다.
...
1. 콘텐츠 승인 워크플로로 다시 이동합니다. 드래그 앤 드롭 **OR 분할** 구성 요소(&#39;워크플로&#39; 카테고리 아래의 사이드 킥에 있음) **작업 시작** 단계. 공통 대화 상자에서 3 개의 분기에 대한 라디오 버튼을 선택합니다. OR 분할은 워크플로 메타데이터 값을 읽습니다. **&quot;lastTaskAction&quot;** 을 클릭하여 워크플로의 경로를 결정합니다. 다음 **&quot;lastTaskAction&quot;** 속성은 4단계에서 구성된 라우팅 탭의 값 중 하나로 설정됩니다. 각 분기 탭에 대해 **스크립트** 다음 값이 있는 텍스트 영역:
...
1. 승인자 그룹에는 추가 개정을 위해 워크플로우를 원래 편집기로 다시 전송하는 옵션이 있으므로 **이동** 단계에서 수행한 마지막 작업을 읽고 워크플로를 시작 부분으로 라우팅하거나 계속하도록 합니다.
   ...
   다시 연결되는 OR 분할 아래에 있는 이동 단계 구성 요소( 워크플로 아래의 사이드 킥에 있음)를 드래그 앤 드롭합니다. 두 번 클릭하고 대화 상자에서 다음 속성을 구성합니다.
...
1. 페이로드를 게시하려면 ootb를 사용합니다. **페이지/자산 활성화** 프로세스 단계. 이 프로세스 단계는 구성이 거의 필요하지 않으며 활성화를 위해 워크플로의 페이로드를 복제 큐에 추가합니다. 이동 단계 아래에 단계를 추가하겠습니다. 이 방법으로 승인자 그룹이 게시할 콘텐츠를 승인하거나 원래 편집자가 승인 무시 경로를 선택한 경우에만 단계에 도달할 수 있습니다.
   ...
   드래그 앤 드롭 **페이지/자산 활성화** 모델의 이동 단계 아래에 있는 프로세스 단계(WCM 워크플로 아래의 사이드 킥에 있음)입니다.
