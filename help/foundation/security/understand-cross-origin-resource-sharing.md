...
Adobe Experience Manager의 교차 도메인 리소스 공유([!DNL CORS])을 사용하면 AEM이 아닌 웹 속성을 사용하여 인증되고 인증되지 않은 AEM에 클라이언트측 호출을 하여 컨텐츠를 가져오거나 AEM과 직접 상호 작용할 수 있습니다.
...
* `"alloworigin" <origin> | *`
* 목록 `origin` 리소스에 액세스할 수 있는 URI를 지정하는 매개 변수입니다. 자격 증명이 없는 요청의 경우 서버는 &#42; 와일드카드로 사용할 수 있으므로 모든 원본에서 리소스에 액세스할 수 있습니다. *사용하지 않는 것이 좋습니다 `Allow-Origin: *` 따라서 모든 외부(즉, 공격자) 웹 사이트에서 CORS가 없는 경우 브라우저가 이를 엄격하게 금지한다고 요청할 수 있으므로 프로덕션에서 사용할 수 있습니다.*

#### [!UICONTROL 허용된 원본(Regexp)]

* `"alloworiginregexp" <regexp>`
* 목록 `regexp` 리소스에 액세스할 수 있는 URI를 지정하는 정규 표현식. *정규 표현식을 주의 깊게 빌드하지 않으면 의도하지 않은 일치 항목이 발생할 수 있으므로 공격자가 정책과 일치하는 사용자 지정 도메인 이름을 사용할 수 있습니다.* 일반적으로 를 사용하여 각 특정 원본 호스트 이름에 대해 별도의 정책을 갖는 것이 좋습니다 `alloworigin`를 설정하는 것이 좋습니다. 서로 다른 기원은 라이프 사이클과 요구 사항이 다르기 때문에 명확한 분리로부터 이익을 얻는 경향이 있다.
...
* `"maxage" <seconds>`
* A `seconds` 사전 전송 요청 결과를 캐시할 수 있는 시간을 나타내는 매개 변수입니다.

#### [!UICONTROL 지원되는 헤더]

* `"supportedheaders" <header>`
* 목록 `header` 실제 요청을 만들 때 사용할 수 있는 HTTP 헤더를 나타내는 매개 변수입니다.
...
* A `boolean` 요청에 대한 응답을 브라우저에 노출할 수 있는지 여부를 나타냅니다. 플라이트 전 요청에 대한 응답의 일부로 사용되는 경우, 자격 증명을 사용하여 실제 요청을 수행할 수 있는지 여부를 나타냅니다.
...
Dispatcher 4.1.1+ 응답 헤더부터 캐시할 수 있습니다. 이렇게 하면 캐싱이 가능합니다 [!DNL CORS] 머리글 옆에 [!DNL CORS]요청이 익명 요청인 한 -요청된 리소스입니다.

일반적으로 Dispatcher에서 콘텐츠를 캐싱하는 것과 동일한 고려 사항을 Dispatcher에서 CORS 응답 헤더를 캐싱하는 데 적용할 수 있습니다. 다음 표에서는 [!DNL CORS] 헤더(따라서 [!DNL CORS] 요청)을 캐시할 수 있습니다.

| 캐시 가능 | 환경 | 인증 상태 | 설명 |
|-----------|-------------|-----------------------|-------------|
| 아니요 | AEM 게시 | 인증됨 | AEM 작성자에 대한 Dispatcher 캐싱은 정적 비작성 자산으로 제한됩니다. 따라서 HTTP 응답 헤더를 포함하여 대부분의 리소스를 AEM 작성자에게 캐시하는 것이 어렵고 비현실적입니다. |
| 아니요 | AEM 게시 | 인증됨 | 인증된 요청 시 CORS 헤더를 캐싱하지 마십시오. 이는 요청 사용자의 인증/승인 상태가 전달된 리소스에 어떤 영향을 주는지 결정하기가 어렵기 때문에 인증된 요청을 캐싱하지 않는다는 일반적인 지침을 따릅니다. |
| 예 | AEM 게시 | 익명 | dispatcher에서 캐시 가능한 익명 요청은 응답 헤더도 캐시할 수 있으므로 향후 CORS 요청이 캐시된 컨텐츠에 액세스할 수 있도록 합니다. AEM 게시의 모든 CORS 구성 변경 **반드시** 영향을 받는 캐시된 리소스의 무효화가 그 뒤에 옵니다. 우수 사례에 따라 디스패처 캐시가 삭제되는 코드 또는 구성 배포에 대해 결정됩니다. 캐시된 콘텐츠가 영향을 받을 수 있는지 확인하는 것은 어렵습니다. |
...
Logging은 `com.adobe.granite.cors`:

* 활성화 `DEBUG` 자세한 내용은 [!DNL CORS] 요청이 거부되었습니다.
* 활성화 `TRACE` CORS 핸들러를 통해 전달되는 모든 요청에 대한 세부 사항을 보려면
...
* CORS 처리기에서 요청이 거부되었는지, 인증, CSRF 토큰 필터, 디스패처 필터 또는 기타 보안 레이어가 아닌지 확인합니다
   * CORS 처리기가 200에서 응답하는 경우 `Access-Control-Allow-Origin` 헤더가 응답에 없으면 로그를 검토하여 [!DNL DEBUG] in `com.adobe.granite.cors`
* 의 디스패처 캐싱이 [!DNL CORS] 요청이 활성화됨
   * 다음을 확인합니다. `/cache/headers` 구성이 `dispatcher.any` 웹 서버를 다시 시작한 후
