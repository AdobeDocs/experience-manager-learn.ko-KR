...
1. 다음으로 이동 __도구 > 구성 브라우저__ headless 경험을 위한 구성을 만들기 위해

   ![폴더 만들기](./assets/1/create-configuration.png)

   다음을 제공합니다. __제목__ 및 __이름__, 및 확인 __GraphQL 지속 쿼리__ 및 __컨텐츠 조각 모델__.


## 콘텐츠 조각 모델

1. 다음으로 이동 __도구 > 콘텐츠 조각 모델__ 1단계에서 만든 구성의 이름으로 폴더를 선택합니다.

   ![모델 폴더](./assets/1/model-folder.png)

1. 폴더 내에서 __만들기__ 모델 이름 지정 __티저__. 에 다음 데이터 형식 추가 __티저__ 모델.
...
1. 폴더 내에서 라는 두 번째 모델을 만듭니다. __오퍼__. 만들기 를 클릭하고 모델에 &quot;Offer&quot; 이름을 지정하고 다음 데이터 형식을 추가합니다.
...
1. 폴더 내에서 라는 세 번째 모델을 만듭니다. __이미지 목록__. 만들기 를 클릭하고 모델에 &quot;이미지 목록&quot; 이름을 지정하고 다음 데이터 형식을 추가합니다.
...
1. 이제 Assets로 이동하여 새 사이트에 대한 폴더를 만듭니다. 만들기 를 클릭하고 폴더 이름을 지정합니다.

   ![폴더 추가](./assets/1/create-folder.png)

1. 폴더가 만들어지면 폴더를 선택하고 폴더를 엽니다 __속성__.
1. 폴더의 __클라우드 구성__ 탭에서 구성을 선택합니다. [이전에 생성됨](#enable-content-fragments-and-graphql).
   ...
   새 폴더를 클릭하고 티저를 만듭니다. 클릭 __만들기__ 및 __컨텐츠 조각__ 및 선택 __티저__ 모델. 모델 이름 지정 __영웅__ 및 클릭 __만들기__.
...
1. 클릭 __만들기__ 새 끝점에 이름을 지정하고 새로 만든 구성을 선택합니다.

   ![AEM Headless GraphQL 엔드포인트](./assets/1/endpoint.png)

## GraphQL 지속 쿼리

1. 새 엔드포인트를 테스트해 보겠습니다. 다음으로 이동 __도구 > GraphQL 쿼리 편집기__ 그리고 창의 오른쪽 상단에 있는 드롭다운에 대한 엔드포인트를 선택합니다.
...
1. 쿼리 편집기에서 몇 가지 다른 쿼리를 만듭니다.
   ...
   만들어진 단일 조각이 포함된 목록을 가져와야 합니다. [위](#create-content).
   ...
   다음에서 __쿼리 변수__ 맨 아래에 을(를) 입력하고 다음을 입력합니다.
...
1. 클릭 __저장__  쿼리를 유지(저장)하고 쿼리 이름을 지정합니다. __티저__. 이를 통해 애플리케이션에서 이름별로 쿼리를 참조할 수 있습니다.

## 다음 단계

축하합니다! AEM 콘텐츠 조각 및 GraphQL 끝점을 만들 수 있도록 as a Cloud Service으로 구성했습니다. 또한 콘텐츠 조각 모델 및 콘텐츠 조각을 만들고 GraphQL 엔드포인트 및 지속 쿼리를 정의했습니다. 이제 다음 튜토리얼 장으로 이동하여 이 장에서 만든 콘텐츠 조각 및 GraphQL 엔드포인트를 사용하는 AEM Headless React 애플리케이션을 만드는 방법을 배울 준비가 되었습니다.
