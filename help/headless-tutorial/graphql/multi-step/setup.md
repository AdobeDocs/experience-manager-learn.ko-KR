...
>[!NOTE]
>
> 또한 AEM as a Cloud Service 환경에서 자습서를 따를 수도 있습니다. 클라우드 환경 사용에 대한 추가 참고 사항은 자습서 전체에서 포함되어 있습니다.
...
1. 선택한 IDE에서 파일을 엽니다 `.env.development` at `aem-guides-wknd-graphql/react-app/.env.development`. 다음 사항을 확인합니다. `REACT_APP_AUTHORIZATION` 줄이 주석 처리되지 않고 파일의 모양은 다음과 같습니다.
   ...
   >[!NOTE]
   >
   > 프로덕션 시나리오에서는 앱이 AEM에 연결됩니다 **게시** 환경. 자세한 내용은 [프로덕션 배포](production-deployment.md) 제2장.
...
1. GraphiQL IDE로 이동합니다. [http://localhost:4502/content/graphiql.html](http://localhost:4502/content/graphiql.html) GraphQL API 탐색을 시작합니다.
   ...
   >[!NOTE]
   >
   > GraphiQL 도구 및 GraphQL API는 다음과 같습니다 [튜토리얼의 후반부에 더 자세히 살펴보십시오](./explore-graphql-api.md).
...
다음 장에서 [컨텐츠 조각 모델 정의](content-fragment-models.md)를 사용하여 컨텐츠를 모델링하고 스키마를 구축하는 방법을 알아봅니다. **컨텐츠 조각 모델**. 기존 모델을 검토하고 새 모델을 만듭니다. 또한 모델의 일부로 스키마를 정의하는 데 사용할 수 있는 다양한 데이터 유형에 대해서도 학습합니다.
