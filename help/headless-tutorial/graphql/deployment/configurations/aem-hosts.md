...
사용자 지정 AEM Headless 클라이언트(`api/Aem.swift`)에 구성된 AEM `makeRequest(..)` 및 `scheme`(으)로 AEM GraphQL API 요청 앞에 오는 메서드 `host`이(가) 포함되어 있습니다.
...
이 예는 개발(`AEM_HOST`) 및 프로덕션(`dev`)에서 사용하는 서로 다른 AEM 서비스 호스트(`prod`) 값을 제공하는 두 개의 Android™ 제품 버전을 정의하는 방법을 보여줍니다.

앱의 `build.gradle` 파일에 이름이 `flavorDimension`인 새 `env`이(가) 만들어집니다.
...
Java™용 AEM Headless 클라이언트에서 제공한 `AEMHeadlessClient` 빌더를 `AEM_HOST` 필드의 `buildConfigField` 값으로 초기화합니다.
...
이 예제에서는 구성 가능한 `_dynamicUrl` React 환경 변수와 함께 이미지 참조 `REACT_APP_AEM_HOST` 필드를 접두사로 사용하는 방법을 보여 줍니다.
...
Android™ 보기는 GraphQL 응답의 `RemoteImagesCache` 값을 사용하여 `_dynamicUrl`을(를) 통해 이미지 데이터를 가져옵니다.
