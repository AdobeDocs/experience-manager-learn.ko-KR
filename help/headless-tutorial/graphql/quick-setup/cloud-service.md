...
1. WKND Site 프로젝트를 위한 Git 리포지토리 만들기
   ...
   1. 새 Git 리포지토리의 이름을 지정합니다. `aem-headless-quick-setup`
...
1. 첨부 __비프로덕션 배포 파이프라인__ 새 Git 리포지토리에
   ...
   1. 설정 __소스 코드__ 탭
      ...
      1. 선택 `aem-headless-quick-setup` 에서 __저장소__ 선택 상자
...
1. 를 실행합니다. __개발 배포 파이프라인__
   ...
   1. 선택 __세부 사항 보기__
1. 파이프라인 실행의 세부 정보에서 성공적으로 완료될 때까지 진행 상황을 모니터링합니다. 파이프라인 실행은 45-60분 정도 소요됩니다.
...
1. 폴더를 엽니다. `~/Code/aem-guides-wknd-graphql` IDE에서
1. IDE에서 파일을 엽니다 `react-app/.env.development`.
...
1. 다음으로 이동 __자산 > 파일 > WKND > 영어 > 모험__
...
1. React 앱에서 지금 업데이트됨 을(를) 선택하고 컨텐츠 조각에 대한 컨텐츠 변경 사항을 확인합니다.
