...
SPA 코드베이스는 AEM 외부에서 관리되지만, 구성 및 콘텐츠 요구 사항을 지원하려면 AEM 프로젝트가 필요합니다. 이 장에서는 필요한 구성을 포함하는 AEM 프로젝트를 만드는 과정을 안내합니다.

* AEM WCM 코어 구성 요소 프록시
* AEM 원격 SPA 페이지 프록시
* AEM 원격 SPA 페이지 템플릿
* 기본 원격 SPA AEM 페이지
* SPA와 AEM URL 매핑을 정의하는 하위 프로젝트
* OSGi 구성 폴더
...
_항상 최신 버전의 [AEM Archetype](https://github.com/adobe/aem-project-archetype)을(를) 사용합니다._
...
SPA는 원격 SPA이므로 AEM 프로젝트 외부에서 개발 및 관리되었다고 가정하십시오. 충돌을 방지하려면 배포에서 `ui.frontend` 프로젝트를 제거하십시오. `ui.frontend` 프로젝트를 제거하지 않으면 `ui.frontend` 프로젝트에 제공된 기본 SPA인 두 개의 SPA와 원격 SPA가 AEM SPA 편집기에서 동시에 로드됩니다.
...
AEM이 SPA 편집기에서 원격 SPA를 로드하려면 SPA의 경로와, 컨텐츠를 열고 작성하는 데 사용되는 AEM 페이지 간의 매핑을 설정해야 합니다.
...
* `alloworigin`은(는) AEM에서 콘텐츠를 검색할 수 있는 호스트를 지정합니다.
   * 로컬에서 실행되는 SPA를 지원하기 위해 `localhost:3000`이(가) 추가되었습니다.
   * `https://external-hosted-app`은(는) 원격 SPA가 호스팅되는 도메인으로 대체할 자리 표시자 역할을 합니다.
* `allowedpaths`은(는) 이 CORS 구성에서 다루는 AEM의 경로를 지정합니다. 기본적으로 AEM의 모든 콘텐츠에 액세스할 수 있지만 SPA가 액세스할 수 있는 특정 경로로만 범위를 지정할 수 있습니다(예: `/content/wknd-app`).

## AEM 페이지를 원격 SPA 페이지 템플릿으로 설정

AEM Project Archetype은 AEM과 원격 SPA의 통합을 위해 준비된 프로젝트를 생성하지만 자동 생성된 AEM 페이지 구조에 대한 작지만 중요한 조정이 필요합니다. 자동 생성된 AEM 페이지의 유형은 **SPA 페이지**&#x200B;가 아닌 **원격 SPA 페이지**(으)로 변경되어야 합니다.
...
이러한 변경 사항을 통해 AEM에서 SPA의 루트인 역할을 하는 이 페이지가 SPA 편집기에서 원격 SPA를 로드할 수 있습니다.
...
AEM 프로젝트가 배포되면 SPA 편집기를 준비하여 원격 SPA를 로드하는 마지막 단계가 있습니다. AEM에서 AEM Project Archetype으로 생성된 SPA의 루트 `/content/wknd-app/us/en/home`에 해당하는 AEM 페이지를 표시합니다.

1. AEM 작성자에 로그인
1. **사이트 > WKND 앱 > us > en**(으)로 이동합니다.
1. **WKND 앱 홈 페이지**&#x200B;를 선택하고 **속성**&#x200B;을 탭하세요.

   ![WKND 앱 홈 페이지 - 속성](./assets/aem-content/edit-home-properties.png)

1. **SPA** 탭으로 이동
1. **원격 SPA 구성** 작성
   1. **SPA 호스트 URL**: `http://localhost:3000`
      1. 원격 SPA의 루트에 대한 URL

   ![WKND 앱 홈 페이지 - 원격 SPA 구성](./assets/aem-content/remote-spa-configuration.png)

1. **저장 및 닫기** 탭

이 페이지의 유형을 **원격 SPA 페이지**&#x200B;의 유형으로 변경했습니다. 이렇게 하면 **페이지 속성**&#x200B;에서 **SPA** 탭을 볼 수 있습니다.
...
* `ui.frontend`의 종속성에 주석을 달아 AEM Project Archetype 생성 SPA를 제거합니다.
* SPA 경로를 AEM의 리소스에 매핑하는 AEM에 Sling 매핑 추가
* 원격 SPA가 AEM의 콘텐츠를 사용할 수 있도록 하는 AEM의 원본 간 리소스 공유 보안 정책을 설정합니다
* AEM 프로젝트를 로컬 AEM SDK Author 서비스에 배포합니다
* SPA 호스트 URL 페이지 속성을 사용하여 AEM 페이지를 원격 SPA의 루트로 표시

## 다음 단계

AEM이 구성되면 AEM SPA 편집기를 사용하여 편집 가능한 영역을 지원하여 [원격 SPA 부트스트랩](./spa-bootstrap.md)에 집중할 수 있습니다!
