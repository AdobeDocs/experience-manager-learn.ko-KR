...
이 장에서는 [AEM 프로젝트 원형](https://github.com/adobe/aem-project-archetype). AEM 프로젝트는 React SPA에 대한 매우 간단한 시작점이 부트스트되었습니다.
...
1. 명령줄 터미널을 열고 다음 Maven 명령을 입력합니다.
   ...
   >[!NOTE]
   >
   > AEM 6.5.5+를 타깃팅하는 경우 `aemVersion="cloud"` with `aemVersion="6.5.5"`. 6.4.8+를 타깃팅하는 경우 `aemVersion="6.4.8"`.

   다음 사항에 주의하십시오. `frontendModule=react` 속성을 사용합니다. 이렇게 하면 AEM Project Archetype에서 시작 모드로 프로젝트를 부트스트랩하도록 지시합니다 [React 코드 기본](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/uifrontend-react.html) AEM SPA 편집기에 사용할 수 있습니다. 과 같은 속성 `appTitle`, `appId`, `artifactId`, 및 `groupId` 프로젝트 및 목적을 식별하는 데 사용됩니다.
...
1. 다음 폴더 및 파일 구조는 로컬 파일 시스템의 Maven 원형에 의해 생성됩니다.
   ...
   각 폴더는 개별 Maven 모듈을 나타냅니다. 이 자습서에서는 주로 `ui.frontend` 모듈 - React 앱입니다. 개별 모듈에 대한 자세한 내용은 [AEM 프로젝트 원형 설명서](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/overview.html).
...
1. 명령줄에서 `aem-guides-wknd-spa.react` 프로젝트 디렉터리입니다.
...
1. 접두사가 여러 개 있어야 합니다 `aem-guides-wknd-spa.react`.
...
1. 로 이동합니다 **Sites** 콘솔: [http://localhost:4502/sites.html/content](http://localhost:4502/sites.html/content).
...
2. 를 엽니다. **미국** > **en** > **WKND SPA React 홈 페이지** 페이지를 선택하고 **편집** 메뉴 모음의 단추:

   ![사이트 콘솔](./assets/create-project/open-home-page.png)

3. A **텍스트** 구성 요소가 페이지에 이미 추가되었습니다. 이 구성 요소는 AEM의 다른 구성 요소와 같이 편집할 수 있습니다.
...
1. 에서 **페이지 편집기**&#x200B;를 클릭하고 **페이지 정보** 버튼 > **게시됨으로 보기**:

   ![게시됨으로 보기 단추](./assets/create-project/view-as-published.png)

   이렇게 하면 쿼리 매개 변수가 포함된 새 탭이 열립니다 `?wcmmode=disabled` AEM 편집기를 효과적으로 끕니다. [http://localhost:4502/content/wknd-spa-react/us/en/home.html?wcmmode=disabled](http://localhost:4502/content/wknd-spa-react/us/en/home.html?wcmmode=disabled)
...
4. 새로 고침 중에 브라우저의 개발자 도구를 열고 페이지의 네트워크 트래픽을 검사합니다. 보기 **XHR** 요청:
   ...
   에 대한 요청이 있어야 합니다. [http://localhost:4502/content/wknd-spa-react/us/en.model.json](http://localhost:4502/content/wknd-spa-react/us/en.model.json). 여기에는 SPA을 구동하는 JSON으로 포맷된 모든 컨텐츠가 포함됩니다.

5. 새 탭에서 를 엽니다. [http://localhost:4502/content/wknd-spa-react/us/en.model.json](http://localhost:4502/content/wknd-spa-react/us/en.model.json)
