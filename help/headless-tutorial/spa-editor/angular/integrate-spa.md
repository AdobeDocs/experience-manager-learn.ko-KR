...
Next, inspect the `ui.frontend` module to understand the SPA that has been auto-generated by the [AEM Project archetype](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/uifrontend-angular.html?lang=ko).

1. 선택한 IDE에서 WKND SPA에 대한 AEM 프로젝트를 엽니다. This tutorial will use the [Visual Studio Code IDE](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/local-development-environment-set-up/development-tools.html?lang=ko#microsoft-visual-studio-code).
...
4. There are also three dependencies prefixed with `@adobe`:
   ...
   The above modules make up the [AEM SPA Editor JS SDK](https://experienceleague.adobe.com/docs/experience-manager-65/developing/headless/spas/spa-blueprint.html?lang=ko) and provide the functionality to make it possible to map SPA Components to AEM Components.
...
5. In the `package.json` file several `scripts` are defined:
   ...
   More details about the available scripts can be found [here](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/uifrontend-angular.html?lang=ko).
