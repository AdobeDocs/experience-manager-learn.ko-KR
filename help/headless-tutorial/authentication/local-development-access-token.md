...
AEM as a Cloud Service에 프로그래밍 방식으로 액세스해야 하는 통합을 구축하는 개발자는 로컬 개발 활동을 용이하게 하기 위해 AEM에 대한 임시 액세스 토큰을 가져오는 간단하고 빠른 방법이 필요합니다. 이러한 요구 사항을 충족하기 위해 AEM 개발자 콘솔을 사용하여 개발자가 프로그래밍 방식으로 AEM에 액세스하는 데 사용할 수 있는 임시 액세스 토큰을 자동으로 생성할 수 있습니다.
...
1. AEM as a Cloud Service과 프로그래밍 방식으로 상호 작용하는 외부 애플리케이션을 개발 중입니다
1. 외부 응용 프로그램이 로컬 개발 액세스 토큰에서 읽습니다
1. 외부 응용 프로그램은 AEM as a Cloud Service에 HTTP 요청을 구성하고 로컬 개발 액세스 토큰을 HTTP 요청의 인증 헤더에 베어러 토큰으로 추가합니다
...
1. 명령줄에서 매개 변수로 읽기(`getCommandLineParams()`)
1. AEM as a Cloud Service에 인증하는 데 사용되는 액세스 토큰을 가져옵니다(`getAccessToken(...)`)
1. 명령줄 매개 변수에 지정된 AEM 자산 폴더의 모든 자산을 나열합니다(`listAssetsByFolder(...)`)
1. 나열된 자산의 메타데이터를 명령줄 매개 변수에 지정된 값으로 업데이트(`updateMetadata(...)`)
...
1. 확인 [Node.js](/help/cloud-service/local-development-environment/development-tools.md?lang=en#node-js) 외부 응용 프로그램을 실행하는 데 사용되는 로컬 개발 시스템에 설치됩니다
1. 다운로드 및 압축 해제 [샘플 외부 애플리케이션](./assets/aem-guides_token-authentication-external-application.zip)
1. 명령줄에서 이 프로젝트의 폴더에서 을(를) 실행합니다. `npm install`
1. 를 복사합니다. [로컬 개발 액세스 토큰 다운로드됨](#download-local-development-access-token) 라는 파일에 `local_development_token.json` 프로젝트의 루트에서
   + 그러나 Git에 자격 증명을 커밋하지 마십시오!
1. 열기 `index.js` 외부 애플리케이션 코드와 주석을 검토합니다.
   ...
   */
   ...
   를 검토합니다. `fetch(..)` 의 투자 `listAssetsByFolder(...)` 및 `updateMetadata(...)`, 및 알림 `headers` 정의 `Authorization` 값이 인 HTTP 요청 헤더 `Bearer ACCESS_TOKEN`. 외부 응용 프로그램에서 시작된 HTTP 요청이 AEM as a Cloud Service으로 인증되는 방식입니다.
...
AEM as a Cloud Service 환경에 로그인하여 메타데이터가 업데이트되었는지 확인합니다(에 전달된 동일한 호스트 확인). `aem` 명령줄 매개 변수에 액세스함).
...
1. 를 엽니다. __속성__ 폴더의 모든(컨텐츠 조각 아님) 자산에 대해
1. 탭하여 __고급__ 탭
1. 업데이트된 속성의 값(예: )을 검토합니다 __저작권__ 업데이트한 항목에 매핑됩니다. `metadata/dc:rights` 에 제공된 값을 반영하는 JCR 속성 `propertyValue` 매개 변수(예: __WKND 제한된 사용__
