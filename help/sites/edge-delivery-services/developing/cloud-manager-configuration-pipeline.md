---
title: Edge Delivery 구성 파이프라인을 사용하여 구성 배포
description: Cloud Manager에서 Edge Delivery 구성 파이프라인을 만들고 실행하는 방법에 대해 알아봅니다.
version: Experience Manager as a Cloud Service
feature: Edge Delivery Services
topic: Development
role: Admin, Developer
level: Beginner
doc-type: Technical Video
duration: 120
last-substantial-update: 2025-12-10T00:00:00Z
jira: KT-18126
source-git-commit: 6acf7f1d5c91b29a64b372b678f04b7cb880039b
workflow-type: tm+mt
source-wordcount: '75'
ht-degree: 1%

---


# Edge Delivery 구성 파이프라인

CDN 규칙, Edge Delivery 필터 또는 로그 전달과 같은 구성의 제어된 배포를 허용하는 Adobe [Cloud Manager](https://my.cloudmanager.adobe.com)에서 WAF 구성 파이프라인을 만들고 실행하는 방법을 알아봅니다.

>[!VIDEO](https://video.tv.adobe.com/v/3458975/?learn=on&enablevpops)

구성 옵션 및 형식에 대한 전체 개요는 [Edge Delivery 구성 파이프라인 설명서](https://experienceleague.adobe.com/ko/docs/experience-manager-cloud-service/content/operations/config-pipeline#yamls-for-eds)를 참조하십시오.
