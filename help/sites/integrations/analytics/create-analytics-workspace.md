...
1. 구성 [전환 변수(eVar)](https://experienceleague.adobe.com/docs/analytics/admin/admin-tools/conversion-variables/conversion-var-admin.html) 및 [성공 이벤트(이벤트)](https://experienceleague.adobe.com/docs/analytics/admin/admin-tools/success-events/success-event.html) 를 반환합니다.
...
1. 각 eVar 및 **저장** 변경 사항 다음 섹션에서 이러한 eVar를 사용하여 Analysis Workspace 프로젝트를 만듭니다. 따라서 사용자에게 친숙한 이름으로 변수를 쉽게 검색할 수 있습니다.
...
1. 클릭 **전환** > **성공 이벤트**
1. 사용 **새로 추가** 선택 사항을 적용하려면 새 사용자 지정 성공 이벤트를 만들어 CTA 단추 클릭 및 추적 **저장** 변경 사항
   * `Event` : `event8`
   * `Name`:`CTA Click`
   * `Type`:`Counter`
...
다음으로, 새 만들기 [프로젝트](https://experienceleague.adobe.com/docs/analytics/analyze/analysis-workspace/build-workspace-project/freeform-overview.html#analysis-workspace) 사이트 전체에서 CTA 버튼의 성능을 분석할 대시보드를 만들려면
...
다음으로, WKND 사이트 홈 페이지에서 사용자가 CTA(Call to Action) 버튼과 상호 작용하는 방법을 시각적으로 보여주는 표를 만듭니다. 이러한 표현을 빌드하려면 [클릭한 구성 요소를 Adobe Analytics에서 추적](./track-clicked-component.md). 다음은 WKND Site에 대한 작업 호출 버튼을 사용하여 사용자 상호 작용에 대해 추적된 데이터의 빠른 요약입니다.
...
1. 을(를) 클릭합니다. **파일 선택** 선택 사항을 클릭하여 시스템에서 탭으로 구분된 파일을 업로드한 다음 **파일 가져오기**
...
1. Analytics 도구 모음에서 를 선택합니다 **작업 공간** 에서 만든 작업 영역을 엽니다. [Analysis Workspace에서 새 프로젝트 만들기](#workspace-project) 이 자습서의 섹션.
