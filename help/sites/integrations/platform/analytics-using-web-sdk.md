...
에서 설정 단계를 완료했습니다. **[Experience Platform 웹 SDK 통합](./web-sdk.md)** 튜토리얼.
...
Platform Web SDK를 사용하여 AEM과 Analytics의 통합 프로세스를 살펴보기 전에 _필수 구성 요소 및 주요 요소 요약_ 에서 설정된 [Experience Platform 웹 SDK 통합](./web-sdk.md) 튜토리얼. 이는 통합을 위한 견고한 기반을 제공합니다.
...
이 자습서에서는  `eVar5, eVar6, and eVar7` track  _WKND 페이지 이름, WKND CTA ID 및 WKND CTA 이름_ 및 `event7` 추적에 사용됨  _WKND CTA 클릭 이벤트_.
...
데이터 스트림은 Platform Edge Network에 수집된 데이터를 보낼 위치를 지시합니다. 다음에서 [이전 자습서](./web-sdk.md): Experience Platform에 데이터를 전송하도록 데이터 스트림이 구성됩니다. 이 데이터스트림은 데이터를에 구성된 Analytics 보고서 세트로 보내도록 업데이트됩니다. [위](#setup-analytics---report-suite-analysis-workspace) 단계.
...
다음에서 [이전 자습서](./web-sdk.md), 태그 속성이 만들어지고, 페이지 보기 데이터를 수집, 매핑 및 전송하는 데이터 요소와 규칙이 있습니다. 다음을 위해 향상되어야 합니다.

+ 페이지 이름 매핑 대상 `eVar5`
+ 트리거 **페이지 보기** Analytics 호출(또는 비콘 보내기)
+ Adobe 클라이언트 데이터 레이어를 사용하여 CTA 데이터 수집
+ CTA ID 및 이름 매핑 대상 `eVar6` 및 `eVar7` 각각. 또한 CTA 클릭 수 `event7`
+ 트리거 **링크 클릭** Analytics 호출(또는 비콘 보내기)
...
데이터 집합은 스키마를 따르는 데이터베이스 테이블과 같은 데이터 수집을 위한 저장 및 관리 구성입니다. 에서 만들어진 데이터 세트 [이전 자습서](./web-sdk.md) 는 페이지 보기 및 CTA 클릭 데이터가 Experience Platform 데이터 세트에 수집되는지 확인하는 데 재사용됩니다. 데이터 세트 UI 내에서 총 레코드, 크기 및 수집된 일괄 처리와 같은 다양한 세부 정보가 시각적으로 호소력 있는 막대 그래프와 함께 표시됩니다.
