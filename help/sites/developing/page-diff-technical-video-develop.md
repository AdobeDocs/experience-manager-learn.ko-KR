...
# 페이지 차이에 대한 개발 {#developing-for-page-difference}

이 비디오는 AEM Sites의 페이지 차이 기능에 대한 사용자 지정 스타일을 제공하는 방법을 보여줍니다.

## 페이지 차이 스타일 사용자 정의 {#customizing-page-difference-styles}
...
AEM 페이지 차이는 OOTB CSS를 직접 로드하여 얻습니다 `/libs/cq/gui/components/common/admin/diffservice/clientlibs/diffservice/css/htmldiff.css`.
...
프로젝트의 `authoring` clientlib이 `/apps/my-project/clientlib/authoring.`
...
재정의 스타일을 제공할 수 `authoring` `css.txt` 있는 보다 작은 파일을 가리키는 프로젝트의 clientlib에 추가합니다. [이](https://lesscss.org/) 예에서 활용되는 클래스 래핑 등 편리한 여러 기능으로 인해 Less가 더 선호됩니다.
...
스타일 `less` 재정의를 포함하는 파일을 만들고 필요한 경우 `/apps/my-project/clientlibs/authoring/css/htmldiff.less`오버링 스타일을 제공합니다.
...
제작 clientlibs 카테고리를 프로젝트 기본 페이지의 태그 `/apps/my-project/components/structure/page/customheaderlibs.html` 바로 앞에 포함시켜 스타일을 `</head>` 로드할 수 있습니다.
