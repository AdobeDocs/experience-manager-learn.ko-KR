...
Dynamic Media은 만들기(및 업로드), 작성자(및 게시) 및 게재 워크플로우 프로세스를 지원합니다. 에셋을 업로드한 다음 이미지 세트를 구축하고 최종적으로 게시하여 라이브로 만드는 것과 같은 작업을 수행합니다. 빌드 단계는 일부 워크플로우에서 선택 사항입니다. 예를 들어, 동적 크기 조정 및 이미지 확대만 수행하거나 스트리밍을 위해 비디오를 변환 및 게시하는 것이 목표라면 필요한 빌드 단계는 없습니다.
...
의 전체 목록 보기 [지원되는 파일 유형](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/uploading-files.html#supported-asset-file-formats).
...
- Dynamic Media Classic FTP 서버에서 [방법 알아보기](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/uploading-files.html#upload-files-using-via-ftp).
...
>[!TIP]
>
>폴더를 수동으로 추가하지 마십시오. 대신 FTP에서 업로드를 실행하고 **하위 폴더 포함** Dynamic Media Classic 내에서 폴더 구조를 다시 만드는 옵션입니다.

가장 중요한 두 업로드 옵션은 기본적으로 활성화되어 있습니다. **게시로 표시**&#x200B;앞에서 설명한 대로 **덮어쓰기**. 덮어쓰기는 업로드되는 파일의 이름이 시스템에 이미 있는 파일과 같은 경우 새 파일이 기존 버전을 대체함을 의미합니다. 이 옵션을 선택 취소하면 파일이 업로드되지 않을 수 있습니다.
...
**Dynamic Media Classic에 업로드하기 전에 이미지를 업샘플링하거나 축소하지 마십시오.** 예를 들어 2000픽셀 이미지로 만들기 위해 더 작은 이미지를 업샘플링하지 마십시오. 좋지 않을 거야. 업로드하기 전에 이미지를 가능한 한 완벽에 가깝게 만듭니다.

**확대/축소에 대한 최소 크기는 없지만 기본적으로 뷰어는 100% 이상 확대하지 않습니다.** 이미지가 너무 작으면 이미지가 전혀 확대되지 않거나 손상되지 않도록 작은 양만 축소됩니다.

**이미지 크기에 대한 최소 요구 사항은 없지만 대용량 이미지는 업로드하지 않는 것이 좋습니다.** 거대한 이미지는 4000픽셀 이상으로 간주될 수 있습니다. 이 크기의 이미지를 업로드하면 이미지의 먼지 또는 털의 그레인과 같은 잠재적인 결함을 보여줄 수 있습니다. 이러한 이미지는 Dynamic Media Classic 서버에서 더 많은 공간을 차지하므로 계약된 스토리지 한도를 초과할 수 있습니다.

자세히 알아보기 [파일 업로드 중](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/uploading-files.html#uploading-your-files).
...
게시 는 게시로 표시된 모든 컨텐츠를 게시하며 컨텐츠만 게시하지 않습니다. 일반적으로 단일 관리자는 게시를 실행하는 개별 사용자가 아닌 모든 사용자를 대신하여 게시합니다. 관리자는 필요에 따라 게시하거나 자동으로 게시되는 매일, 매주 또는 10분마다 반복되는 작업을 설정할 수 있습니다. 비즈니스에 적합한 일정에 게시합니다.

>[!TIP]
>
>게시 작업을 자동화하고 매일 오전 12:00이나 저녁 늦은 시간에 실행되도록 전체 게시 일정을 예약합니다.
...
첫 번째 예에서 이미지 &quot;Backpack_A&quot;에 대한 가상 경로는 `http://sample.scene7.com/is/image/s7train/BackpackA`. 이미지 서버 수정자는 이미지 크기를 250픽셀 너비(wid=250)로 조정하고, 크기 조정 시 선명해지는 Lanczos 보간 알고리즘(resMode=sharp2)을 사용하여 이미지를 재샘플링합니다.
...
- 자세히 알아보기 [게시 작업 만들기](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/publishing-files.html#creating-a-publish-job).
- 자세히 알아보기 [게시](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/publishing-files.html).

## 3단계: 제공

Dynamic Media Classic 워크플로의 최종 제품은 자산을 가리키는 URL이라는 점을 기억하십시오. URL은 개별 이미지, 이미지 세트, 스핀 세트 또는 기타 이미지 세트 컬렉션이나 비디오를 가리킬 수 있습니다. 해당 URL을 가져와서 HTML을 편집하는 등, 다음과 같은 작업을 수행해야 합니다. `<IMG>` 태그는 현재 사이트에서 오는 이미지를 가리키지 않고 Dynamic Media Classic 이미지를 가리킵니다.
...
업로드했거나 생성 또는 편집 중인 에셋을 미리 보고 고객이 에셋을 볼 때 원하는 대로 표시되도록 해야 할 수 있습니다. 다음을 클릭하여 미리보기 창에 액세스할 수 있습니다. **미리 보기** 자산 축소판의 맨 위에 있는 단추 **패널 찾아보기/빌드**&#x200B;또는 로 이동하여 **파일 > 미리 보기**. 브라우저 창에서 이미지, 비디오 또는 이미지 세트처럼 빌드된 자산인지 여부에 관계없이 현재 패널에 있는 자산을 미리 봅니다.

### 동적 크기 미리 보기(이미지 사전 설정)

다음을 사용하여 이미지를 여러 크기로 미리 볼 수 있습니다. **크기** 미리 보기. 사용 가능한 이미지 사전 설정 목록이 로드됩니다. 나중에 이미지 사전 설정에 대해 논의하겠지만, 특정 선명하게 하기 및 이미지 품질을 사용하여 지정된 크기로 이미지를 로드하기 위한 &quot;레시피&quot;로 생각해 보겠습니다.

### 확대/축소 미리 보기

다음을 사용할 수도 있습니다 **확대/축소** 포함된 다양한 확대/축소 뷰어를 기반으로 하는 미리 작성된 여러 확대/축소 사전 설정 중 하나에서 이미지를 미리 보는 옵션입니다.

자세히 알아보기 [자산 미리 보기](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/managing-assets/previewing-asset.html).
