...
> `Note:` 작동하는 동안 AEM의 상태가 정상인지 실제 측정이 없습니다.  Dispatcher(Apache 웹 서버)가 작동 및 실행 중인지 여부만 테스트합니다.

## AMS 상태 확인

비정상 AEM 인스턴스를 앞두고 있는 정상 Dispatcher에 트래픽을 보내지 않기 위해 AMS는 Dispatcher뿐만 아니라 다리의 상태를 평가하는 몇 가지 추가 기능을 만들었습니다.
...
이는 AEM이 작동 중인지 확인하기 위해 테스트할 수 있는 경량 엔드포인트입니다.
...
> `Note:` 게시 AEM 인스턴스 또는 작성자 AEM 인스턴스가 비정상인 경우 dispatcher가 서비스를 중단할 수 있습니다.  그 중 한 명이 건강하다면 트래픽도 받지 못한다는 의미입니다
...
> `Note:` 상태 검사가 원하는 대로 작동하지 않고 재정의가 로드 밸런서에 AEM 인스턴스를 유지하도록 허용하는 경우 이 스크립트가 사용됩니다.
