...
![트래픽 흐름을 보여 주는 애니메이션 GIF](assets/load-balancer-healthcheck/health-check.gif "상태 확인 단계")

## 기본 로드 밸런서 상태 확인

고객 트래픽이 인터넷을 통해 AEM 인스턴스에 도달하면 로드 밸런서를 통과합니다

![이 그림에서는 로드 밸런서를 통해 인터넷에서 aem으로 이동하는 트래픽 흐름을 보여 줍니다](assets/load-balancer-healthcheck/load-balancer-traffic-flow.png "load-balancer-traffic-flow")
...
> `Note:` 이것이 작동하는 동안 AEM이 건강한지 아닌지에 대한 실제 측정은 없습니다.  Dispatcher(Apache 웹 서버)가 작동 및 실행 중인지 여부만 테스트합니다.

## AMS 상태 확인

비정상 AEM 인스턴스와 직면한 건강한 Dispatcher로 트래픽을 보내지 않도록 AMS는 Dispatcher뿐만 아니라 다리의 상태를 평가하는 몇 가지 추가 기능을 만들었습니다.

![이 그림에서는 상태 검사가 작동하는 다양한 부분을 보여 줍니다](assets/load-balancer-healthcheck/health-check-pieces.png "건강검진구")
...
![이 그림에서는 AMS 에이전트 페이지를 보여 줍니다.](assets/load-balancer-healthcheck/health-check-page.png "health-check-page")

> `Note:` 페이지를 캐시할 수 없는지 확인합니다.  캐시된 페이지를 반환할 때마다 실제 상태를 확인하지 않습니다!
...
![이 그림에서는 AWS 로드 밸런서 상태 확인 구성을 보여 줍니다](assets/load-balancer-healthcheck/aws-settings.png "aws-lb-settings")

![이 그림에서는 Azure 로드 밸런서 상태 검사 구성을 보여 줍니다.](assets/load-balancer-healthcheck/azure-settings.png "azure-lb-settings")

### Apache 상태 검사 가상 호스트

#### CGI-BIN 가상 호스트 `(/etc/httpd/conf.d/available_vhosts/ams_health.vhost)`

다음 은 `<VirtualHost>` CGI-Bin 파일을 실행할 수 있는 Apache 구성 파일입니다.
...
이 파일의 이름은 다음과 같습니다 `000_` 를 접두사로 사용하십시오.  라이브 사이트와 동일한 도메인 이름을 사용하도록 의도적으로 구성되었습니다.  상태 검사에서 AEM 백엔드 중 하나에 문제가 있음을 감지하면 이 파일을 사용할 수 있게 됩니다.  그런 다음 페이지 없이 503 HTTP 응답 코드 대신 오류 페이지를 제공합니다.  그것은 평상시보다 더 많은 트래픽을 빼앗아갈 것이다 `.vhost` 그 전에 로드되었기 때문에 파일 `.vhost` 동일한 파일을 공유하는 동안 `ServerName` 또는 `ServerAlias`.  결과적으로 특정 도메인으로 향하는 페이지가 정상적인 트래픽이 통과하는 기본 페이지가 아닌 비정상 vhost로 이동합니다.
...
에서 다시 로드 모드 설정을 구성하여 작성자 또는 게시된 사이트가 이 오류 페이지를 로드할 수 있는지 여부를 제어할 수 있습니다. `/var/www/cgi-bin/health_check.conf`
...
을(를) 볼 때 `VirtualHost` 이를 설정하면 활성화될 때 발생하는 각 요청에 대해 오류 페이지와 동일한 문서가 로드됩니다.
...
CSE가 로드 밸런서에서 Dispatcher를 가져올 때 비헤이비어 또는 기준을 변경하는 로드 밸런서 설정에 구성할 수 있는 5개의 스크립트가 있습니다.

#### /bin/checkauthor

이 스크립트를 사용하면 프론트하고 있는 인스턴스를 확인하고 기록하지만 `author` AEM 인스턴스가 비정상 상태입니다.

> `Note:` 게시 AEM 인스턴스가 비정상인 경우 트래픽이 작성자 AEM 인스턴스로 흐를 수 있도록 Dispatcher가 서비스 상태를 유지합니다

#### /bin/checkpublish(기본값)

이 스크립트를 사용하면 프론트하고 있는 인스턴스를 확인하고 기록하지만 `publish` AEM 인스턴스가 비정상 상태입니다.

> `Note:` 작성자 AEM 인스턴스가 비정상인 경우 트래픽이 게시 AEM 인스턴스로 유입될 수 있도록 Dispatcher가 서비스 상태를 유지합니다

#### /bin/checkeither

이 스크립트를 사용하면 프론트하고 있는 인스턴스를 확인하고 기록하지만 `author` 또는 `publisher` AEM 인스턴스가 비정상 상태입니다.

> `Note:` 게시 AEM 인스턴스 또는 작성자 AEM 인스턴스가 비정상인 경우 Dispatcher가 서비스를 중단한다는 점을 기억하십시오.  그 중 한 명이 건강하다면 트래픽도 받지 못한다는 의미입니다

#### /bin/checkboth

이 스크립트를 사용하면 프론트하고 있는 인스턴스를 확인하고 기록하지만 `author` 및 `publisher` AEM 인스턴스가 비정상 상태입니다.
...
> `Note:` 이 스크립트는 상태 검사가 원하는 대로 작동하지 않고 재정의가 로드 밸런서에 AEM 인스턴스를 유지하도록 하는 경우에 사용됩니다.
